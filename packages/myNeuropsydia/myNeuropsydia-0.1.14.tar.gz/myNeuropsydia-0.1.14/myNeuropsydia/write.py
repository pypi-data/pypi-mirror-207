...
# ==============================================================================
# ==============================================================================
def write(text="Write something here", 
    ...
    *Dependencies*
    ...
    """
    if fast is True:
        size = int(size*scr.screen_width/35.0)
        x, y = Coordinates.to_pygame(x=x, y=y)
        if style != 'body':
            font = scr.Font.get(style, size)
        else:
            font = scr.Font.get('RobotoRegular.ttf', size)
        surface = font.render(text, True, core_color(color))
        rectangle = surface.get_rect()
        rectangle.center = (x, y)
        scr.screen.blit(surface, rectangle)

    else:
        size = int(size*scr.screen_width/35.0)
        outline_size = int(outline_size*scr.screen_width/35.0)
        text = str(text)
        ...
        font = scr.Font.get(font_name,size)
        if outline==True:
            font_outline = scr.Font.get(font_name, size+outline_size)
