...
def load(fn, isIntValue=True, default=None):
	return DB(fn, isIntValue=isIntValue, default=default)
...
class LargeDB(object):
	...
	def inc(self, key, increment=1, default=0):
		key = str(key)
		value = self._db.get(key, default)
		self._db.items[key] = value + increment
		self._db.appendSave(key, value + increment)
