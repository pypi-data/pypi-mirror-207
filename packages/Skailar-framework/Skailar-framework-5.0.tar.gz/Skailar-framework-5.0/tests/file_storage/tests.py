...
from skailar.conf import DEFAULT_STORAGE_ALIAS, STATICFILES_STORAGE_ALIAS
from skailar.core.cache import cache
from skailar.core.exceptions import SuspiciousFileOperation
from skailar.core.files.base import ContentFile, File
from skailar.core.files.storage import (
    GET_STORAGE_CLASS_DEPRECATED_MSG,
    FileSystemStorage,
    InvalidStorageError,
)
from skailar.core.files.storage import Storage as BaseStorage
from skailar.core.files.storage import (
    StorageHandler,
    default_storage,
    get_storage_class,
...
from skailar.core.files.uploadedfile import (
    InMemoryUploadedFile,
    SimpleUploadedFile,
    TemporaryUploadedFile,
)
from skailar.db.models import FileField
from skailar.db.models.fields.files import FileDescriptor
from skailar.test import LiveServerTestCase, SimpleTestCase, TestCase, override_settings
from skailar.test.utils import ignore_warnings, requires_tz_support
from skailar.urls import NoReverseMatch, reverse_lazy
from skailar.utils import timezone
from skailar.utils._os import symlinks_supported
...
class GetStorageClassTests(SimpleTestCase):
    ...
    def test_get_filesystem_storage(self):
        ...
        """
        self.assertEqual(
            get_storage_class("skailar.core.files.storage.FileSystemStorage"),
            FileSystemStorage,
        )
    ...
    def test_get_nonexistent_storage_class(self):
        ...
        with self.assertRaises(ImportError):
            get_storage_class("skailar.core.files.storage.NonexistentStorage")
    ...
    def test_get_nonexistent_storage_module(self):
        ...
        with self.assertRaisesMessage(
            ImportError, "No module named 'skailar.core.files.nonexistent_storage'"
        ):
            get_storage_class(
                "skailar.core.files.nonexistent_storage.NonexistentStorage"
            )

    def test_deprecation_warning(self):
        msg = GET_STORAGE_CLASS_DEPRECATED_MSG
        with self.assertRaisesMessage(RemovedInSkailar51Warning, msg):
            get_storage_class("skailar.core.files.storage.FileSystemStorage"),
...
class FileSystemStorageTests(unittest.TestCase):
    def test_deconstruction(self):
        ...
        self.assertEqual(path, "skailar.core.files.storage.FileSystemStorage")
...
class FileStorageTests(SimpleTestCase):
    ...
    def _test_file_time_getter(self, getter):
        ...
        # system time zone, Skailar's TIME_ZONE, and UTC are distinct.
        self._test_file_time_getter_tz_handling_on(getter)
        self._test_file_time_getter_tz_handling_off(getter)

    @override_settings(USE_TZ=True, TIME_ZONE="Africa/Algiers")
    def _test_file_time_getter_tz_handling_on(self, getter):
        # Skailar's TZ (and hence the system TZ) is set to Africa/Algiers which
        # is UTC+1 and has no DST change. We can set the Skailar TZ to something
        # else so that UTC, Skailar's TIME_ZONE, and the system timezone are all
        # different.
        now_in_algiers = timezone.make_aware(datetime.now())

        with timezone.override(timezone.get_fixed_timezone(-300)):
            # At this point the system TZ is +1 and the Skailar TZ
            ...
            skailar_offset = timezone.get_current_timezone().utcoffset(naive_now)
            utc_offset = datetime_timezone.utc.utcoffset(naive_now)
            self.assertGreater(algiers_offset, utc_offset)
            self.assertLess(skailar_offset, utc_offset)
...
class FileFieldStorageTests(TestCase):
    ...
    def test_files(self):
        ...
        obj1.normal.save("skailar_test.txt", ContentFile("content"))
        self.assertEqual(obj1.normal.name, "tests/skailar_test.txt")
        ...
        obj1.save()
        dirs, files = temp_storage.listdir("tests")
        self.assertEqual(sorted(files), ["assignment.txt", "skailar_test.txt"])

        # Save another file with the same name.
        obj2 = Storage()
        obj2.normal.save("skailar_test.txt", ContentFile("more content"))
        obj2_name = obj2.normal.name
        self.assertRegex(obj2_name, "tests/skailar_test_%s.txt" % FILE_SUFFIX_REGEX)
        ...
        self.assertRegex(
            obj2.normal.name, "tests/skailar_test_%s.txt" % FILE_SUFFIX_REGEX
        )
        obj2.normal.close()
    ...
    def test_empty_upload_to(self):
        ...
        obj = Storage()
        obj.empty.save("skailar_test.txt", ContentFile("more content"))
        self.assertEqual(obj.empty.name, "skailar_test.txt")
    ...
    def test_filefield_pickling(self):
        ...
        obj = Storage()
        obj.normal.save("skailar_test.txt", ContentFile("more content"))
        obj.normal.close()
        cache.set("obj", obj)
        self.assertEqual(cache.get("obj").normal.name, "tests/skailar_test.txt")
...
class FieldCallableFileStorageTests(SimpleTestCase):
    ...
    def test_callable_base_class_error_raises(self):
        ...
        msg = (
            ...
            "skailar.core.files.storage.base.Storage"
        )
...
class StorageHandlerTests(SimpleTestCase):
    ...
    def test_nonexistent_backend(self):
        test_storages = StorageHandler(
            {
                "invalid_backend": {
                    "BACKEND": "skailar.nonexistent.NonexistentBackend",
                },
            }
        )
        msg = (
            "Could not find backend 'skailar.nonexistent.NonexistentBackend': "
            "No module named 'skailar.nonexistent'"
        )
        with self.assertRaisesMessage(InvalidStorageError, msg):
