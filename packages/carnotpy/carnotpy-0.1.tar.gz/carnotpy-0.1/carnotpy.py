...
def process_date_range(payload, date_range):
    if payload['useCustomRange']:
        if payload['useCustomRelativeRange']:
            ...
            if rel_range in ['yesterday', 'today']:
                new_range = {
            ...
            payload['date_range'] = [new_range['start'], new_range['end']]

        elif payload.get('customRange'):
            if payload['customRangeStartOnly'] and date_range and date_range['end']:
