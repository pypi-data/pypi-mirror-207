...
class BaseContextManager:
    ...
    def __init__(self, context_map: ContextMap):
        ...
        self._events = EventSource(['context-sync', 'auth-begin', 'auth-disabled', 'auth-end'])
        ...
        if not has_context_before:
            ...
            if exact_url_requested:
                ...
                if registry_url:
                    ...
                    raise InvalidServiceRegistryError(f'The given URL ({registry_hostname_or_url}) is not the root URL of the service registry.')
            else:
                registry = self._scan_for_registry_endpoint(target_hostname)
                if not registry:
                    raise InvalidServiceRegistryError(f'The given hostname ({registry_hostname_or_url}) is not a hostname of the service registry service.')
