...
class Route:
    ...
    def __init__(
        ...
        self.parameters: dict[str, Parameter] = {y.name: copy.copy(y) for y in parameters}
    ...
    def get_return(self, forceData=False, return_status=None):
        ...
        if "txt" in format_:
            ...
            route_output = jsonify(data) if "make" in format_ else ujson.dumps(data)
        else:
            # returned = jsonify(self.returned)
            route_output = self.returned.to_json()
            if not "make" in format_:
                route_output = ujson.dumps(route_output)
