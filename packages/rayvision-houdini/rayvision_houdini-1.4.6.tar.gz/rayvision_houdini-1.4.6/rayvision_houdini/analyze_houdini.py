...
class AnalyzeHoudini(object):
    ...
    def _get_install_path(self, version):
        ...
        try:
            ...
            location, type_name = _winreg.QueryValueEx(handle, "InstallPath")
            self.logger.debug("%s %s", location, type_name)
    ...
    def location_from_reg(self, version):
        ...
        When the smaller version does not match,
         it looks for the larger version,
        ...
        try:
            ...
            self.logger.debug("User local houdini software versions===> %s",
                              version_list)
        try:
            if len(version_list):
                ...
                if folder_name not in version_list:
                    ...
                    for ver in version_list:
                        if b_ver in ver:
                            ...
                            self.logger.info("Match large version===>",
                                             b_version)
                            return self._get_install_path(b_version)

                    error_msg = "Your houdini Software version" \
                                " is not any match"
    ...
    def analyse_cg_file(self):
        ...
        if version != self.software_version:
            self.add_tip(tips_code.CG_NOTMATCH, "{} {}".format(
