...
class UpdateUserDataUsageJobData(BaseModel):
    channel_id: str
    bot_name: str
    filtered: bool
    user_ids: List[str]
...
class UpdateUserDataUsageJob(ABC):
    @property
    ...
    def exec(self, data: UpdateUserDataUsageJobData):
        ...
        for user in users:
            ...
            if dedicated_bot and not dedicated_bot.invalid_creds:
                self.increment(f"{user.id}", data.filtered, bot_name=data.bot_name, dedicated_bot_id=dedicated_bot.id)
