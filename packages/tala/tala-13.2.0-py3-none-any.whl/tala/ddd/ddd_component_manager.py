...
class DDDComponentManager(object):
    ...
    def get_ddd_specific_components(self, name, domain_orchestration=None):
        if name not in self._components_of_ddds and domain_orchestration:
            self._load_ddd(name, domain_orchestration)
        return self._components_of_ddds[name]

    def _load_ddd(self, name, domain_orchestration):
        if name not in domain_orchestration.ddd_names:
            raise UnexpectedDDDException(f"Expected one of the known DDDs {domain_orchestration.ddd_names}, but got '{name}'")
        if not self._is_loaded(name):
            ddd_as_json = self._get_ddd_as_json(name, domain_orchestration)
            self._parse_and_add(ddd_as_json)

    def _is_loaded(self, ddd_name):
        return ddd_name in self._components_of_ddds

    def _get_ddd_as_json(self, name, domain_orchestration):
        for ddd in domain_orchestration.ddds_as_json:
    ...
    def get_device_classes(self):
        return [ddd.device_class for ddd in list(self._components_of_ddds.values()) if ddd.device_class is not None]
    ...
    def get_ddd_of_device_name(self, device_name):
        ...
        raise UnexpectedDeviceClassException(
            "Expected to find '%s' among known device classes %s but did not." % (device_name, device_class_names)
        )
