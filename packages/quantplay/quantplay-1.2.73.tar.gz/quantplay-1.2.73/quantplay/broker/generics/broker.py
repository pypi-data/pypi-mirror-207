...
class Broker():
    ...
    def exit_all_trigger_orders(self, tag="ALL", symbol_contains=None):
        stoploss_orders = self.orders(order_type="TRIGGER PENDING")
        stoploss_orders = stoploss_orders[stoploss_orders.status == "OPEN"]
        ...
        for stoploss_order in stoploss_orders:
            ...
            self.modify_order(order_id=stoploss_order['order_id'],
    ...
    @staticmethod
    def add_transaction_charges(orders):
        ...
        brokerage_charges_choices = [
            0.0003 * orders.quantity * orders.average_price,
            0,
            20,
            20,
        ]
