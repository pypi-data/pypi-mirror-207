...
class TinyB(object):
    ...
    async def create_token(self, name: str, scope: str, origin_code: Optional[str], origin_resource_name_or_id: Optional[str]):
        ...
        data = {
            ...
            'scope': scope,
            'origin': origin,
            'resource_id': origin_resource_name_or_id
        }
        return await self._req("/v0/tokens", method='POST', data=data)

    async def alter_tokens(self, name: str, scopes: List[str]):
        data = {'scope': scope for scope in scopes}
        return await self._req(f"/v0/tokens/{name}", method='PUT', data=data)
