...
from rc.utils.folder_file import check_empty_dirs
...
def model_upload(message, repo, model_version):
    if check_git_add_untrack_files():
        print('Untracked files present (use "git add" to track)')
        sys.exit()
    if check_git_commit_files():
        print('Untracked files present (use "git commit" to track)')
        sys.exit()
    if check_push_left():
        print('Use "git push" to publish your local commits')
        sys.exit()
    if check_git_deleted_files():
        print('File deleted (use "git commit" to track)')
        sys.exit()
