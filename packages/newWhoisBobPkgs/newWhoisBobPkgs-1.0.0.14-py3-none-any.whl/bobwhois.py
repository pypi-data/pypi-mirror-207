...
def GetInfo(ip, i=0):
    try:
        w = IPWhois(ip, timeout=5).lookup_rdap()
        print(f'lookup succeeded - [{i}]')
    except:
        print(f'lookup failed - [{i}]')
        return {'state' : False}
    _net = w["network"]
    _obj = w["objects"]
    red = "\x1b[0;31m"
    ...
    if header.startswith('/help'):
        ...
        if re.search(regex_ip, response['text']):
            ...
            if re.match(regex_ip_withFile_param, header):
                fName_sort = re.findall(regex_ip_withFile_param, header)[0]
            files = response['files'][0]
            response = requests.request("GET", files, headers=headers)
            response.raise_for_status()
            response.encoding="UTF-8"
        ...
        if len(outputs_ip) > 10:
            with open(f'list_ip{fName_sort}.txt', 'w') as f:
                f.write(msg.replace('\x0a', '\x0d\x0a'))
            SendFile(f'list_ip{fName_sort}.txt', roomId, '')
        else:
        ...
        if len(opt_format) > 0 :
            fullPath = f'{fPath_working}/result{fName_sort}.csv'
