            ...
            if not self._disable_hana_execution:
                ...
                with conn.connection.cursor() as cur:
                    ...
                    execute_logged(cur,
                                   ...
                                   conn.sql_tracer,
                                   conn)
                param_rows.extend([tuple(row[1]) for row in self.optim_param_.collect().replace(np.nan, None).iterrows()])#pylint:disable=line-too-long
...
class KNNRegressor(_KNNBase):
    ...
    >>> knn.fit(data=df_regr_train, key='ID', categorical_variable='X1', label='VALUE')
    ...
    >>> res, stats = knn.predict(data=df_class_predict, key='ID', categorical_variable='X1')
