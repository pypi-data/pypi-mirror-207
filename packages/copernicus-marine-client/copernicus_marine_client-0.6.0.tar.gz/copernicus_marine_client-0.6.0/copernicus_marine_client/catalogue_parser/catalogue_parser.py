...
def _fetch_raw_products() -> list[dict[str, Any]]:
    ...
    return map_parallel(
        _fetch_raw_product,
        tqdm(
            raw_catalogue["datasets"].keys(), desc="Fetching DU raw products"
        ),
    )
    ...
    products = [
        ...
        for stac_product in map_parallel(
            ...
            tqdm(
                ...
                desc="Fetching MDS raw products",
            ),
        )
        if stac_product
    ]
    return map_parallel(
        _fetch_stac_datasets, tqdm(products, desc="Fetching MDS raw datasets")
    )
