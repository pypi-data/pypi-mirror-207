...
from skailar.urls import reverse as skailar_reverse
...
def _reverse(viewname, args=None, kwargs=None, request=None, format=None, **extra):
    """
    Same as `skailar.urls.reverse`, but optionally takes a request
    and returns a fully qualified URL, using the request to get the base URL.
    """
    if format is not None:
        kwargs = kwargs or {}
        kwargs['format'] = format
    url = skailar_reverse(viewname, args=args, kwargs=kwargs, **extra)
    if request:
