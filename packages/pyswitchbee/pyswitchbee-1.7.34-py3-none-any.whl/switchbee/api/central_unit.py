...
class CentralUnitAPI(ABC):
    ...
    async def fetch_configuration(
        ...
        logger.error(f'Fetch Configuration: {data}')
        ...
        for zone in data[ApiAttribute.DATA][ApiAttribute.ZONES]:
            for item in zone[ApiAttribute.ITEMS]:
                ...
                # add group switch only of hardware type != VIRTUAL as we can't read their statuses
                    ...
                    logger.error(f"Found Thermostat: {item}")
                    ...
                    logger.error(f"Thermostat: {self._devices_map[item[ApiAttribute.ID]].__dict__}")
    ...
    async def fetch_states(
        ...
        logger.error(f"My Config: {self._devices_map}")
        to_fetch = [
                dev
        ...
        logger.error(f"About to fetch: {to_fetch}")
        ...
        logger.error(f"Result of fetch: {states}")
