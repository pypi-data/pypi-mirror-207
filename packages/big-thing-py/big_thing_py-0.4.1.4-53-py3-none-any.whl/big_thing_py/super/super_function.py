...
class MXSuperFunction(MXFunction):
    ...
    def _print_schedule_result(self):
        MXLOG_DEBUG(f'[SCHEDULE RESULT] ============================================', 'green')
        for super_k, super_execute_req in self._mapping_table.items():
            MXLOG_DEBUG(f'super_key: {super_k} -> super_service: {self._name}', 'green')
            for sub_k, sub_service_req in super_execute_req._sub_service_request_table.items():
                MXLOG_DEBUG(f'    sub_key: {sub_k} -> sub_service: {sub_service_req._sub_service_type.get_name()}|{sub_service_req._sub_service_request_order}', 'green')
                for target_req in sub_service_req._target_request_list:
                    MXLOG_DEBUG(f'        target: {target_req.trigger_msg.sub_service_name}|{target_req.trigger_msg.target_middleware_name}', 'green')
    ...
    def start_super_execute_thread(self, super_execute_msg: MXSuperExecuteMessage, super_service_request_key_table: Dict[str, List[str]]) -> MXThread:
        ...
        request_key = dict(super_service_request_key=super_service_request_key, sub_request_key_list=list(super_service_execute_request._sub_service_request_table))
        if self.get_name() in super_service_request_key_table:
            super_service_request_key_table[self.get_name()].append(request_key)
        else:
            super_service_request_key_table[self.get_name()] = [request_key]
        ...
        super_execute_thread = MXThread(
            ...
            name=f'{self._func.__name__}_{super_service_execute_request._action_type.value}_thread',
            daemon=True,
