...
def get_nexus(anio=2020, cache=False):  
    ...
    df_nexus_group['COD_MOD']=df_nexus_group['COD_MOD'].astype('str')    
    #df_nexus_group.to_hdf('cache.h5', key='df_nexus_codmod', mode='a')
    hc.save_cache(df_nexus_group,filename,key_cache)
    return df_nexus_group

 
def get_ratios_plaza_codmod(df_nexus_):

    df_nexus_.loc[(df_nexus_['ESTPLAZA'].str.contains('Activo')), 'AC'] = 1
    df_nexus_.loc[(df_nexus_['ESTPLAZA'].str.contains('Encargatura')), 'EN'] = 1
    df_nexus_.loc[(df_nexus_['ESTPLAZA'].str.contains('Abandono')), 'AD'] = 1
    df_nexus_.loc[(df_nexus_['ESTPLAZA'].str.contains('Cese')), 'CS'] = 1    
    df_nexus_.loc[(df_nexus_['ESTPLAZA'].str.contains('Designacion')), 'DG'] = 1
    df_nexus_.loc[(df_nexus_['ESTPLAZA'].str.contains('Destaque')), 'DT'] = 1
    df_nexus_.loc[(df_nexus_['ESTPLAZA'].str.contains('Licencia')), 'LC'] = 1

    df_nexus_group = df_nexus_.groupby(['codmod'])[['AC',"EN",'AD',"CS","DG","DT","LC"]].agg("sum").reset_index()
    df_nexus_group['tot_plaz_nex'] = df_nexus_group[['AC',"EN",'AD',"CS","DG","DT","LC"]].sum(axis=1)

    df_nexus_group[f'RATIO_PLZ_ACTIVO_X_CMOD']= df_nexus_group['AC']/df_nexus_group['tot_plaz_nex']
    df_nexus_group[f'RATIO_PLZ_ENCARG_X_CMOD']= df_nexus_group['EN']/df_nexus_group['tot_plaz_nex']
    df_nexus_group[f'RATIO_PLZ_ABANDO_X_CMOD']= df_nexus_group['AD']/df_nexus_group['tot_plaz_nex']
    df_nexus_group[f'RATIO_PLZ_CESE_X_CMOD']= df_nexus_group['CS']/df_nexus_group['tot_plaz_nex']
    df_nexus_group[f'RATIO_PLZ_DESIGN_X_CMOD']= df_nexus_group['DG']/df_nexus_group['tot_plaz_nex']
    df_nexus_group[f'RATIO_PLZ_DESTAQ_X_CMOD']= df_nexus_group['DT']/df_nexus_group['tot_plaz_nex']
    df_nexus_group[f'RATIO_PLZ_LICENCIA_X_CMOD']= df_nexus_group['LC']/df_nexus_group['tot_plaz_nex']

    df_nexus_group.rename(columns={"AC": f"TOTAL_PLZ_ACTIVO_X_CMOD"},inplace=True)
    df_nexus_group.rename(columns={"EN": f"TOTAL_PLZ_ENCARG_X_CMOD"},inplace=True)
    df_nexus_group.rename(columns={"AD": f"TOTAL_PLZ_ABANDO_X_CMOD"},inplace=True)
    df_nexus_group.rename(columns={"CS": f"TOTAL_PLZ_CESE_X_CMOD"},inplace=True)
    df_nexus_group.rename(columns={"DG": f"TOTAL_PLZ_DESIGN_X_CMOD"},inplace=True)
    df_nexus_group.rename(columns={"DT": f"TOTAL_PLZ_DESTAQ_X_CMOD"},inplace=True)
    df_nexus_group.rename(columns={"LC": f"TOTAL_PLZ_LICENCIA_X_CMOD"},inplace=True)
