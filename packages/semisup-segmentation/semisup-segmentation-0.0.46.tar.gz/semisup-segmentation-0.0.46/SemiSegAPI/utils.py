...
def omniData(path, pathUnlabelled, learn, transformations, size=(480,640)):
    ...
    for image in images:
        ...
        for transform in transformations:
            ...
            print(p.shape)
            arr=[]
            for i,arr in enumerate(p[0]):
                if i==0:
                    res=np.expand_dims(getTransformReverse(transform, i),axis=0)
                else:
                    res=np.append(res, np.expand_dims(getTransformReverse(transform, i),axis=0), axis=0)
                print(res.shape)
