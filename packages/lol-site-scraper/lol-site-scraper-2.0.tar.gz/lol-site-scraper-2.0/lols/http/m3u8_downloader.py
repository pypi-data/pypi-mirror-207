...
class M3u8Downloader(HttpClient):
    def __init__(self, url, output_filename, session: requests.Session, tempdir=".", poolsize=1):
    ...
    def mirror_url_resource(self, remote_file_url):
        ...
        # content = get_url_content(remote_file_url, self.session)
        res = self.send_request(method='GET', url=remote_file_url)
    ...
    def download_fragments(self, fragment_urls):
        ...
        for url in fragment_urls:
            if url in self.fragments:
                ...
                continue
            try:
                result = self.download_fragment(url)
                self.fragment_downloaded(result)
            except Exception as e:
                self.fragment_download_failed(e)
    ...
    def process_media_playlist(self, url, content=None):
        ...
        if content is None:
            content = self.send_request(url=url, method='GET').content
