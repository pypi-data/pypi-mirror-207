...
class StyleRegistry:
    ...
    MODIFIER_WIN_STYLE = Style(fg=cv.HI_BLUE, bg=cv.NAVY_BLUE)
...
class Modifier(Fragment, Sized):
    ...
    def format_legend(self) -> Text:
        label_style = Style(self.style, fg=NOOP_COLOR)
        return Fragment(KeyCombo.KEY_PAD + self.label[0], self.style) + Fragment(
...
@cli_command(
    ...
    short_help="display current tmux bindings",
)
