...
def downloadFile(name, media_type, post_text, media_resource_url, audio_resource_url):
...
def processPosts(uuid, data):
    ...
    for it in medias:
        ...
        try:
            downloadAllCount += 1
            downloadFile(f"{uuid}_{idx}", media_type, post_text, media_resource_url, audio_resource_url)
            time.sleep(1)
...
def ftpdownload(url, curGroupId):
    ...
    if urldata.hostname == "192.168.3.220":
        ...
        ftp = FTP('192.168.3.220', 'ftpuser', 'ftpuser')
