...
from hestia_earth.distribution.cycle import sum_pesticides
...
def _should_validate(cycle: dict):
    return cycle.get('completeness', {}).get('pesticidesAntibiotics', False) and not is_empty(sum_pesticides(cycle))
