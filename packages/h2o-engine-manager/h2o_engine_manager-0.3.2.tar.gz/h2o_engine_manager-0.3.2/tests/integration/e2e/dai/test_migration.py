...
@pytest.mark.skipif(
    os.getenv("RUN_MIGRATION", 'False').lower() not in ('true', '1', 't'),
    reason="Cannot be run in shared PR test environment. Run only on the isolated nightly pipeline."
           "More info: https://github.com/h2oai/ai-engine-manager/issues/927"
)
def test_steam_migration(dai_client):
    ...
    try:
        ...
        engine = dai_client.get_engine(workspace_id="default", engine_id="mig1-19")
        engine.resume()
        engine.wait()
        dai = engine.connect()
