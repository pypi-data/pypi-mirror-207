...
from skailar.utils import text
from skailar.utils.functional import lazystr
from skailar.utils.text import format_lazy
from skailar.utils.translation import gettext_lazy, override
...
class TestUtilsText(SimpleTestCase):
    ...
    def test_format_lazy(self):
        self.assertEqual("skailar/test", format_lazy("{}/{}", "skailar", lazystr("test")))
        self.assertEqual("skailar/test", format_lazy("{0}/{1}", *("skailar", "test")))
        self.assertEqual(
            "skailar/test", format_lazy("{a}/{b}", **{"a": "skailar", "b": "test"})
        )
        self.assertEqual(
            "skailar/test", format_lazy("{a[0]}/{a[1]}", a=("skailar", "test"))
        )

        t = {}
        s = format_lazy("{0[a]}-{p[a]}", t, p=t)
        t["a"] = lazystr("skailar")
        self.assertEqual("skailar-skailar", s)
        t["a"] = "update"
