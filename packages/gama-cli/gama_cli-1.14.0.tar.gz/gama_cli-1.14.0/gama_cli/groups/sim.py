...
def base_ue(ue_version: str, memory: str):
    ...
    call(
        f"ue4-docker build --ue-version {ue_version} --cuda={cuda_ver} -basetag=ubuntu{ubuntu_ver} -username={os.environ['GH_USERNAME']} -password={os.environ['API_TOKEN_GITHUB']}{args} --opt credential_mode=secrets --exclude ddc",
