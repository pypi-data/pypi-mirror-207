...
class single_signal_test(object):
    ...
    def factor_ret_test_sheet(self,index_price,fcname,res_df,Price,quantiles,days,savedir):
        ...
        if index_price is not None:
            clean_factor = mate_al.trans_ex_return(clean_factor,index_price)
        qreturn = self.one_factor_return(clean_factor,fcname,days,str(days)+'D',w_method = 'average')
        plottools.factor_plt(qreturn,to,fcname,quantiles,savedir)
    ...
    def short_judge(self,qreturn,fc):
        ...
        if p_top5>0.05 and p_bottom5<=0.05:
...
class multi_factor_test(object):
    ...
    def multif_barra_norm(self,allfactors,quantiles,Bft):
        ...
        df = allfactors.copy()
        for fcname in list(df):
            test_fc = df[[fcname]].copy().rename_axis(['date','symbol'])
            res_df = sst.one_factor_grouper(test_fc,fcname,quantiles)# 序数标准化
            # 风格中性化-回归取残差
            # 注意！：经过barra正则化的因子若再进行rank标准化会导致在barra因子上再次有暴露，尽管这种暴露可能是不真实的
            residual_ols,params_ols = Bft.barra_compose(res_df[[fcname+'_rank']])
            df[fcname] = residual_ols # 中性化之后的因子替换原始因子
        return df

    def multif_select_noisies(self,allfactors,Price,quantiles,days,savedir,index_price = None):
        '''
        输入:
        因子矩阵
        ...
        for fc in list(allfactors):
            ...
            qreturn,clean_factor = sst.factor_ret_test_sheet(index_price,fc,res_df,Price,quantiles,days,savedir)
        ...
        for nzfc in list(noise_factors_df):
            ...
            nz_qreturn,clean_factor = sst.factor_ret_test_sheet(index_price,nzfc,nz_res_df,Price,quantiles,days,savedir)
            # 因子判别
            if sst.noise_judge(nz_qreturn,nzfc) == True:
    ...
    def multif_denoisies(self,noise_factors_list,allfactors):
        ...
        noise_factors_df = self.mat_orthog(allfactors[noise_factors_list],noise_factors_list)
        factordf = pd.concat([other_factors_df,noise_factors_df],axis = 1)
        # 去噪
        other_factors = list(other_factors_df)
        corrdf = self.multif_corr_ana(factordf,noise_factors_list)[0]
        print('相关性详情')
        print(corrdf)
    ...
    def multif_select_short(self,allfactors,Price,quantiles,days,savedir,index_price = None):
        '''
        输入:
        去噪后的因子
        ...
        for fc in list(allfactors):
            ...
            # 因子判别
            if sst.short_judge(qreturn,fc) == True:
                efficient_short_factors.append(fc)

        decorr_short_factors = []
        if len(efficient_short_factors)==0:
            return []
        
        short_factors_df = self.mat_orthog(allfactors[efficient_short_factors],efficient_short_factors)
        for srtfc in list(short_factors_df):
            # 因子测试
            print(srtfc)
            srt_res_df = sst.factor_prepare(short_factors_df,srtfc,quantiles)
            srt_qreturn,clean_factor = sst.factor_ret_test_sheet(index_price,srtfc,srt_res_df,Price,quantiles,days,savedir)
            # 因子判别
            if sst.noise_judge(srt_qreturn,srtfc) == True:
                decorr_short_factors.append(srtfc)

        return decorr_short_factors
    ...
    def batch_factors_test(self,allfactors,Price,quantiles,days,savedir):
        returndict = {}
        sst = single_signal_test()
        for fc in list(allfactors):
            print(fc)
            res_df = sst.factor_prepare(allfactors,fc,quantiles)
            sst.factor_ret_test_sheet(fc,res_df,Price,quantiles,days,savedir)
...
class plot_tools(object):
    ...
    def factor_plt(self,qreturn,to,fc,quantiles,savedir=''):
        from alphalens import utils
        ...
        plt.subplot(221,title = fc+'_cumret_bygroup')
        plt.plot(qreturn.index,qreturn.cumsum(),label = list(qreturn))
        plt.subplot(222,title = fc+'_turnover_bygroup')
        ...
        plt.plot(qreturn.index,(qreturn[quantiles]-qreturn[1]).cumsum(),color="g")
        plt.xticks(rotation=30)
        plt.xticks(ticks=xticks, labels=xlabels)
        try:
