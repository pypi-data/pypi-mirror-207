...
class SES:
    ...
    def check_iam_policies(self,ses_identity):
        """
        Checks if the specified SES identity has IAM policies attached.
        ...
        try:
            ...
            logger.error(
                "error while checking the IAM polices of the identity %s: %s", ses_identity, str(ex))
            check_result = application_constants.ResultStatus.UNKNOWN
        except Exception as ex:
            logger.error(
                "exception while checking the IAM polices of the identity %s: %s", ses_identity, str(ex))
            raise ex
        logger.info(
            "Completed checking the IAM polices of the identity %s", ses_identity)
        return check_result
