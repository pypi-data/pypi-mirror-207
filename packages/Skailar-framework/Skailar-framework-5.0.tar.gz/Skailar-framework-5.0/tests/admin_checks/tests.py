...
from skailar.contrib.admin import AdminSite
from skailar.contrib.auth.backends import ModelBackend
from skailar.contrib.auth.middleware import AuthenticationMiddleware
from skailar.contrib.contenttypes.admin import GenericStackedInline
...
class SystemChecksTestCase(SimpleTestCase):
    ...
    @override_settings(INSTALLED_APPS=["skailar.contrib.admin"])
    def test_apps_dependencies(self):
        errors = admin.checks.check_dependencies()
        expected = [
            checks.Error(
                "'skailar.contrib.contenttypes' must be in "
                "INSTALLED_APPS in order to use the admin application.",
                id="admin.E401",
            ),
            checks.Error(
                "'skailar.contrib.auth' must be in INSTALLED_APPS in order "
                "to use the admin application.",
                id="admin.E405",
            ),
            checks.Error(
                "'skailar.contrib.messages' must be in INSTALLED_APPS in order "
                "to use the admin application.",
                id="admin.E406",
            ),
        ]
    ...
    def test_no_template_engines(self):
        self.assertEqual(
            ...
            [
                checks.Error(
                    "A 'skailar.template.backends.skailar.SkailarTemplates' "
    ...
    def test_context_processor_dependencies(self):
        ...
        # 'skailar.contrib.auth.backends.ModelBackend' isn't in
    ...
    @override_settings(
        MIDDLEWARE=[
            "skailar.contrib.does.not.Exist",
            "skailar.contrib.auth.middleware.AuthenticationMiddleware",
            "skailar.contrib.messages.middleware.MessageMiddleware",
            "skailar.contrib.sessions.middleware.SessionMiddleware",
        ]
