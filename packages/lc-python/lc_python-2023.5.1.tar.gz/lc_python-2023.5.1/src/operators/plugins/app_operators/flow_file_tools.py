...
class Actions:
    ...
    def reindex(flow, as_json=False, longids=False):
        ...
        ax = lambda op: op.get('name').split('.')[-1]
        lu = {
            'ax-src': lambda op: 'src-' + ax(op),
            'ax-snk': lambda op: 'snk-' + ax(op),
            'ax-op': lambda op: 'ax-' + ax(op),
            'any': lambda op: '',
        }
        type_counts = {}
        smalls = set()
        ...
        def new_id(op, longids=longids):
            nr = ''
            t, tc = op['type'], type_counts
            if t.startswith('subflow:'):
                t = op['type'].split(':', 1)[1] + 'i'
                t = '_'.join([c for c in t])
            else:
        ...
        sf_inst = [op for op in flow if op['type'].startswith('subflow:')]
        have_sfi = False
        for id in ids:
            op = all[id]
            ...
            # their type determines the short, i.e. we must replace.
            # -> Works because subflow specs are always before instances:
            if op['type'] == 'subflow':
                for sfi in sf_inst:
                    sfi['type'] = sfi['type'].replace('ow:%s' % id, 'ow:%s' % nid)
        return fs if as_json else json.loads(fs)

    def clean(flow):
        """Removing dangling nodes, i.e. nodes with input but no wires into it"""
        e = lambda *a, **kw: app.info(*a, **kw)
        rm = set()
        all = {op['id']: op for op in flow}
        e('have ops', count=len(all))
        dests = {
            id: all[id] for op in flow for ws in op.get('wires', [[]]) for id in ws
        }
        ...
        flow = [op for op in flow if not op['id'] in rm]
        return flow


get_sf_tmpl = lambda sfi, all: all.get((sfi['type'] + ':').split(':')[1])
