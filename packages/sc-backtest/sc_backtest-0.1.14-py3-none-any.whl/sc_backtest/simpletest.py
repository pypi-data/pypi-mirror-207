...
class simpletest():
    ...
    def simple_pnl_cs(self, x, y, dn=False, ax=None, horizon=3, intra=False, **kwargs):
        ...
        def factor_2_delta(factor_og):
            ...
            if dn:
                ...
                # 统一计算vol
                delta = factor/(factor * ret).groupby(factor.index.date).sum().fillna(0.0).sum(axis=1).std()

                # 分品种计算vol
                # delta = factor.div((factor * ret).groupby(factor.index.date).sum().std(), axis=1)
        ...
        #
        delta_bench = pd.DataFrame(1.0, index=delta.index, columns=delta.columns)
        delta_bench = delta_bench.div((delta_bench * ret).groupby(delta_bench.index.date).sum().std(), axis=1)
    ...
    def universe_pnl_cs(self, x, y, ax=None, dn=False, universe_limit=5, **kwargs):
        if not (isinstance(x, pd.DataFrame)) or not (isinstance(y, pd.DataFrame)):
            print('x and y must be pd.DataFrame')
            return
        ...
        delta = factor_2_delta(factor)
        pnl_sum = (delta * ret).sum().sort_values(ascending=False)

        for c in pnl_sum.index[:universe_limit]:
            pnl = (delta.loc[:, c] * ret.loc[:, c])
            day_pnl = pnl.groupby(pnl.index.date).sum()

            # report
            sr = round(day_pnl.mean() / day_pnl.std() * 16.0, 2)
            pnl_y = round(day_pnl.mean() * 251, 2)
            pot = round(pnl.sum() / delta.loc[:, c].diff().abs().sum() * 1e4, 2)

            #
            day_pnl.cumsum().plot(ax=ax,
                                  label=f'{c}: ' + 'PnL={:.2f} Sharpe={:.2f} PoT={:.2f}'.format(pnl_y, sr, pot))

        if 'title' in kwargs:
            title = kwargs.pop('title')
            ax.set_title(f'Universe PnL (info: {str(title)})')
        else:
            ax.set_title('Universe PnL')
    ...
    def plot_composite_cs(self, x, y,
                          markout_periods=20, cdf_period2=5, horizon=3, universe_limit=5,
                          fig_return=False, intra=False, dn=False,
                          **kwargs):
        """"""
        if len(x) != len(y):
            print(F'mismatched length of x, y, len(x) == {len(x)} len(y) == {len(y)}')
        ...
        if 'ic' in kwargs:
            if kwargs.pop('ic'):
                _ic = round(self.ic(x.stack(), y.stack()), 3)
                print('IC: ', _ic)
        ...
        # cdf
        ax1 = plt.subplot2grid((4, 2), (0, 0), fig=fig)
        self.plot_cdf(x.stack(dropna=False), y.stack(dropna=False), ax=ax1)
        ax1.set_title('CDF 1Period')

        ax2 = plt.subplot2grid((4, 2), (0, 1), fig=fig)
        ...
        ax3 = plt.subplot2grid((4, 2), (1, 0), fig=fig)
        self.plot_markout_quantile(x.stack(dropna=False), Y, ax=ax3, islegend=True)
        ax3.set_title('Markout')
        ax3.legend()

        # quantile true ret
        ax4 = plt.subplot2grid((4, 2), (1, 1), fig=fig)
        self.plot_quantile_ret(x.stack(dropna=False), y.stack(dropna=False), ax=ax4)
        ax4.set_title('Quantile Ret Mean')

        # universe pnl cs
        ax5 = plt.subplot2grid((4, 2), (2, 0), colspan=2, fig=fig)
        self.universe_pnl_cs(x, y, ax=ax5, dn=dn, universe_limit=universe_limit, title=f'IC = {_ic}')

        # simple pnl cs
        ax6 = plt.subplot2grid((4, 2), (3, 0), colspan=2, fig=fig)
        if 'title' in kwargs:
            title = kwargs.pop('title')
        else:
            title = ''

        self.simple_pnl_cs(x, y, ax=ax6, dn=dn, horizon=horizon, title=title, intra=intra)
