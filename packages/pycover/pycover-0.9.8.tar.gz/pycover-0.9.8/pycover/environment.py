...
from .exceptions import TooOldFipperVersion
...
class Environment:
    def __init__(
        self,
        min_js_version: str,
        min_fipper_version: str,
        min_telethon_version: str,
        client_name: str,
    ):
        self._REQUIRED_NODEJS_VERSION = min_js_version
        self._REQUIRED_FIPPER_VERSION = min_fipper_version
    ...
    def check_environment(self):
        ...
        if self._client_name == 'fipper':
            import fipper
            if VersionManager.version_tuple(
                fipper.__version__,
            ) < VersionManager.version_tuple(
                self._REQUIRED_FIPPER_VERSION,
            ):
                raise TooOldFipperVersion(
                    self._REQUIRED_FIPPER_VERSION,
                    fipper.__version__,
                )
