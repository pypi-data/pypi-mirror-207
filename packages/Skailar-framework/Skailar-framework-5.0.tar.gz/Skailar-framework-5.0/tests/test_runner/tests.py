...
from skailar.core.management.base import SystemCheckError
from skailar.test import SimpleTestCase, TransactionTestCase, skipUnlessDBFeature
from skailar.test.runner import (
    DiscoverRunner,
    Shuffler,
    _init_worker,
    reorder_test_bin,
...
from skailar.test.testcases import connections_support_transactions
...
class ManageCommandParallelTests(SimpleTestCase):
    ...
    @mock.patch.dict(os.environ, {"SKAILAR_TEST_PROCESSES": "7"})
    def test_no_parallel_skailar_test_processes_env(self, *mocked_objects):
        with captured_stderr() as stderr:
            call_command("test", testrunner="test_runner.tests.MockTestRunner")
        self.assertEqual(stderr.getvalue(), "")

    @mock.patch.dict(os.environ, {"SKAILAR_TEST_PROCESSES": "invalid"})
    def test_skailar_test_processes_env_non_int(self, *mocked_objects):
    ...
    @mock.patch.dict(os.environ, {"SKAILAR_TEST_PROCESSES": "7"})
    def test_skailar_test_processes_parallel_default(self, *mocked_objects):
...
class CustomTestRunnerOptionsSettingsTests(AdminScriptTestCase):
    ...
    def test_default_options(self):
        ...
        out, err = self.run_skailar_admin(args)
...
class CustomTestRunnerOptionsCmdlineTests(AdminScriptTestCase):
    ...
    def test_testrunner_option(self):
        ...
        out, err = self.run_skailar_admin(args, "test_project.settings")
        self.assertNoOutput(err)
        self.assertOutput(out, "bar:foo:31337")
...
class NoInitializeSuiteTestRunnerTests(SimpleTestCase):
    @mock.patch.object(multiprocessing, "get_start_method", return_value="spawn")
    @mock.patch(
        "skailar.test.runner.ParallelTestSuite.initialize_suite",
    ...
    def test_no_initialize_suite_test_runner(self, *mocked_objects):
        ...
        with self.assertRaisesMessage(Exception, "initialize_suite() is called."):
            ...
            runner.run_tests(
                [
                    "test_runner_apps.sample.tests_sample.TestSkailarTestCase",
...
class SQLiteInMemoryTestDbs(TransactionTestCase):
    ...
    def test_transaction_support(self):
        ...
        for option_key, option_value in (
            ...
            with mock.patch("skailar.test.utils.connections", new=tested_connections):
...
class SetupDatabasesTests(unittest.TestCase):
    ...
    def test_setup_aliased_databases(self):
        tested_connections = db.ConnectionHandler(
            {
                "default": {
                    "ENGINE": "skailar.db.backends.dummy",
                    "NAME": "dbname",
                },
                "other": {
                    "ENGINE": "skailar.db.backends.dummy",
        ...
        with mock.patch(
            "skailar.db.backends.dummy.base.DatabaseWrapper.creation_class"
    ...
    def test_setup_test_database_aliases(self):
        ...
        with mock.patch("skailar.test.utils.connections", new=tested_connections):
            ...
            self.assertEqual(
                test_databases,
                {
                    ("", "", "skailar.db.backends.dummy", "test_dbname"): (
                        "dbname",
                        ["default", "other"],
                    ),
                },
    ...
    def test_destroy_test_db_restores_db_name(self):
        ...
        with mock.patch("skailar.db.connections", new=tested_connections):
...
class EmptyDefaultDatabaseTest(unittest.TestCase):
    def test_empty_default_database(self):
        ...
        with mock.patch("skailar.db.connections", new=tested_connections):
            ...
            self.assertEqual(
                connection.settings_dict["ENGINE"], "skailar.db.backends.dummy"
            )
...
class RunTestsExceptionHandlingTests(unittest.TestCase):
    def test_run_checks_raises(self):
        ...
        """
        with mock.patch(
            "skailar.test.runner.DiscoverRunner.setup_test_environment"
        ), mock.patch("skailar.test.runner.DiscoverRunner.setup_databases"), mock.patch(
            "skailar.test.runner.DiscoverRunner.build_suite"
        ), mock.patch(
            "skailar.test.runner.DiscoverRunner.run_checks", side_effect=SystemCheckError
        ), mock.patch(
            "skailar.test.runner.DiscoverRunner.teardown_databases"
        ) as teardown_databases, mock.patch(
            "skailar.test.runner.DiscoverRunner.teardown_test_environment"
    ...
    def test_run_checks_passes_and_teardown_raises(self):
        ...
        with mock.patch(
            ...
            "skailar.test.runner.DiscoverRunner.run_checks"
        ), mock.patch(
            "skailar.test.runner.DiscoverRunner.teardown_databases",
            side_effect=ValueError,
        ) as teardown_databases, mock.patch(
            ...
            runner = DiscoverRunner(verbosity=0, interactive=False)
            with self.assertRaises(ValueError):
                # Suppress the output when running TestSkailarTestCase.
