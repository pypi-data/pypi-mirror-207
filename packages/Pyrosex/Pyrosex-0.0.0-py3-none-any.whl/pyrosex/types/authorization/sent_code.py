...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pyrosex.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pyrosex.Client.sign_in` or :meth:`~pyrosex.Client.sign_up`).

        next_type (:obj:`~pyrosex.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pyrosex.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pyrosex.Client.resend_code`.
    """
