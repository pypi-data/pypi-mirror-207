...
class BASED():
    def __init__(self):
        ...
        self.model = joblib_load(os.path.abspath(__file__)[:-9]+"model/based_random_forest.joblib")
    ...
    def __call__(self, image1: Union[PIL.Image.Image, np.array, str], image2: Union[PIL.Image.Image, np.array, str]) -> Union[float, None]:
        ...
        if isinstance(image2, str):
            try:
                image2 = PIL.Image.open(image2).convert("RGB")
            except Exception as e:
                print(e)
                return None
        ...
        if isinstance(image2, PIL.Image.Image):
            image2 = np.array(image2)[...,::-1]
        features = []
        names = []
        for comp in self.components:
            try:
                res = comp(image1, image2)
            except:
                print(F"You have possibly passed images which formats are not supported (PIL.Image, np.array, path/to/image), your types are: image1 :{type(image1)}, image2 :{type(image2)}")
                return None
            if isinstance(res, list):
                for i, elem in enumerate(res):
                    features.append(elem)
                    names.append(comp.__name__ + str(i))
            else:
                features.append(res)
                names.append(comp.__name__)
        metric = self.model.predict(pd.DataFrame([features], columns=names))
