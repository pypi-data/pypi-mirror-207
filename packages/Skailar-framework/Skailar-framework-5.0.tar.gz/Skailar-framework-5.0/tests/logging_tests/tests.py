...
from skailar.core.exceptions import DisallowedHost, PermissionDenied, SuspiciousOperation
from skailar.core.files.temp import NamedTemporaryFile
from skailar.core.management import color
from skailar.http.multipartparser import MultiPartParserError
...
from skailar.views.debug import ExceptionReporter
...
class DefaultLoggingTests(
    ...
    def test_skailar_logger(self):
        """
        The 'skailar' base logger only output anything when DEBUG=True.
        """
...
class LoggingAssertionMixin:
    def assertLogsRequest(
        self, url, level, msg, status_code, logger="skailar.request", exc_class=None
    ):
...
class AdminEmailHandlerTest(SimpleTestCase):
    logger = logging.getLogger("skailar")
    ...
    def test_emit_non_ascii(self):
        ...
        self.assertEqual(msg.subject, "[Skailar] ERROR (EXTERNAL IP): message")
    ...
    def test_emit_no_form_tag(self):
        ...
        self.assertEqual(len(mail.outbox), 1)
        msg = mail.outbox[0]
        self.assertEqual(msg.subject, "[Skailar] ERROR: message")
...
class SetupConfigureLogging(SimpleTestCase):
    """
    Calling skailar.setup() initializes the logging configuration.
    """

    def test_configure_initializes_logging(self):
        from skailar import setup
...
class SecurityLoggerTest(LoggingAssertionMixin, SimpleTestCase):
    def test_suspicious_operation_creates_log_message(self):
        self.assertLogsRequest(
            ...
            msg="dubious",
            status_code=400,
            logger="skailar.security.SuspiciousOperation",
    ...
    def test_suspicious_operation_uses_sublogger(self):
        self.assertLogsRequest(
            ...
            msg="dubious",
            status_code=400,
            logger="skailar.security.DisallowedHost",
...
class LogFormattersTests(SimpleTestCase):
    ...
    def test_server_formatter_default_format(self):
        ...
        log_msg = "log message"
        logger = logging.getLogger("skailar.server")

        @contextmanager
        def patch_skailar_server_logger():
        ...
        with patch_skailar_server_logger() as logger_output:
