...
def get_checkpoint():
    if not os.path.exists(CHECKPOINT_CACHE_PATH):
        ...
        resp = requests.get('https://github.com/Anjok07/ultimatevocalremovergui/releases/download/5.0.2/v5_July_2021_5_Models.zip')
        with open(ZIP_PATH, 'w+b') as fs:
            fs.write(resp.content)
        with ZipFile(ZIP_PATH, 'r') as zipped_vr:
            zipped_vr.extract('models/HP2-4BAND-3090_4band_2.pth', os.path.dirname(CHECKPOINT_CACHE_PATH))
            
        shutil.move(os.path.join(os.path.dirname(CHECKPOINT_CACHE_PATH), 'models/HP2-4BAND-3090_4band_2.pth'), CHECKPOINT_CACHE_PATH)
        shutil.rmtree(os.path.join(os.path.dirname(CHECKPOINT_CACHE_PATH), 'models'), ignore_errors=False, onerror=None)
        os.remove(ZIP_PATH)
...
class CascadedASPPNet(nn.Module):
    ...
    def __init__(self, n_fft):
        ...
        self.stg2_bridge = Conv2DBNActiv(66, 32, 1, 1, 0)
        self.stg2_full_band_net = BaseASPPNet(32, 64)

        self.stg3_bridge = Conv2DBNActiv(130, 64, 1, 1, 0)
...
class ShmartUltimateVocalRemover():
    ...
    def __call__(self, wav_path, tta=False, high_end_process='mirroring', aggressiveness=0.5, is_vocal_model=False, sr=22050):
        ...
        X_spec = {}
        X, sr = librosa.load(wav_path, sr=sr, mono=False, dtype=np.float32, res_type=bp['res_type'])
        ...
        for d in range(bands_n - 1, 0, -1):        
            ...
            X = librosa.resample(X, orig_sr=mp.param['band'][d+1]['sr'], target_sr=bp['sr'], res_type=bp['res_type'])
        ...
        if high_end_process == 'bypass':
            instruments_wave = cmb_spectrogram_to_wave_ffmpeg(y_spec_m, mp, ffmpeg_tmp_filename, input_high_end_h, input_high_end)
        elif high_end_process.startswith('mirroring'):        
            input_high_end_ = mirroring(high_end_process, y_spec_m, input_high_end, mp)
            instruments_wave = cmb_spectrogram_to_wave_ffmpeg(y_spec_m, mp, ffmpeg_tmp_filename, input_high_end_h, input_high_end_)       
        else:
            instruments_wave = cmb_spectrogram_to_wave_ffmpeg(y_spec_m, mp, ffmpeg_tmp_filename)
        ...
        if high_end_process.startswith('mirroring'):        
            ...
            vocals_wave = cmb_spectrogram_to_wave_ffmpeg(v_spec_m, mp, ffmpeg_tmp_filename, input_high_end_h, input_high_end_)       
        else:        
            vocals_wave = cmb_spectrogram_to_wave_ffmpeg(v_spec_m, mp, ffmpeg_tmp_filename)
