...
from skailar.db import connections, router
from skailar.db.utils import ConnectionRouter
from skailar.dispatch import Signal, receiver
from skailar.utils import timezone
from skailar.utils.formats import FORMAT_SETTINGS, reset_format_cache
from skailar.utils.functional import empty
...
def clear_cache_handlers(*, setting, **kwargs):
    if setting == "CACHES":
        from skailar.core.cache import caches, close_caches
...
def update_installed_apps(*, setting, **kwargs):
    if setting == "INSTALLED_APPS":
        ...
        from skailar.contrib.staticfiles.finders import get_finder

        get_finder.cache_clear()
        # Rebuild management commands cache
        from skailar.core.management import get_commands

        get_commands.cache_clear()
        # Rebuild get_app_template_dirs cache.
        from skailar.template.utils import get_app_template_dirs

        get_app_template_dirs.cache_clear()
        # Rebuild translations cache.
        from skailar.utils.translation import trans_real
...
def reset_template_engines(*, setting, **kwargs):
    if setting in {
        ...
        engines._engines = {}
        from skailar.template.engine import Engine

        Engine.get_default.cache_clear()
        from skailar.forms.renderers import get_default_renderer
...
def storages_changed(*, setting, **kwargs):
    ...
    from skailar.core.files.storage import default_storage, storages
...
def file_storage_changed(*, setting, **kwargs):
    if setting == "DEFAULT_FILE_STORAGE":
        from skailar.conf import DEFAULT_STORAGE_ALIAS
        from skailar.core.files.storage import default_storage, storages
...
def root_urlconf_changed(*, setting, **kwargs):
    if setting == "ROOT_URLCONF":
        from skailar.urls import clear_url_caches, set_urlconf
...
def static_storage_changed(*, setting, **kwargs):
    ...
    if setting == "STATICFILES_STORAGE":
        from skailar.conf import STATICFILES_STORAGE_ALIAS
        from skailar.core.files.storage import storages
...
def auth_password_validators_changed(*, setting, **kwargs):
    if setting == "AUTH_PASSWORD_VALIDATORS":
        from skailar.contrib.auth.password_validation import (
...
def user_model_swapped(*, setting, **kwargs):
    if setting == "AUTH_USER_MODEL":
        apps.clear_cache()
        try:
            from skailar.contrib.auth import get_user_model
            ...
            from skailar.contrib.auth import backends

            backends.UserModel = UserModel

            from skailar.contrib.auth import forms

            forms.UserModel = UserModel

            from skailar.contrib.auth.handlers import modwsgi

            modwsgi.UserModel = UserModel

            from skailar.contrib.auth.management.commands import changepassword

            changepassword.UserModel = UserModel

            from skailar.contrib.auth import views
