...
class Untils:
    ...
    @staticmethod
    def get_stock_funds_pool(asofQ,time_length=1.75,still_exist=True):
    ...
    @staticmethod
    # 写一个通用的多元一次回归的算法模型，自变量个数不确定
    def my_general_linear_model_func_industry(A1, b1,stock_wight_ub,stock_wight_lb,industry_ub,industry_lb):
...
class Plot:
    ...
    def plotly_markers(self, df, title_text,fix_range=False):
        ...
        layout = go.Layout(
            title=dict(text=title_text),
            autosize=False, width=fig_width, height=fig_height,
            # yaxis=dict(showgrid=True,range=[-0.005, 0.005]),
