...
from the command line from '{os.getcwd()}'. If you are still having trouble, please contact Chalk support.""",
    ...
    def offline_query(
        ...
        if not isinstance(recompute_features, bool):
            revision = initialized_dataset.revisions[-1]
            revision._show_progress = show_progress
            revision._hydrated = True
            revision.wait_for_completion()
            initialized_dataset.is_finished = True

            ds = initialized_dataset.get_data_as_polars().collect()
            sampled_givens = _to_offline_query_input(
                {col: ds[col] for col in ds.columns if col not in query_input.columns}, None
            )
            response = self._create_dataset_job(
                optional_output=set(optional_output_root_fqns) & set(recompute_features),
                required_output=set(required_output_root_fqns) & set(recompute_features),
                query_input=query_input,
                dataset_name=dataset_name,
                branch=branch,
                context=OfflineQueryContext(environment=environment),
        ...
        revision._hydrated = (
            revision._hydrated or unbranched or (isinstance(wait, float) and wait == 0.0) or not recompute_features
        )
    ...
    def _create_dataset_job(
        ...
        context: OfflineQueryContext,
        recompute_features: Union[bool, List[FeatureReference]] = False,
        sampled_givens: Optional[OfflineQueryInput] = None,
    ) -> DatasetResponse:
        req = CreateOfflineQueryJobRequest(
            output=optional_output,
            required_output=required_output,
