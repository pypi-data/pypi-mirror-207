...
class GeneratorConfig(SingleDecoderConfigBase, GeneratorMixin):
    ...
    def build_vocab(self, *partitions):
        flattened_partitions = [[{'trg_tokens': tokens} for entry in data for tokens in entry['full_trg_tokens']] for data in partitions]
...
class Generator(DecoderBase, GeneratorMixin):
    ...
    def forward2logits(self, batch: Batch, src_hidden: torch.Tensor, src_mask: torch.Tensor=None, return_atten_weight: bool=False):
        ...
        if self.training and self.teacher_forcing_rate > 0.99:
            try:
                return self.forward2logits_all_at_once(batch, src_hidden, src_mask=src_mask, return_atten_weight=return_atten_weight)
            except NotImplementedError:
                pass
        # Fallback to step-by-step forward
        return self.forward2logits_step_by_step(batch, src_hidden, src_mask=src_mask, return_atten_weight=return_atten_weight)
...
class RNNGenerator(Generator):
    ...
    def _expand_states(self, batch_size: int, h_tm1: torch.Tensor):
        if isinstance(self.rnn, torch.nn.LSTM):
            h_tm1 = (h_tm1[0].expand(-1, batch_size, -1).contiguous(), 
    ...
    def _select_states(self, batch_indexing: torch.Tensor, h_tm1: torch.Tensor):
        if isinstance(self.rnn, torch.nn.LSTM):
            h_tm1 = (h_tm1[0][:, batch_indexing].contiguous(), 
