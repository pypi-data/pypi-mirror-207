...
def parse_date(month_day: int, week_day: int, month: int, year: int, c_format: Date_Format = "none", american: bool = False) -> Union[Date_Object, str, bool]:
    """
    ...
    date_dict: Date_Object = {
        "day": {
            ...
            "ordinal_month": "N/A" if month_day > 31 or month_day < 1 else ordinals[month_day] if month_day in ordinals else "th",
            "ordinal_week": "N/A" if week_day > 6 or week_day < 1 else ordinals[week_day + 1] if week_day + 1 in ordinals else "th",
        ...
        "month": {
            ...
            "ordinal": "N/A" if month > 12 or month < 1 else ordinals[month] if month in ordinals else "th",
			"short": "N/A" if month > 12 or month < 1 else month_dict[month][0:3]
		},
        "year": {
			"short": int(f"{year}"[2:4]),
            "long": year
		}
	}
