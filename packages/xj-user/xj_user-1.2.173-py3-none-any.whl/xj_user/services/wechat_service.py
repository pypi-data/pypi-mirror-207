...
class WechatService:
    ...
    def wechat_login(self, phone_code, login_code, sso_serve_id=None, platform_id=None, other_params=None):
        ...
        if not current_user:
            ...
            # 创建用户登录信息，绑定token
            auth_set, auth_err = LoginService.make_token(user_info.get('id', None), token)
            if auth_err:
            ...
            token = LoginService.make_token(user_info.get('id', None), phone, platform_id, platform_code)
            # 修改用户登录信息，绑定token
            auth_set, auth_err = LoginService.make_token(user_id=user_info.get('id', None), token=token,
                                                        is_create=False)
            if auth_err:
                return None, auth_err
    ...
    #
    #             # 生成登录token
    #             token = LoginService.set_token(user_info.get('id', ""), user_info.get("user_name", ""), platform_id,
    #                                             platform_code)
    #
    #             try:
    ...
    #                 return None, sso_err
    #             # 创建用户登录信息，绑定token
    #             auth_set, auth_err = LoginService.get_token(user_info.get('id', ""), token)
    #             if auth_err:
    #                 return None, auth_err
    #         # ----------------------------------------------------------------------
    #         else:
    ...
    #                                             platform_code)
    #             # 修改用户登录信息，绑定token
    #             auth_set, auth_err = LoginService.get_token(user_id=user_info.get('id', ""), token=token,
    #                                                          is_create=False)
    #             if auth_err:
    #                 return None, auth_err
    #
    ...
    def login_integration_interface(params):
        ...
        data, err = LoginService.logical_processing(current_user, phone, sso_serve_id, sso_app_id, openid, unionid,
                                                    platform_id,
    ...
    def login_empower(params):
        ...
        sso_set, sso_err = LoginService.sso_verify(sso_serve_id, None, app_id, True,
                                                   openid,
                                                   unionid)
        if sso_err:
            return None, sso_err
