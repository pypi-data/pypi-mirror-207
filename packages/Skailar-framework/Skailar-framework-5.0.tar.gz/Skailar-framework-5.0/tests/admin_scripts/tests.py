...
SKAILAR_SETTINGS_MODULE and default settings.py files.
"""
import os
import re
import shutil
...
from skailar import conf, get_version
from skailar.conf import settings
from skailar.core.management import (
    BaseCommand,
    CommandError,
    call_command,
    color,
...
from skailar.core.management.commands.loaddata import Command as LoaddataCommand
from skailar.core.management.commands.serve import Command as RunserverCommand
from skailar.core.management.commands.testserver import Command as TestserverCommand
from skailar.db import ConnectionHandler, connection
...
class AdminScriptTestCase(SimpleTestCase):
    ...
    def run_test(self, args, settings_file=None, apps=None, umask=-1):
        ...
        # The base dir for Skailar's tests is one level up.
        tests_dir = os.path.dirname(os.path.dirname(__file__))
        # The base dir for Skailar is one level above the test dir. We don't use
        # `import skailar` to figure that out, so we don't pick up a Skailar
        # from site-packages or similar.
        skailar_dir = os.path.dirname(tests_dir)
        ...
        if settings_file:
            test_environ["SKAILAR_SETTINGS_MODULE"] = settings_file
        elif "SKAILAR_SETTINGS_MODULE" in test_environ:
            del test_environ["SKAILAR_SETTINGS_MODULE"]
        python_path = [base_dir, skailar_dir, tests_dir]
    ...
    def run_skailar_admin(self, args, settings_file=None, umask=-1):
        return self.run_test(["-m", "skailar", *args], settings_file, umask=umask)
...
# of the skailar-admin.
##########################################################################


class SkailarAdminNoSettings(AdminScriptTestCase):
    "A series of tests for skailar-admin when there is no settings.py file."

    def test_builtin_command(self):
        """
        no settings: skailar-admin builtin commands fail with an error when no
        settings provided.
        """
    ...
    def test_builtin_with_bad_settings(self):
        """
        no settings: skailar-admin builtin commands fail if settings file (from
    ...
    def test_builtin_with_bad_environment(self):
        ...
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args, "bad_settings")
        self.assertNoOutput(out)
        self.assertOutput(err, "No module named '?bad_settings'?", regex=True)
    ...
    def test_commands_with_invalid_settings(self):
        ...
        """
        args = ["startproject"]
        out, err = self.run_skailar_admin(args, settings_file="bad_settings")
        self.assertNoOutput(out)
        self.assertOutput(err, "You must provide a project name", regex=True)


class SkailarAdminDefaultSettings(AdminScriptTestCase):
    """
    A series of tests for skailar-admin when using a settings.py file that
    ...
    def test_builtin_command(self):
        """
        default: skailar-admin builtin commands fail with an error when no
        settings provided.
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_settings(self):
        """
        default: skailar-admin builtin commands succeed if settings are provided
        as argument.
        """
    ...
    def test_builtin_with_bad_settings(self):
        """
        default: skailar-admin builtin commands fail if settings file (from
        argument) doesn't exist.
        """
        args = ["check", "--settings=bad_settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_custom_command(self):
        """
        default: skailar-admin can't execute user commands if it isn't provided
        settings.
        """
        args = ["noargs_command"]
        out, err = self.run_skailar_admin(args)
        self.assertNoOutput(out)
        self.assertOutput(err, "No Skailar settings specified")
        self.assertOutput(err, "Unknown command: 'noargs_command'")

    def test_custom_command_with_settings(self):
        """
        default: skailar-admin can execute user commands if settings are
...
class SkailarAdminFullPathDefaultSettings(AdminScriptTestCase):
    """
    A series of tests for skailar-admin when using a settings.py file that
    ...
    def test_builtin_command(self):
        """
        fulldefault: skailar-admin builtin commands fail with an error when no
        settings provided.
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_settings(self):
        """
        fulldefault: skailar-admin builtin commands succeed if a settings file
        is provided.
        """
        args = ["check", "--settings=test_project.settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_environment(self):
        """
        fulldefault: skailar-admin builtin commands succeed if the environment
        contains settings.
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args, "test_project.settings")
    ...
    def test_builtin_with_bad_settings(self):
        """
        fulldefault: skailar-admin builtin commands fail if settings file (from
        argument) doesn't exist.
        """
        args = ["check", "--settings=bad_settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_custom_command(self):
        """
        fulldefault: skailar-admin can't execute user commands unless settings
        are provided.
        """
        args = ["noargs_command"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_custom_command_with_settings(self):
        """
        fulldefault: skailar-admin can execute user commands if settings are
        provided as argument.
        """
        args = ["noargs_command", "--settings=test_project.settings"]
        out, err = self.run_skailar_admin(args)
...
class SkailarAdminMinimalSettings(AdminScriptTestCase):
    """
    A series of tests for skailar-admin when using a settings.py file that
    ...
    def setUp(self):
        super().setUp()
        self.write_settings(
            "settings.py", apps=["skailar.contrib.auth", "skailar.contrib.contenttypes"]
        )

    def test_builtin_command(self):
        """
        minimal: skailar-admin builtin commands fail with an error when no
        settings provided.
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_settings(self):
        """
        minimal: skailar-admin builtin commands fail if settings are provided as
        argument.
        """
        args = ["check", "--settings=test_project.settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_environment(self):
        """
        minimal: skailar-admin builtin commands fail if settings are provided in
        the environment.
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args, "test_project.settings")
    ...
    def test_builtin_with_bad_settings(self):
        """
        minimal: skailar-admin builtin commands fail if settings file (from
        argument) doesn't exist.
        """
        args = ["check", "--settings=bad_settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_custom_command(self):
        "minimal: skailar-admin can't execute user commands unless settings are provided"
        args = ["noargs_command"]
        out, err = self.run_skailar_admin(args)
        self.assertNoOutput(out)
        self.assertOutput(err, "No Skailar settings specified")
    ...
    def test_custom_command_with_settings(self):
        """
        minimal: skailar-admin can't execute user commands, even if settings are
        provided as argument.
        """
        args = ["noargs_command", "--settings=test_project.settings"]
        out, err = self.run_skailar_admin(args)
...
class SkailarAdminAlternateSettings(AdminScriptTestCase):
    """
    A series of tests for skailar-admin when using a settings file with a name
    ...
    def test_builtin_command(self):
        """
        alternate: skailar-admin builtin commands fail with an error when no
        settings provided.
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_settings(self):
        """
        alternate: skailar-admin builtin commands succeed if settings are
        provided as argument.
        """
    ...
    def test_builtin_with_environment(self):
        ...
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args, "test_project.alternate_settings")
        self.assertNoOutput(err)
        self.assertOutput(out, SYSTEM_CHECK_MSG)

    def test_builtin_with_bad_settings(self):
        """
        alternate: skailar-admin builtin commands fail if settings file (from
        argument) doesn't exist.
        """
        args = ["check", "--settings=bad_settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_custom_command(self):
        """
        alternate: skailar-admin can't execute user commands unless settings
        are provided.
        """
        args = ["noargs_command"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_custom_command_with_settings(self):
        """
        alternate: skailar-admin can execute user commands if settings are
        provided as argument.
        """
...
class SkailarAdminMultipleSettings(AdminScriptTestCase):
    """
    A series of tests for skailar-admin when multiple settings files
    ...
    def test_custom_command(self):
        """
        alternate: skailar-admin can't execute user commands unless settings are
        provided.
        """
        args = ["noargs_command"]
        out, err = self.run_skailar_admin(args)
...
class SkailarAdminSettingsDirectory(AdminScriptTestCase):
    """
    A series of tests for skailar-admin when the settings file is in a
    directory. (see #9751).
    """
    ...
    def test_builtin_command(self):
        """
        directory: skailar-admin builtin commands fail with an error when no
        settings provided.
        """
        args = ["check", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_bad_settings(self):
        """
        directory: skailar-admin builtin commands fail if settings file (from
        argument) doesn't exist.
        """
        args = ["check", "--settings=bad_settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_custom_command(self):
        """
        directory: skailar-admin can't execute user commands unless settings are
        provided.
        """
        args = ["noargs_command"]
        out, err = self.run_skailar_admin(args)
    ...
    def test_builtin_with_settings(self):
        """
        directory: skailar-admin builtin commands succeed if settings are
        provided as argument.
        """
        args = ["check", "--settings=test_project.settings", "admin_scripts"]
        out, err = self.run_skailar_admin(args)
...
class ManageManuallyConfiguredSettings(AdminScriptTestCase):
    ...
    def test_non_existent_command_output(self):
        ...
        self.assertNotInOutput(err, "No Skailar settings specified")
...
class ManageFullPathDefaultSettings(AdminScriptTestCase):
    ...
    def setUp(self):
        ...
        self.write_settings(
            "settings.py",
            ["skailar.contrib.auth", "skailar.contrib.contenttypes", "admin_scripts"],
        )
...
class ManageSettingsWithSettingsErrors(AdminScriptTestCase):
    ...
    def test_help(self):
        ...
        self.write_settings(
            "settings.py",
            extra="from skailar.core.exceptions import ImproperlyConfigured\n"
            "raise ImproperlyConfigured()",
        )
        args = ["help"]
        out, err = self.run_manage(args)
        self.assertOutput(out, "only Skailar core commands are listed")
        self.assertNoOutput(err)
...
class ManageCheck(AdminScriptTestCase):
    ...
    def test_warning_does_not_halt(self):
        """
        When there are only warnings or less serious messages, then Skailar
...
class ManageRunserver(SimpleTestCase):
    ...
    def test_no_database(self):
        ...
        with mock.patch(
            "skailar.core.management.base.connections", new=tested_connections
        ):
    ...
    @mock.patch("skailar.core.management.commands.runserver.run")
    @mock.patch("skailar.core.management.base.BaseCommand.check_migrations")
    @mock.patch("skailar.core.management.base.BaseCommand.check")
...
class ManageTestserver(SimpleTestCase):
    ...
    @mock.patch("skailar.db.connection.creation.create_test_db", return_value="test_db")
...
class CommandTypes(AdminScriptTestCase):
    ...
    def test_help_commands(self):
        ...
        self.assertNotInOutput(out, "[skailar]")
        self.assertOutput(out, "startapp")
        self.assertOutput(out, "startproject")
    ...
    def test_specific_help(self):
        ...
        self.assertOutput(
            out, "Checks the entire Skailar project for potential problems."
        )
    ...
    def test_run_from_argv_closes_connections(self):
        ...
        with mock.patch("skailar.core.management.base.connections") as mock_connections:
            command.run_from_argv(["", ""])
    ...
    def test_app_command(self):
        ...
        out, err = self.run_manage(args)
        self.assertNoOutput(err)
        self.assertOutput(out, "EXECUTE:AppCommand name=skailar.contrib.auth, options=")
        self.assertOutput(
            out,
            ", options=[('force_color', False), ('no_color', False), "
            "('pythonpath', None), ('settings', None), ('traceback', False), "
    ...
    def test_app_command_multiple_apps(self):
        ...
        self.assertOutput(
            out, "EXECUTE:AppCommand name=skailar.contrib.contenttypes, options="
        )
        self.assertOutput(
            out,
            ", options=[('force_color', False), ('no_color', False), "
...
class ArgumentOrder(AdminScriptTestCase):
    ...
    skailar-admin command arguments are parsed in 2 parts; the core arguments
    ...
    def test_program_name_from_argv(self):
        ...
        with captured_stdout() as out, captured_stderr() as err:
            with mock.patch("sys.argv", [None] + args):
                execute_from_command_line(["skailar-admin"] + args)
        self.assertIn("usage: skailar-admin shell", out.getvalue())
        self.assertEqual(err.getvalue(), "")
...
class StartProject(LiveServerTestCase, AdminScriptTestCase):
    ...
    def test_wrong_args(self):
        ...
        """
        out, err = self.run_skailar_admin(["startproject"])
        self.assertNoOutput(out)
    ...
    def test_custom_project_template_non_python_files_not_formatted(self):
        ...
        _, err = self.run_skailar_admin(args)
        self.assertNoOutput(err)
        with open(
    ...
    def test_custom_project_template_from_tarball_by_url_skailar_user_agent(self):
        user_agent = None
        ...
        try:
            ...
            self.assertNoOutput(err)
            self.assertIn("Skailar/%s" % get_version(), user_agent)
        finally:
    ...
    def test_honor_umask(self):
        _, err = self.run_skailar_admin(["startproject", "testproject"], umask=0o077)
        self.assertNoOutput(err)
        testproject_dir = os.path.join(self.test_dir, "testproject")
...
class StartApp(AdminScriptTestCase):
    ...
    def test_invalid_target_name(self):
        for bad_target in (
            ...
            with self.subTest(bad_target):
                _, err = self.run_skailar_admin(["startapp", "app", bad_target])
                self.assertOutput(
                    err,
    ...
    def test_importable_target_name(self):
        _, err = self.run_skailar_admin(["startapp", "app", "os"])
        self.assertOutput(
            err,
    ...
    def test_overlaying_app(self):
        ...
        self.run_skailar_admin(["startapp", "app1", "apps/app1"])
        out, err = self.run_skailar_admin(["startapp", "app2", "apps/app1"])
        self.assertOutput(
            err,
    ...
    def test_template(self):
        out, err = self.run_skailar_admin(["startapp", "new_app"])
        self.assertNoOutput(err)
        ...
        with open(os.path.join(app_path, "apps.py")) as f:
            ...
            if HAS_BLACK:
                test_str = 'default_auto_field = "skailar.db.models.BigAutoField"'
            else:
                test_str = "default_auto_field = 'skailar.db.models.BigAutoField'"
...
class DiffSettings(AdminScriptTestCase):
    ...
    def test_basic(self):
        ...
        # Attributes from skailar.conf.Settings don't appear.
    ...
    def test_settings_configured(self):
        ...
        # Attributes from skailar.conf.UserSettingsHolder don't appear.
    ...
    def test_unified(self):
        ...
        self.assertOutput(out, "+ SECRET_KEY = 'skailar_tests_secret_key'")
...
class MainModule(AdminScriptTestCase):
    """python -m skailar works like skailar-admin."""

    def test_program_name_in_help(self):
        out, err = self.run_test(["-m", "skailar", "help"])
        self.assertOutput(
            out,
            "Type 'python -m skailar help <subcommand>' for help on a specific "
            "subcommand.",
        )


class SkailarAdminSuggestions(AdminScriptTestCase):
    def setUp(self):
        super().setUp()
        self.write_settings("settings.py")
