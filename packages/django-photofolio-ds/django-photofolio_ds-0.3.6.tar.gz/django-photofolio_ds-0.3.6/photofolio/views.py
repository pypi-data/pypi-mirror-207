...
def contact(request):
    ...
    if request.method == 'GET':
        ...
        print('post')
        c.update(make_post_context(request.POST, c['basic_info']['consult_email'], 'contact'))
        return render(request, 'photofolio/sub/contact.html', c)
...
def make_post_context(request_post, consult_mail, anchor='appointment'):
    ...
    if form.is_valid():
        ...
        if is_sendmail:
            context['post_message'] = '담당자에게 상담 내용이 전달 되었습니다. 확인 후 바로 연락 드리겠습니다. 감사합니다.'
        else:
            context['post_message'] = '메일 전송에서 오류가 발생 하였습니다. 카카오톡이나 전화로 문의주시면 감사하겠습니다. 죄송합니다.'
        return context
    else:
