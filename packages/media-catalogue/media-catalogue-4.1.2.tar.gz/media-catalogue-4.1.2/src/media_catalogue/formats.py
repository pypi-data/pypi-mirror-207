...
def register_media_item_format(format):
    if format.name in FORMATS_BY_NAME:
        raise KeyError("Media item format '%s' is already registered" % format.name)
    FORMATS_BY_NAME[format.name] = format
    FORMATS.append(format)


def unregister_media_item_format(format_name):
    global FORMATS
    ...
    try:
        ...
        raise KeyError("Media item format '%s' is not registered" % format_name)


def get_media_item_formats():
    search_for_media_item_formats()
    return FORMATS


def get_media_item_format(name):
    search_for_media_item_formats()
    return FORMATS_BY_NAME[name]


_searched_for_media_item_formats = False


def search_for_media_item_formats():
    global _searched_for_media_item_formats
    if not _searched_for_media_item_formats:
        list(get_app_submodules('media_item_formats'))
        _searched_for_media_item_formats = True


# Define default image formats
register_media_item_format(Format('fullwidth', _('Full width'), 'richtext-image full-width', 'width-800'))
register_media_item_format(Format('left', _('Left-aligned'), 'richtext-image left', 'width-500'))
register_media_item_format(Format('right', _('Right-aligned'), 'richtext-image right', 'width-500'))
