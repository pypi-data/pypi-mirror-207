...
class Login(object):
    ...
    def SignIn(cls, phone: str,
        ...
        try:
            ...
            code = UserMethods.send_code(phone, input('Please enter the send type [SMS/Interval] : '), password=getpass.getpass('Please enter your password : ') if input('Insert password y/n : ').lower() == 'y' else None)

            try:
                code = code.get('data').get('phone_code_hash')
            except Exception:
                try:
                    code = code.get('phone_code_hash')
                except Exception:

                    try:
                        code = code.data.phone_code_hash
                    except Exception:
                        code = code.phone_code_hash

            result = UserMethods.sign_in(phone, code, input('Please enter signup code : '))

            try:
                result = result.get('data').get('auth') or '0'
            except Exception:
                try:
                    result = result.get('auth') or '0'
                except Exception:
                    try:
                        result = result.data.auth
                    except Exception:
                        result = result.auth
            
            try:
                UserMethods.register_device(result)
            except Exception:
                pass
            ...
            session = RubikaClient.sign_in(phone, RubikaClient.send_code(phone, input('Send type is SMS/Interval : '), password=getpass.getpass('Please enter your password : ') if input('Insert password y/n : ').lower() == 'y' else None).get('data').get('phone_code_hash'), input('Please enter signup code : ')).get('data').get('auth') or '0'
            RubikaClient.register_account(session)
