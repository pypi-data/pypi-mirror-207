...
from skailar.template.response import (
...
from skailar.test.utils import require_jinja2
...
class SimpleTemplateResponseTest(SimpleTestCase):
    ...
    def test_render(self):
        ...
        template = engines["skailar"].from_string("bar{{ baz }}")
    ...
    def test_using(self):
        ...
        response = SimpleTemplateResponse(
            "template_tests/using.html", using="skailar"
        ).render()
        self.assertEqual(response.content, b"DTL\n")
        response = SimpleTemplateResponse(
...
class TemplateResponseTest(SimpleTestCase):
    ...
    def test_using(self):
        ...
        self.assertEqual(response.content, b"DTL\n")
        response = TemplateResponse(
            request, "template_tests/using.html", using="skailar"
        ).render()
        self.assertEqual(response.content, b"DTL\n")
        response = TemplateResponse(
...
@modify_settings(
    MIDDLEWARE={
        "append": [
            "skailar.middleware.cache.FetchFromCacheMiddleware",
            "skailar.middleware.cache.UpdateCacheMiddleware",
        ],
    },
)
@override_settings(
