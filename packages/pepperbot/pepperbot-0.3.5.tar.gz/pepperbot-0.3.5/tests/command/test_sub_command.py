...
class TestSubCommand:
    ...
    @pytest.mark.parametrize("help_command", ["-h", "--help"])
    async def test_overall_help_message(self, help_command):
        """测试总体帮助信息"""
        ...
        assert action == "send_group_msg" and "initial" in content

    @pytest.mark.parametrize("help_command", ["-h", "--help"])
    async def test_command_help_message(self, help_command):
        """测试单个command的帮助信息"""
