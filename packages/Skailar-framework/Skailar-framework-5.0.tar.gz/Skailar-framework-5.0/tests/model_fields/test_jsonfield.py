...
from skailar.core.exceptions import ValidationError
from skailar.core.serializers.json import SkailarJSONEncoder
...
from skailar.db.models.fields.json import (
    KT,
    KeyTextTransform,
    KeyTransform,
    KeyTransformFactory,
...
from skailar.db.models.functions import Cast
from skailar.test import SimpleTestCase, TestCase, skipIfDBFeature, skipUnlessDBFeature
...
class JSONFieldTests(TestCase):
    ...
    def test_db_check_constraints(self):
        ...
        with mock.patch.object(SkailarJSONEncoder, "encode", return_value=value):
...
class TestMethods(SimpleTestCase):
    def test_deconstruct(self):
        ...
        self.assertEqual(path, "skailar.db.models.JSONField")
        self.assertEqual(args, [])
        self.assertEqual(kwargs, {})

    def test_deconstruct_custom_encoder_decoder(self):
        field = models.JSONField(encoder=SkailarJSONEncoder, decoder=CustomJSONDecoder)
        name, path, args, kwargs = field.deconstruct()
        self.assertEqual(kwargs["encoder"], SkailarJSONEncoder)
...
class TestValidation(SimpleTestCase):
    def test_invalid_encoder(self):
        ...
        with self.assertRaisesMessage(ValueError, msg):
            models.JSONField(encoder=SkailarJSONEncoder())
    ...
    def test_custom_encoder(self):
        field = models.JSONField(encoder=SkailarJSONEncoder)
        value = uuid.UUID("{d85e2076-b67c-4ee7-8c3a-2bf5a2cc2475}")
        field.clean({"uuid": value}, None)
...
class TestFormField(SimpleTestCase):
    ...
    def test_formfield_custom_encoder_decoder(self):
        model_field = models.JSONField(
            encoder=SkailarJSONEncoder, decoder=CustomJSONDecoder
        )
        form_field = model_field.formfield()
        self.assertIs(form_field.encoder, SkailarJSONEncoder)
...
class TestSerialization(SimpleTestCase):
    ...
    def test_xml_serialization(self):
        test_xml_data = (
            '<skailar-objects version="1.0">'
            '<object model="model_fields.nullablejsonmodel">'
            '<field name="value" type="JSONField">%s'
            "</field></object></skailar-objects>"
        )
        for value, serialized in self.test_values:
            with self.subTest(value=value):
...
class TestSaveLoad(TestCase):
    ...
    def test_ambiguous_str_value_deprecation(self):
        ...
        with self.assertWarnsMessage(RemovedInSkailar51Warning, msg):
