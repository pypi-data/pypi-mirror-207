...
from taichu_serve.third.tracer import add_event
...
def model_stream_inference(model_name, model_version, data, request_id, method=None):
    try:
        ...
        add_event(span=span, name="[model_stream_inference] pipe acquire start")
        p = workers_pipe.acquire(request_id)
        if p is None:
            raise TooManyRequestsError()
        add_event(span=span, name="[model_stream_inference] pipe acquire end")
        ...
        add_event(span=span, name="[model_stream_inference] pipe send")
        p.send(payload)
        while True:
            ret = p.recv()
            add_event(span=span, name="[model_stream_inference] pipe recv")
            if ret.get('status') == 'error':
                add_event(span=span, name="[model_stream_inference] pipe error")
                raise ModelPredictError(message=ret.get('error'))

            if ret.get('status') == 'end':
                add_event(span=span, name="[model_stream_inference] pipe end")
                return
...
def model_inference(model_name, model_version, data, request_id, method=None):
    try:
        span = trace.get_current_span()
        add_event(span=span, name="[model_inference] pipe acquire start")
        p = workers_pipe.acquire(request_id)
        if p is None:
            LOGGER.info("no available worker")
            raise TooManyRequestsError()
        add_event(span=span, name="[model_inference] pipe acquire end")
        ...
        add_event(span=span, name="[model_inference] pipe send")
        p.send(payload)
        ret = p.recv()
        add_event(span=span, name="[model_inference] pipe recv")
