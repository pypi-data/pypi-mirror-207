...
def get_call_logger():
    ...
    f = getattr(Funcs['root'], 'log_call', 0)
    if f:
        return f
    if app.log._logger.level > 10:
        return False
    return lambda op, _: app.debug(op['name'])
...
class Op:
    ...
    def parse_args(op, func):
        ...
        def call(
            ...
            # do we even get here (see filter_exceptions)?
            if is_exc(msg):
                if c_err:
                    c_err.inc()  # per func
                    err_total().inc()  # total
            ...
            fl = _log_call[0]
            if fl is None:
                fl = _log_call[0] = get_call_logger()
            if fl:
                fl(op, msg) if fl else None
