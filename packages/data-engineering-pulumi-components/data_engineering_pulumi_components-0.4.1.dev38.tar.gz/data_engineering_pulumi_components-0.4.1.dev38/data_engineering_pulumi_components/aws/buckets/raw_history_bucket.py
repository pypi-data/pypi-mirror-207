...
class RawHistoryBucket(Bucket):
    def __init__(
        self, name: str, tagger: Tagger, lifecycle_rules_expiration_days: dict = {}, opts: Optional[ResourceOptions] = None
    ) -> None:
        super().__init__(
            name=name + "-raw-history",
            tagger=tagger,
            lifecycle_rules=self.lifecycle_rules_expiration_days_provided(lifecycle_rules_expiration_days, name),
