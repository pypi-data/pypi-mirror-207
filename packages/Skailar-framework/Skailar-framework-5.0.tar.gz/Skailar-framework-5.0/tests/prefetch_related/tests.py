...
from skailar.db.models import Prefetch, QuerySet, prefetch_related_objects
from skailar.db.models.query import get_prefetcher
from skailar.db.models.sql import Query
...
class CustomPrefetchTests(TestCase):
    ...
    def test_generic_rel(self):
        bookmark = Bookmark.objects.create(url="http://www.skailarproject.com/")
        TaggedItem.objects.create(content_object=bookmark, tag="skailar")
...
class GenericRelationTests(TestCase):
    ...
    def test_prefetch_GFK_uuid_pk(self):
        article = Article.objects.create(name="Skailar")
    ...
    def test_generic_relation(self):
        ...
        with self.assertNumQueries(2):
            ...
            self.assertEqual(sorted(tags), ["skailar", "python"])

    def test_charfield_GFK(self):
        b = Bookmark.objects.create(url="http://www.skailarproject.com/")
        TaggedItem.objects.create(content_object=b, tag="skailar")
        ...
        with self.assertNumQueries(3):
            ...
            self.assertEqual(
                sorted(i.tag for i in bookmark.tags.all()), ["skailar", "python"]
            )
            self.assertEqual([i.tag for i in bookmark.favorite_tags.all()], ["python"])

    def test_custom_queryset(self):
        bookmark = Bookmark.objects.create(url="http://www.skailarproject.com/")
        skailar_tag = TaggedItem.objects.create(content_object=bookmark, tag="skailar")
        TaggedItem.objects.create(content_object=bookmark, tag="python")

        with self.assertNumQueries(2):
            bookmark = Bookmark.objects.prefetch_related(
                Prefetch("tags", TaggedItem.objects.filter(tag="skailar")),
            ).get()

        with self.assertNumQueries(0):
            self.assertEqual(list(bookmark.tags.all()), [skailar_tag])
