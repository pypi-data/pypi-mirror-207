...
def change_style(app, theme):
    p = os.path.abspath(os.path.join(os.path.dirname(__file__), '../resource/theme/main.qss'))
    with open(p, 'r') as f:
        temp = f.read()
...
def repalce_color(style_text: str, name: str) -> str:
    p = os.path.abspath(os.path.join(os.path.dirname(__file__), f'../resource/theme/{name}.xml'))
    try:
        tree = ET.parse(p)
        root = tree.getroot()
        colors = {}
