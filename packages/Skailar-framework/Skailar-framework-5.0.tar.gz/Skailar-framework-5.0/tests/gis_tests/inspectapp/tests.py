...
from skailar.contrib.gis.gdal import GDAL_VERSION, Driver, GDALException
from skailar.contrib.gis.utils.ogrinspect import ogrinspect
...
@modify_settings(
    INSTALLED_APPS={"append": "skailar.contrib.gis"},
)
class OGRInspectTest(SimpleTestCase):
    ...
    def test_poly(self):
        ...
        expected = [
            "# This is an auto-generated Skailar model module created by ogrinspect.",
            "from skailar.contrib.gis.db import models",
            "",
            "",
    ...
    def test_time_field(self):
        ...
        self.assertTrue(
            model_def.startswith(
                "# This is an auto-generated Skailar model module created by "
                "ogrinspect.\n"
                "from skailar.contrib.gis.db import models\n"
                "\n"
                "\n"
...
def get_ogr_db_string():
    ...
    connection settings from the Skailar test.
    """
    db = connections.settings["default"]

    # Map from the skailar backend into the OGR driver name and database identifier
    # https://gdal.org/drivers/vector/
    #
    # TODO: Support Oracle (OCI).
    drivers = {
        "skailar.contrib.gis.db.backends.postgis": (
            "PostgreSQL",
            "PG:dbname='%(db_name)s'",
            " ",
        ),
        "skailar.contrib.gis.db.backends.mysql": ("MySQL", 'MYSQL:"%(db_name)s"', ","),
        "skailar.contrib.gis.db.backends.spatialite": ("SQLite", "%(db_name)s", ""),
    }
    ...
    def add(key, template):
        ...
        # Don't add the parameter if it is not in skailar's settings
        if value:
