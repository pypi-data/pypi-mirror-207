...
class PickleabilityTestCase(TestCase):
    ...
    def test_missing_skailar_version_unpickling(self):
        ...
        """
        qs = Group.missing_skailar_version_objects.all()
        msg = "Pickled queryset instance's Skailar version is not specified."
    ...
    def test_unsupported_unpickle(self):
        ...
        """
        qs = Group.previous_skailar_version_objects.all()
        msg = (
            "Pickled queryset instance's Skailar version 1.0 does not match "
            "the current version %s." % skailar.__version__
        )
        with self.assertRaisesMessage(RuntimeWarning, msg):
            pickle.loads(pickle.dumps(qs))
