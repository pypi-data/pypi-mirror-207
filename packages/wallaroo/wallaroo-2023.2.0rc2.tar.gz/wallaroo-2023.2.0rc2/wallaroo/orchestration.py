...
from wallaroo.task import Task
from . import queries
from .object import (
    DehydratedValue,
    Object,
...
class Orchestration(Object):
    ...
    def list_orchestrations(client: Client) -> List[Orchestration]:
        from wallaroo.wallaroo_ml_ops_api_client.api.orchestration.list_ import (
            sync,
        )
    ...
    def upload(
        ...
        from wallaroo.wallaroo_ml_ops_api_client.api.orchestration.upload import (
            sync,
        )
        from wallaroo.wallaroo_ml_ops_api_client.models.upload_payload_doc import (
            UploadPayloadDoc,
        )
        from wallaroo.wallaroo_ml_ops_api_client.models.upload_request import (
            UploadRequest,
        )
        ...
        metadata = UploadRequest.from_dict(
            {"workspace_id": client.get_current_workspace().id()}
        )
        u = UploadPayloadDoc(file, metadata)
    ...
    def run_scheduled(
        ...
        :param: timeout Optional[int] A timeout in milliseconds.
        :param: json_args Dict[Any, Any] A JSON object containing deploy-specific arguments.
        :return: A metadata object associated with the deploy Task
        """
    ...
    def _repr_html_(self) -> str:
        ...
        <table>
          ...
          <tr>
            <td>File Name</td><td>{self.file_name()}</td>
          </tr>
          <tr>
            <td>SHA</td><td>{sha_first_6}...{sha_last_6}</td>
          </tr>
          <tr>
            <td>Status</td><td>{self.status()}</td>
          </tr>
          <tr>
...
class OrchestrationList(List[Orchestration]):
    ...
    def _repr_html_(self) -> str:
        def row(orchestration):
            fmt = orchestration.client._time_format

            return (
                "<tr>"
                + f"<td>{orchestration.id()}</td>"
                + f"<td>{orchestration.status()}</td>"
                + f"<td>{orchestration.file_name()}</td>"
                + f"<td>{orchestration.sha()}</td>"
                + f"<td>{orchestration.created_at().strftime(fmt)}</td>"
                + f"<td>{orchestration.updated_at().strftime(fmt)}</td>"
                + "</tr>"
            )
