...
class Coco(BaseLabelme):
    ...
    def self2coco(self):
        ...
        for dataset in tqdm(self.datasets):
            ...
            if dataset.get('background'):  # 为真表示为处理背景图片
    ...
    def image_handle(self, dataset):
        ...
        if dataset.get('http_url'):  # 如果http_url有值，表示转换为带http路径的coco图像路径
            ...
            self.result_addImage_id = self.add_image(file_name, dataset.get('image_width'), dataset.get('image_height'),
                                                     None)
    ...
    def polygon_shape(self, shape, dataset, one_img_ann_list):
        ...
        if len(shape['points']) == 4:
            ...
            if self.rectangle_cross_the_border(bbox, dataset):  # 如果函数返回为True，表示标注形状已经超出图像边界
