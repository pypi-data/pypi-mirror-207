        ...
        if python_value is None:
            ...
            if isinstance(python_value, list):
                if len(python_value) != len(data_type.fields):
                    raise TypeError(
                        f"Inconsistent number of fields for {key}(a Struct type): expected {len(data_type.fields)} fields, got {len(python_value)} fields"
                    )
                api_value.list_value.SetInParent()  # Needed in the case of empty arrays.
                ...
                raise TypeError(f"Invalid type for {key}: expected dict or list, got {type(python_value).__name__}")
        else:
            # should never happen
