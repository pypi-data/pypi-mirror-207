...
from ia.gaius.pvt.pvt_utils import init_emotive_on_node, plot_confusion_matrix, \
    ...
    f1_score, false_discovery_rate, false_positive_rate, update_accuracy, update_precision
...
class PerformanceValidationTest():
        ...
        for test_num in range(0, self.num_of_tests):
            ...
            try:
                if self.dataset_location != 'mongodb':
                    ...
                    plot_confusion_matrix(test_num=test_num, class_metrics_data_structures=self.class_metrics_data_structures, QUIET=self.QUIET)
            except Exception as e:
                if not self.QUIET: # pragma: no cover
    ...
    def get_classification_metrics(self):
        ...
        for node in self.class_metrics_data_structures:
            ...
            self.class_metrics_data_structures[node]['metrics']['precision']                = node_test_metrics['running_precisions'][node]
            self.class_metrics_data_structures[node]['metrics']['accuracy']                 = node_test_metrics['running_accuracy'][node]
            ...
            if node == 'hive':
                continue
            self.class_metrics_data_structures[node]['labels'] = list(set(list(self.labels_set[node]) + ['UNKNOWN']))
    ...
    def compute_incidental_probabilities(self, test_step_info: dict):
        ...
        if self.test_type == 'classification':
            ...
            for k in test_step_info['predicted'].keys():
                ...
                test_step_info['running_precisions'][k] = update_precision(tp=test_step_info['true_positive'][k],
                                                                            tn=0,
                ...
                #                                                                tn=0)
                test_step_info['running_accuracy'][k] = update_accuracy(tp=test_step_info['true_positive'][k],
                                                                        tn=0,
                                                                        overall_count=idx+1)
            ...
            for k in test_step_info['predicted'].keys():
                for emotive in test_step_info['actual'].keys():
                    ...
                    if emotive in test_step_info['predicted'][k].keys():
                        ...
                        test_step_info['running_precisions'][k][emotive] = update_precision(tp=test_step_info['true_positive'][k][emotive],
                                                                                            tn=test_step_info['true_negative'][k][emotive],
                                                                                            response_count=test_step_info['response_counts'][k][emotive])
                        # try:
                        #     test_step_info['running_precisions'][k][emotive] = ((test_step_info['true_positive'][k][emotive] + test_step_info['true_negative'][k][emotive]) / test_step_info['response_counts'][k][emotive]) * 100
                        # except ZeroDivisionError:
                        #     test_step_info['running_precisions'][k][emotive] = 0.0

                        # test_step_info['running_accuracy'][k][emotive] = ((test_step_info['true_positive'][k][emotive] + test_step_info['true_negative'][k][emotive]) / (test_step_info['testing_emotive_counter'][emotive])) * 100
                        test_step_info['running_accuracy'][k][emotive] = update_accuracy(tp=test_step_info['true_positive'][k][emotive],
                                                                                         tn=test_step_info['true_negative'][k][emotive],
