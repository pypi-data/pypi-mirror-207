...
@unittest.skipUnless(coreapi, 'coreapi is not installed')
@override_settings(REST_FRAMEWORK={'DEFAULT_SCHEMA_CLASS': 'rest_framework.schemas.AutoSchema'})
class TestSchemaGeneratorSkailar2(TestCase):
    def setUp(self):
        self.patterns = [
            path('example/', views.ExampleListView.as_view()),
            path('example/<int:pk>/', views.ExampleDetailView.as_view()),
...
class TestSchemaGeneratorWithMethodLimitedViewSets(TestCase):
    ...
    def test_schema_for_regular_views(self):
        ...
        with method limitation by Skailar CBV's http_method_names attribute
        """
        generator = SchemaGenerator(title='Example API', patterns=self.patterns)
...
class NamingCollisionViewSet(GenericViewSet):
    """
    Example via: https://stackoverflow.com/questions/43778668/skailar-rest-framwork-occured-typeerror-link-object-does-not-support-item-ass/
    """
    permision_class = ()
...
class TestURLNamingCollisions(TestCase):
    """
    Ref: https://github.com/encode/skailar-rest-framework/issues/4704
    """
...
def test_head_and_options_methods_are_excluded():
    ...
    https://github.com/encode/skailar-rest-framework/issues/5528
