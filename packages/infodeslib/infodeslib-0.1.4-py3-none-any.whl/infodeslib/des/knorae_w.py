...
class KNORAE_W: 
    """
    Late Fusion version of k-Nearest Oracles Eliminate Weight (KNORAE_W). 
    k : int (Default = 7)
    ...
    def estimate_competence(self):  
        ...
        for i in range(num_modalities): 
            ...
            for j in range(len(self.roc_labels)): 
                distance_weight_sum += self.distance_weighting(self.distances[j])
                
                if preds[j] == self.roc_labels.iloc[j]: 
                    average_correct_probs += probs[j]
                    corrects = corrects + 1 
                    model_weight += self.distance_weighting(self.distances[j]) 
