...
var projects = require(NRH + "/../@node-red/runtime/lib/api/projects");
var flows = require(NRH + "/../@node-red/runtime/lib/api/flows");
...
function rebuild_all_flows(AH, RED, g) {
	let n;
	AH.RED = RED;
	delete AH.object_cache;
	AH.supported_ax_exit_nodes = {};
	AH.entry_nodes_checked_for_ax_wires_in = {};
	AH.ax_ops_no_send = {};
	flows.getFlows({ req: null }).then((v) => {
		let a;
		AH.all_flows = a = v;
		AH.by_id = {};
		AH.all_ax_ops = {};
		for (var i = 0; i < a.flows.length; i++) {
			n = a.flows[i];
			AH.by_id[n.id] = n;
			if (is_ax(n)) {
...
function dyn_snk_ws_out(hub, RED, g, snk) {
	/* when a dyn snk sees a subs change we must potentially inform all clients */
	let srv = RED.nodes.getNode(snk.server);

	// function on_ws_conn(g, snk, srv) {
	//     function cb(ws) {
	//         return
	//         function hub_msg(mode) {
	//             g.AXHub.$hub_msgs.next({
	//                 msg: {
	//                     type: 'subs_change',
	//                     ws: ws,
	//                     srv: srv,
	...
	//         }
	//         hub_msg('subscribed')
	//         ws.on('close', () => hub_msg('unsubscribed'))
	//     }
	//     return cb
	// }
	...
	//RED.nodes
	//    .getNode(node.server)
	//srv.server.on('connection', on_ws_conn(g, snk, srv))
...
var on_sub_change_by_dyn_snk_type = { "websocket out": dyn_snk_ws_out };
...
function find_dyn_snks(AH, RED, g) {
	...
	for (const [id, snk] of Object.entries(AH.by_id)) {
		if (snk.wires && snk.wires.length == 0) {
			let on_subs_change = on_sub_change_by_dyn_snk_type[snk.type];
			if (on_subs_change) {
...
function ax_make_hub(node, g, RED) {
	...
	function to_binary(msg) {
		...
		var buf = Buffer.from(hl + head, "utf-8");
	...
	register_object_removal = (node) => {
		if (node.type == "http response") {
			node = AH.RED.nodes.getNode(node.id);
			// node._InputCallbacks is a list with cbs:
			node.on("input", pop_msg_from_cache);
		}
	};
	...
	function pop_msg_from_cache(msg) {
		let cmsg = AH.object_cache[msg._msgid];
	...
	function best_client_by_input_node_id(node) {
		...
		if (take) return { client: c, subject_id: subj_id };
	...
	function msg_to_client(fn, msg, node) {
		/*
		 * The first ax node of a pipeline got a message from a node red operator
		...
		// if (!AH.foo) { AH.foo = 1; return; }
		let bl = AH.ax_ops_no_send[node.id];
		if (bl && Date.now() - bl < 5000) return;
		console.log("Input"); //debug
		let chain = fn.split(/#/, 100); // dont decode for perf here
		fn = chain[0];
		let c = best_client_by_input_node_id(node);
		if (!c) return;
		// is this created here, or was it from python and should go there again?:
		let pymsg;
		if (!msg._ids) {
			let ids = { msg: msg._msgid };
			pymsg = {
				type: "msg",
				func: chain,
				_ids: ids,
			...
			pymsg = msg;
			msg.sent = msg.sent ? msg.sent + 1 : 0;
		}
		...
		// TODO: formalize object detection, i.e. the need to cache:
		if (msg.req) {
	...
	AH.$jobres = AH.subj_jobres
		.pipe(
			rx.map((client_msg) => {
				 ...
				 */
				let msg, node, id, idi, client;
				msg = client_msg.msg;
				client = client_msg.client;
				...
				if (node.wires.length > 1) {
					let msgs = [msg];
					for (var j = 1; j < node.wires.length; j++) msgs.push(msg);
					node.send(msgs);
				} else node.send(msg);
			}),
		)
	...
	function set_editor_status(node, AH) {
		let count = Object.keys(AH.clients).length;
		if (!count) {
			node.status({
				fill: "red", //shape: 'dot',
				text: "No Connections ",
	...
	function cut_wires_to_py_entry_nodes(cut_wires) {
		let frm, to, nfrom, ws, did, nw;
		for (var p = 0; p < cut_wires.length; p++) {
			frm = cut_wires[p][0];
			...
			for (var k = 0; k < nfrom.wires.length; k++) {
				ws = nfrom.wires[k];
				wg = [];
				for (var j = 0; j < ws.length; j++) {
					did = ws[j];
	...
	function get_node_instance_id(ax_id) {
		 ...
		 */
		let root_node, nodemap, dict, k1, v1, op;
		let parts = ax_id.split("/");
		if (parts.length == 1) {
			return parts[0];
		}
		root_node = AH.RED.nodes.getNode(parts[0]);
	...
	process_msg = (msg_type) => {
		return {
			subs_change: (r) => {
				...
				function send_subs_change(msg) {
					...
					var c,
						job = {
							type: "subs_change",
							payload: { snk: msg.node, mode: msg.mode },
							ts: Date.now(),
						};
					for (let sck in AH.clients) {
						c = AH.clients[sck];
						AH.send(c.ws, job);
					}
				}
				...
				let msg = r.msg;
				let count = msg.connected;
				var snk = AH.dyn_snks[msg.node];
			...
			register: (r) => {
				...
				send(r.ws, { type: "status", payload: "registered", ts: Date.now() });
			...
			flow_def: (r) => {
				/* Client sent a new flow definition to us.
				 *
				 * The client compared ours with his and decided it's structurally different
				 * So we write and exit.
				 *
				 * Any other client trying to send the same will fail, websocket is down
				 * When he reconnects he'll get the updated one and will NOT send it again
				 * (incl the original client)
				 */
				const self = this;
				const client_msg = r;
				const hub = AH;

				let fn = AH.RED.settings.userDir + "/client_flows.json";
				let p = projects;
				projects.getActiveProject({ user: "root" }).then((v) => {
					...
					let f = JSON.stringify(client_msg.msg.payload.flows);
					let fn = v.path + "/flows.json";
					let red = hub.RED;
					fs.writeFile(fn, f, (err) => {
						...
						console.warn("Flows from client saved - restarting", fn, v.package);
			...
			msg: (r) => {
				...
				let open = open_jobs_cur.funcs[r.msg._ids.nr_inp_node];
				if (open) open[r.ws.sck] = (open[r.ws.sck] || 1) - 1;
				// for runtime stats:
				r.msg.dt = Date.now() - r.msg.ts;
				AH.subj_jobres.next({ msg: r.msg, client: r.ws.sck });
			},
			closed: (r) => {
	...
	var ax_ws_port = this.server.address().port + 1;
	console.log("Opening ax-hub server, port", ax_ws_port);
	const wss = (AH.wss = new ws.Server({
		path: "/ws/ax-hub",
		port: ax_ws_port,
	}));
	...
	wss.on("connection", function connection(ws, req) {
		...
		$msgs = Rx.Observable.create((obs) => {
			...
			ws.on("close", () => {
				...
				obs.next({ msg: { type: "closed" }, ws: ws });
				obs.complete();
			});
	...
	var open_jobs_cur = { funcs: {} }; // current open
	var stats_rate = (AH.stats_rate = { funcs: {} });
	var stats_total = (AH.stats_total = { by_cln: {} });
	function stats_reset() {
		for (var key in stats_rate) {
...
module.exports = function(RED) {
	function AXHub(config) {
		...
		node.on("input", function(msg) {
			...
			let url = msg.req.url;
			let fn = url.split("/api/")[1];
			// api call:
			if (fn) {
				fn = fn.split("-%3E", 100).join("#"); // when
				return hub.msg_to_client(fn, msg, node);
			}

			let client = url.split("/status/")[1];
			let clients = hub.clients;
			msg.payload = { stats: { rate: hub.stats_rate, total: hub.stats_total } };
			if (!client) {
		...
		//scan_topology(config, g)
		//find_all_ax_pipelines(config, g)
