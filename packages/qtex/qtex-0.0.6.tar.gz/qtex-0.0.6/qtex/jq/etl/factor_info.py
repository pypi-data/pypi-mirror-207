...
from qtex.jq import utils as jqu
from qtc.ext.logging import set_logger

from qtex.jq.etl.factor_values import logger
...
def persist_factor_groups(factor_groups,
    ...
    conn, database = jqu.get_conn_data_type(data_type=data_type,
                                            **db_config)
    schema = 'factor'
...
def prepare_factor_info(**db_config):
    ...
    factor_info['FactorId'] = factor_info.apply(
        lambda x: jqu.produce_factor_id(source_id=x['SourceId'], factor_code=x['FactorCode'])
    ...
    conn = jqu.get_conn_data_type(data_type='FACTOR_GROUP',
                                  **db_config)
