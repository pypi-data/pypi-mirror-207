...
def compareResultsWithExpected(
    ...
    if rulesResults != expectedRulesResults:
        for rule, ruleResult in rulesResults.items():
            if rule in expectedRulesResults.keys():
                expectedRuleResult = expectedRulesResults[rule]
                if isinstance(ruleResult, str):
                    if isinstance(expectedRuleResult, str): # and the rule is flat in the expected as well
                        if ruleResult != expectedRuleResult:
                            ...
                            violations.append((table, testName, ruleResult, expectedRuleResult, rule, ""))
                    ...
                    if isinstance(expectedRuleResult, str): # but the rule is not nested in the expected
                        if not compare_flat_to_nested_rule(expectedRuleResult, ruleResult):
                            # errors = addError(errors, testName, rule, ruleResult, expectedRuleResult)
                            violations.append((table, testName, rulesResults[rule], expectedRuleResult, rule, ""))

                    else:  # both rules are nested
                        #rule results in expected rule
                        compare_nested(ruleResult, expectedRuleResult[result], violations, testName, rule)
                        #expected rule in rule results
                        compare_nested(expectedRuleResult[result], ruleResult, violations, testName, rule)
                ...
                violations.append((table, testName, result, "\033[33mundefined\033[0m", rule, ""))
...
def compare_nested(nestedResults: Dict[str,Any], nestedExpected: Dict[str,Any], violations: List[Tuple], testName: str, rule: str):
    for result, funcList in nestedResults.items():
        funcList.sort()
        nestedExpected[result].sort()

        # compare functions lists (current results with expected)
        if funcList != nestedExpected[result]:
            for funcName in funcList:
                # if function appears in current results but does not appear in the expected ones
                if funcName not in nestedExpected[result]:
                    # errors = addError(errors, testName, rule, result, "", funcName)
                    expectedResult = findExpected(funcName, nestedExpected)
                    violations.append(((table, testName, result, expectedResult, rule, funcName)))
