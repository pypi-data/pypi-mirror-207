...
import struct, typing, os, re, textwrap
...
class Metas(object):
    ...
    def __init__(self, text: str, user_ids = None,
                 action: bool = True, font: bool = False, *args, **kwargs) -> (None):
        ...
        self.text, self.user_ids, self.action, self.font = text, user_ids, action, font

    def __replacer(self, text: str, action: bool = 'regex',
                   mode: bool = True) -> (str):
    ...
    def __to_parse(self, text: str,
                   user_ids: list = None, filter_hashtag: bool = True,
                   filter_under_line: bool = True) -> (tuple):
    ...
    def light_strike(self, text: str) -> str:
        return ''.join(chain.from_iterable(zip(text, repeat('\u0320'))))

    def strike(self, text: str) -> str:
        return ''.join(chain.from_iterable(zip(text, repeat('\u0336')))) #0320

    def replace_respond(self, text: str) -> str:
        # text = textwrap.shorten(text, width=text.__len__()+1)
        if not (re.compile(r'~').search(text) and (text.count('~') / 2).__str__().split('.')[1].__eq__('0')) and not (re.compile(r'&').search(text) and (text.count('&') / 4).__str__().split('.')[1].__eq__('0')):
            return text

        mode = '&&' in text

        for a, b in zip(map(lambda string: self.strike(string) if not mode else self.light_strike(string), (shit := re.compile(r'\~(\S+)\~' if not mode else r'\&\&(\S+)\&\&').findall(text))), shit):
            text = text.replace(b, a, 1)
        else:
            if mode:
                text = self.replace_respond(text.replace('&', ''))
    ...
    def checker(self) -> (list):
        ...
        if self.user_ids or self.action:
            return self.__to_parse(self.replace_respond(self.text) if self.font else self.text, self.user_ids, False, False)
            ...
            if self.text.__contains__('**'):
                ...
                [(result.append({'from_index': from_index - 2, 'length': length.__len__(), 'type': 'Bold'})) for from_index, length in zip(boldFromIndex, Bold)]
            
            if self.text.__contains__('__'):
                
                Italic: list = findall(r'\_\_(.*?)\_\_', self.text)
                ItalicFromIndex: list = [self.text.index(i) for i in Italic]
                [(result.append({'from_index': from_index - 2, 'length': length.__len__(), 'type': 'Italic'})) for from_index, length in zip(ItalicFromIndex, Italic)]
            
            if self.text.__contains__('``') and not self.text.__contains__('```'):
                
                Mono: list = findall(r'\`\`(.*?)\`\`', self.text)
                monoFromIndex: list = [self.text.index(i) for i in Mono]
                [self.text.index(i) for i in Mono]
                [(result.append({'from_index': from_index - 2, 'length': length.__len__(), 'type': 'Mono'})) for from_index, length in zip(monoFromIndex, Mono)]
            
            if self.text.__contains__('```'):
                Pre: list = findall(r'\`\`\`(.*?)\`\`\`', self.text)
                preFromIndex: list = [self.text.index(i) for i in Pre]
                [self.text.index(i) for i in Pre]
                [(result.append({'from_index': from_index - 3, 'length': length.__len__(), 'type': 'Pre'})) for from_index, length in zip(preFromIndex, Pre)]
