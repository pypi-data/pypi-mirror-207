...
from skailar.conf.global_settings import PASSWORD_HASHERS
from skailar.contrib.auth import get_user_model
from skailar.contrib.auth.backends import ModelBackend
from skailar.contrib.auth.base_user import AbstractBaseUser
from skailar.contrib.auth.hashers import get_hasher
from skailar.contrib.auth.models import (
    AnonymousUser,
    Group,
    Permission,
    User,
...
from skailar.db import connection, migrations
from skailar.db.migrations.state import ModelState, ProjectState
from skailar.db.models.signals import post_save
from skailar.test import SimpleTestCase, TestCase, TransactionTestCase, override_settings
...
class AbstractUserTestCase(TestCase):
    ...
    def test_user_double_save(self):
        ...
        with mock.patch(
            "skailar.contrib.auth.password_validation.password_changed"
        ) as pw_changed:
            user.save()
...
class UserWithPermTestCase(TestCase):
    ...
    @override_settings(
        AUTHENTICATION_BACKENDS=["skailar.contrib.auth.backends.BaseBackend"]
    )
...
class AnonymousUserTests(SimpleTestCase):
    no_repr_msg = "Skailar doesn't provide a DB representation for AnonymousUser."
