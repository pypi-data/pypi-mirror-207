...
from skailar.utils.encoding import (
    SkailarUnicodeDecodeError,
    escape_uri_path,
    filepath_to_uri,
    force_bytes,
    force_str,
...
class TestEncodingUtils(SimpleTestCase):
    ...
    def test_force_str_SkailarUnicodeDecodeError(self):
        msg = (
            "'utf-8' codec can't decode byte 0xff in position 0: invalid "
            "start byte. You passed in b'\\xff' (<class 'bytes'>)"
        )
        with self.assertRaisesMessage(SkailarUnicodeDecodeError, msg):
            force_str(b"\xff")
