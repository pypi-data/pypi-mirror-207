...
from renumics.spotlight.licensing import license_path
...
def main() -> None:
    ...
    # If we are here, license exists and is valid, but its path can be relative,
    # so extract absolute path.
    with set_temp_environ(
        JUPYTER_CONFIG_DIR=jupyter_config_folder,
        SPOTLIGHT_LICENSE_PATH=str(license_path),
    ):
