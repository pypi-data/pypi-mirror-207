...
import csv
import pickle
from gaiaframework.base.common import functions
...
class ZIDS_SharedArtifacts():
    ...
    def load_config_json(self):
        """! Loads config and sets its content to attribute.
        It enables to add any required settings to the config file (located here: config/config.json),
        which will be auto-loaded and be accessible via the
        SharedArtifacts class.
        """
    ...
    def load_vocabs(self):
        """! Loads vocabulary from path specified in config.json to attributes, accessible via SharedArtifacts class.
        file can be in any format, we have the following ready to use:
        json, csv, pickle, tensorflow, key_value and key_value_int.
        For additional formats override the function extend_load_file_type for example:
    ...
    def load_artifacts(self):
        """! Load artifacts from path specified in config.json to attributes, accessible via SharedArtifacts class.
    ...
    def load_file(self, item):
        """! Method is used by load_vocabs and load_artifacts methods.
    ...
    def load_tensorflow(self, path, name):
        ...
        Important:
            This method is remarked and not implemented, please override to use its functionality.
