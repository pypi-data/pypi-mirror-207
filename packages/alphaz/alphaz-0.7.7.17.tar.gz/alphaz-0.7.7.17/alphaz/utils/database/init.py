...
def process_init_files(
    core: AlphaCore, binds: list[str], tables: list[str], init_databases_config: dict, no_log:bool = False
):
    ...
    for file_path in get_init_files(core, binds, init_databases_config):
        ...
        for table_name, conf in ini.items():
            ...
            if not isinstance(conf, dict):
                core.log.error(
                    f"Configuration for {table_name=} in file {file_path} must be of type <dict>", enabled=not no_log
                )
                continue
    ...
    for table_name in tables:
        ...
        for configuration in tables_configs[table_name]:
            ...
            if entries:
                ...
                core.log.info(
                    f"{table_name=} and {bind=} initiatied with <objects> format with file {file_name}", enabled=not no_log
                )
            ...
            if len(values) != 0:
                ...
                core.log.info(
                    f"{table_name=} and {bind=} initiatied with <values> format with file {file_name}", enabled=not no_log
                )
            ...
            if len(values) != 0:
                ...
                core.log.info(
                    f"{table_name=} and {bind=} initiatied with <results> format with file {file_name}", enabled=not no_log
                )
