...
def get_frame_of_day(date_or_j2000, itrf_path_or_df, discon_path_or_df, psd_path_or_df=None, list_path_or_df=None):
    ...
    comboindex = _pd.Index(output.CODE_PT + '_' + output.SOLN.values.astype(str))
    itrf_code_pt = _pd.Index(output.CODE_PT.values, name=None).unique()
    soln_series = _pd.Series(data=0,index=itrf_code_pt,dtype=int)
