...
class EngineTest(SimpleTestCase):
    def test_repr_empty(self):
        ...
        self.assertEqual(
            ...
            "'skailar.template.defaulttags', 'skailar.template.defaultfilters', "
            "'skailar.template.loader_tags'] autoescape=True>",
        )

    def test_repr(self):
        engine = Engine(
            dirs=[TEMPLATE_DIR],
            context_processors=["skailar.template.context_processors.debug"],
            debug=True,
        ...
        self.assertEqual(
            ...
            "context_processors=['skailar.template.context_processors.debug'] "
            "debug=True loaders=['skailar.template.loaders.filesystem.Loader'] "
            "string_if_invalid='x' file_charset='utf-16' "
            "libraries={'custom': 'template_tests.templatetags.custom'} "
            "builtins=['skailar.template.defaulttags', "
            "'skailar.template.defaultfilters', 'skailar.template.loader_tags'] "
            "autoescape=False>",
        )
...
class GetDefaultTests(SimpleTestCase):
    ...
    def test_no_engines_configured(self):
        msg = "No SkailarTemplates backend is configured."
