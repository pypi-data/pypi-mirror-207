...
from scrippy_snmp import ScrippySnmpError, VERSIONS, DATATYPES
...
class Snmp:
  ...
  def __init__(self, host, port, community, version, timeout=1, retries=3):
    logging.info("[+] SNMP agent initialization")
  ...
  def set(self, oid, value, datatype):
    logging.info(f"[+] Setting value to OID {oid} => {value}")
  ...
  def _set_community(self, community, version):
    logging.info(f" '-> Setting community: {community} / Version: {version}")
  ...
  def _set_transport(self, timeout=1, retries=3):
    logging.info(f" '-> Setting transport: UDP: {self.host}:{self.port}")
