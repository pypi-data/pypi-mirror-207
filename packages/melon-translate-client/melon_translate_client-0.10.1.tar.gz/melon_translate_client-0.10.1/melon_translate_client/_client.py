...
class Client:
    ...
    def _remove_existing_keys(self, language: str, translations: List[dict]) -> list:
        """
        ...
        for translate in translations:
            ...
            if _views:
                record = self.cache.hexists(_ID_NAME_MAP, _id_name)
            elif _occurrences:
                record = self.cache.hexists(_SNAKE_NAME_MAP, _snake)
            else:
                record = None

            if record:
                _already_cached.append(record)
            else:
    ...
    def filter(
        ...
        translations_for_caching = self._remove_existing_keys(language, translations)
        ...
        if translations_for_caching:
            ...
            result = {}
            log.info("There are not new translations to cache!")
