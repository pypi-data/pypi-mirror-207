...
from skailar.contrib.auth import BACKEND_SESSION_KEY, REDIRECT_FIELD_NAME, SESSION_KEY
from skailar.contrib.auth.forms import (
    AuthenticationForm,
    PasswordChangeForm,
    SetPasswordForm,
)
from skailar.contrib.auth.models import Permission, User
from skailar.contrib.auth.views import (
...
from skailar.contrib.sites.requests import RequestSite
from skailar.core import mail
from skailar.core.exceptions import ImproperlyConfigured
from skailar.db import connection
from skailar.http import HttpRequest, HttpResponse
from skailar.middleware.csrf import CsrfViewMiddleware, get_token
from skailar.test import Client, TestCase, override_settings
from skailar.test.client import RedirectCycleError
from skailar.urls import NoReverseMatch, reverse, reverse_lazy
from skailar.utils.http import urlsafe_base64_encode
...
@override_settings(ROOT_URLCONF="skailar.contrib.auth.urls")
...
class PasswordResetTest(AuthViewsTestCase):
    ...
    def test_poisoned_http_host(self):
        ...
        with self.assertLogs("skailar.security.DisallowedHost", "ERROR"):
            response = self.client.post(
                "/password_reset/",
                {"email": "staffmember@example.com"},
    ...
    @override_settings(
        AUTHENTICATION_BACKENDS=[
            ...
            "skailar.contrib.auth.backends.AllowAllUsersModelBackend",
        ]
    )
    def test_confirm_login_post_reset_custom_backend(self):
        # This backend is specified in the URL pattern.
        backend = "skailar.contrib.auth.backends.AllowAllUsersModelBackend"
        url, path = self._test_confirm_start()
...
class LoginTest(AuthViewsTestCase):
    def test_current_site_in_context_after_login(self):
        ...
        self.assertEqual(response.status_code, 200)
        if apps.is_installed("skailar.contrib.sites"):
    ...
    def test_login_session_without_hash_session_key(self):
        """
        Session without skailar.contrib.auth.HASH_SESSION_KEY should login
...
class ChangelistTests(AuthViewsTestCase):
    ...
    @mock.patch("skailar.contrib.auth.admin.UserAdmin.has_change_permission")
