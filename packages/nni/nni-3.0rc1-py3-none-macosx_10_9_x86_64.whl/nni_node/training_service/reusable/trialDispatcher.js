...
let TrialDispatcher = TrialDispatcher_1 = class TrialDispatcher {
    ...
    async asyncConstructor(config) {
        await (0, util_1.validateCodeDir)(config.trialCodeDirectory);
        const serviceConfigs = Array.isArray(config.trainingService) ? config.trainingService : [config.trainingService];
        const servicePromises = serviceConfigs.map(serviceConfig => (0, environmentServiceFactory_1.createEnvironmentService)(serviceConfig));
    ...
    async submitTrialJob(form) {
        const trialId = form.id === undefined ? (0, utils_1.uniqueString)(5) : form.id;
    ...
    async environmentMaintenanceLoop() {
        while (!this.stopping) {
            ...
            await (0, utils_1.delay)(this.environmentMaintenceLoopInterval);
        }
    }
    ...
    selectEnvironmentService() {
        ...
        return (0, utils_1.randomSelect)(validEnvironmentServiceList);
    }
    ...
    async setEnvironmentSetting(environment) {
        ...
        runnerSettings.nniManagerIP = this.config.nniManagerIp === undefined ? await (0, utils_1.getIPV4Address)() : this.config.nniManagerIp;
        runnerSettings.nniManagerPort = (0, experimentStartupInfo_1.getBasePort)() + 1;
        ...
        runnerSettings.nniManagerVersion = this.enableVersionCheck ? await (0, utils_1.getVersion)() : '';
    ...
    async requestEnvironment(environmentService) {
        ...
        const envId = (0, utils_1.uniqueString)(5);
    ...
    async handleStdout(commandData) {
        ...
        const trialLogDir = path_1.default.join((0, utils_1.getExperimentRootDir)(), 'trials', commandData["trial"]);
