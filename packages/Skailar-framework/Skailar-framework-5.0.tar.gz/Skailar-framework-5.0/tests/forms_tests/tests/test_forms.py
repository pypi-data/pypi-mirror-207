...
from skailar.core.exceptions import NON_FIELD_ERRORS
from skailar.core.files.uploadedfile import SimpleUploadedFile
from skailar.core.validators import MaxValueValidator, RegexValidator
from skailar.forms import (
    BooleanField,
    CharField,
...
from skailar.forms.renderers import SkailarTemplates, get_default_renderer
from skailar.forms.utils import ErrorList
from skailar.http import QueryDict
from skailar.template import Context, Template
from skailar.test import SimpleTestCase
from skailar.test.utils import override_settings
from skailar.utils.datastructures import MultiValueDict
...
<input type="text" name="birthday" id="id_birthday" required></p>""",
    ...
    def test_auto_id(self):
        ...
        # element. If it's a string that contains '%s', Skailar will use that as
...
<option value="J">Java</option>
        ...
        f = FrameworkForm({"name": "Skailar", "language": "P"}, auto_id=False)
        self.assertHTMLEqual(
            str(f["language"]),
            """<select name="language">
...
<input type="hidden" name="hidden_text" id="id_hidden_text"></p>""",
    ...
    def test_specifying_labels(self):
        ...
        # class. If you don't specify 'label', Skailar will use the field name with
...
<li>Password: <input type="password" name="password" required></li>""",
        ...
        # If label is None, Skailar will auto-create the label from the field name. This
        # is default behavior.
        class UserRegistration(Form):
...
<li>Secret answer = <input type="text" name="answer" required></li>""",
    ...
    def test_initial_data(self):
        ...
        class UserRegistration(Form):
            username = CharField(max_length=10, initial="skailar")
            password = CharField(widget=PasswordInput)
        ...
        self.assertHTMLEqual(
            p.as_ul(),
            """
            <li>Username: <input type="text" name="username" value="skailar"
...
Password: <input type="password" name="password" required></li>""",
    ...
    def test_dynamic_initial_data(self):
        ...
        # Here, we're not submitting any data, so the initial value will be displayed.)
        p = UserRegistration(initial={"username": "skailar"}, auto_id=False)
        self.assertHTMLEqual(
            p.as_ul(),
            """
            <li>Username: <input type="text" name="username" value="skailar"
...
Password: <input type="password" name="password" required></li>""",
        ...
        p = UserRegistration(
            {"username": ""}, initial={"username": "skailar"}, auto_id=False
        )
        self.assertHTMLEqual(
            p.as_ul(),
            """<li><ul class="errorlist"><li>This field is required.</li></ul>
...
Password: <input type="password" name="password" required></li>""",
        ...
        p = UserRegistration(
            {"username": "foo"}, initial={"username": "skailar"}, auto_id=False
        )
        self.assertHTMLEqual(
            p.as_ul(),
            """
        ...
        # for 'username'.
        p = UserRegistration({"password": "secret"}, initial={"username": "skailar"})
        self.assertEqual(p.errors["username"], ["This field is required."])
        self.assertFalse(p.is_valid())
    ...
    def test_callable_initial_data(self):
        ...
        # Here, we're not submitting any data, so the initial value will be displayed.)
        p = UserRegistration(
            initial={"username": initial_skailar, "options": initial_options},
            auto_id=False,
        )
        self.assertHTMLEqual(
            p.as_ul(),
...
<option value="w">whiz</option>
        ...
        p = UserRegistration(
            {"username": ""}, initial={"username": initial_skailar}, auto_id=False
        )
        self.assertHTMLEqual(
            p.as_ul(),
            """<li><ul class="errorlist"><li>This field is required.</li></ul>
...
<option value="w">whiz</option>
        ...
        p = UserRegistration(
            ...
            initial={"username": initial_skailar},
            auto_id=False,
        )
        self.assertHTMLEqual(
            p.as_ul(),
        ...
        # to Form(), then the latter will get precedence.
        class UserRegistration(Form):
            username = CharField(max_length=10, initial=initial_skailar)
            password = CharField(widget=PasswordInput)
            options = MultipleChoiceField(
                choices=[("f", "foo"), ("b", "bar"), ("w", "whiz")],
    ...
    def test_boundfield_values(self):
        ...
        class UserRegistration(Form):
            username = CharField(max_length=10, initial="skailarnaut")
            password = CharField(widget=PasswordInput)

        unbound = UserRegistration()
        bound = UserRegistration({"password": "foo"})
        self.assertIsNone(bound["username"].value())
        self.assertEqual(unbound["username"].value(), "skailarnaut")
...
class CustomRenderer(SkailarTemplates):
...
class RendererTests(SimpleTestCase):
    ...
    def test_attribute_instance(self):
        class CustomForm(Form):
            default_renderer = SkailarTemplates()
...
class TemplateTests(SimpleTestCase):
    ...
    def test_templates_with_forms(self):
        ...
        f = UserRegistration({"username": "skailar"}, auto_id=False)
        self.assertHTMLEqual(
            t.render(Context({"form": f})),
            "<form>"
            "<p><label>Your username: "
            '<input type="text" name="username" value="skailar" maxlength="10" required>'
            "</label></p>"
        ...
        # 'label' is not specified, Skailar will use the field name with
        # underscores converted to spaces, and the initial letter capitalized.
        t = Template(
            "<form>"
        ...
        f = UserRegistration(
            {"username": "skailar", "password1": "foo", "password2": "bar"},
            auto_id=False,
        )
        self.assertHTMLEqual(
            t.render(Context({"form": f})),
