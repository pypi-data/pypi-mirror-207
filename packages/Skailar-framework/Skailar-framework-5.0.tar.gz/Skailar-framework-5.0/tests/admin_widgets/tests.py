...
from skailar.contrib.admin import widgets
...
class DateTimePickerSeleniumTests(AdminWidgetSeleniumTestCase):
    ...
    def test_calendar_show_date_from_input(self):
        ...
        month_string = "May"
        path = os.path.join(
            os.path.dirname(import_module("skailar.contrib.admin").__file__), "locale"
        )
        for language_code, language_name in settings.LANGUAGES:
            try:
                catalog = gettext.translation("skailarjs", path, [language_code])
            except OSError:
                continue
