...
from devapp.tools import FLG, define_flags, now
from devapp.tools import partial as p
...
def wsgi(self, environ, start_response, response=response):
    ...
    try:
        ...
        if 'chunked' in environ.get('HTTP_TRANSFER_ENCODING', '').lower():
            app.warn("Chunked request on non chunk enabled endpoint. Awaiting whole upstream")
        ...
        for k in response.headerlist:
            if k[0] == 'Status-Line':
                ...
                hl.append(k)
        start_response(sl or response._status_line, hl)
        # ------------ end patch ---------------------------------------------:
        return out
...
class http:
    ...
    def _serialize(data): return json.dumps(data, default=str).encode('utf-8')

    @classmethod
    def request(cls, observer, **kw):
        d = connections.con_params(cls)
        d.update(kw)
        ...
        def verify_token(path, token_key, headers, _secr=FLG.http_token_secret):
            try:
                ...
                if not '*' in allowed_verbs:
                    if allowed_verbs and not v in allowed_verbs:
    ...
    def _err(err_msg, details, status_code=500):
