...
class LoginService:
    ...
    # 验证账户检查密码
    @staticmethod
    def check_login(account, password, platform_code=None, **kwargs):
        """
        @param user_id 用户ID
        @param password 用户密码。
        ...
        user_list = BaseInfo.objects.filter(Q(user_name=account) | Q(phone=account) | Q(email=account)).annotate(
            user_id=F("id"))
        # .values('user_id', 'user_name', 'full_name', 'nickname', 'phone', 'email', 'user_info', 'privacies' )
        if platform_code:

            user_list = user_list.filter(Q(platforms_to_users__platform__platform_code=platform_code))
            print(">>>if", user_list, user_list.query)
        else:
            user_list = user_list.filter(Q(platforms_to_users__platform__isnull=True))
            print(">>>else", user_list)
        ...
        #         platforms_users = platforms_users.filter(**{"platform_id": platform_id})
        #     else:
        #         return None, "平台不存在"
        # platforms_users = platforms_users.first()
        #
        # if not platforms_users:
        #     return None, "数据验证失败"
        # platforms_users = model_to_dict(platforms_users)
        # print(platforms_users)
        ...
        # 检查平台是否存在
        where = {
            "user_id": platforms_users.get("user"),
        ...
        #     else:
        #         where['platform_id__isnull'] = True
        # else:
        #     where['platform_id__isnull'] = True

        auth_set = Auth.objects.filter(**where).order_by('-update_time').first()
        if not auth_set:
            return None, "账户尚未开通登录服务：" + account + "(" + str(platforms_users.get("user")) + ")"
        ...
        return {"user_id": platforms_users.get("user")}, None
