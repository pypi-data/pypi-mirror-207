...
# pair_corr2
def pair_corr2(x, y, window=10):
    return x.rolling(window).corr(other=y)
...
#
def rolling_residual_sum(x, y, window=10):
    _y_mean = y.rolling(window).mean()
    _x_mean = x.rolling(window).mean()
    _beta = rolling_beta(x, y, window=window)
    _alpha = _y_mean - _x_mean * _beta
    return y.rolling(window).sum() - (_alpha + _beta * x).rolling(window).sum()


#
def rolling_residual_sum2(x, y, window=10, window2=5):
    _y_mean = y.rolling(window).mean()
    _x_mean = x.rolling(window).mean()
    _beta = rolling_beta(x, y, window=window)
    _alpha = _y_mean - _x_mean * _beta
    return y.rolling(window2).sum() - (_alpha + _beta * x).rolling(window2).sum()
...
# rolling high_low
def rolling_hl(x, window=10):
    rolling_min = x.rolling(window).min()
    rolling_max = x.rolling(window).max()
    return (x - rolling_min)/(rolling_max - rolling_min) - 0.5


#
def rolling_rsquare(x, y, window=10):
    _x = np.array(x)
    _y = np.array(y)

    output = [np.nan for _ in range(window - 1)]
    for i in range(window - 1, len(x)):
        output.append(np.corrcoef(_x[i - window:i], _y[i - window:i])[0, 1])

    return pd.Series(output, index=x.index, name=x.name)


#
def rolling_sim_yoy(x, window=250):
    temp = pd.Series(x)
...
def get_residual(x, y):
    _beta = x.cov(y) / x.var()
    _alpha = y.mean() - x.mean() * _beta
    return y - (_beta * x + _alpha)


#
def get_poly_param(x, deg=1, ind=1):
    if ind > deg:
        return
    return np.polyfit(np.linspace(1, len(x), num=len(x)), x, deg=deg)[deg - ind]


def get_pair_poly_param(x, y, deg=1, ind=1):
    if ind > deg:
        return
    return np.polyfit(x, y, deg=deg)[deg - ind]
...
#
def rolling_pair_poly_param(x, y, window=10, deg=1, ind=1):
    if ind > deg:
        return
    return x.rolling(window).apply(lambda m: np.polyfit(m, y.loc[m.index], deg=deg)[deg - ind])
...
# pubu
def pubu(x, window=10):
    return (sma(x, window=window*4) + sma(x, window=window*2) + ema(x, window=window))/3
...
def cut_time_period(input_list, num=10):
    length = len(input_list)

    point_list = [0] + [int(length / num * i) for i in range(1, num)] + [length - 1]

    output = []

    for i in range(1, len(point_list)):
        if i == len(point_list) - 1:
            start = input_list[point_list[i - 1]]
            end = input_list[point_list[i]]
        else:
            start = input_list[point_list[i - 1]]
            end = input_list[point_list[i] - 1]
