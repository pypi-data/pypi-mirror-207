...
from skailar.utils.jslex import prepare_js_for_gettext
from skailar.utils.regex_helper import _lazy_re_compile
from skailar.utils.text import get_text_list
from skailar.utils.translation import templatize
...
class BuildFile:
    ...
    def is_templatized(self):
        if self.domain == "skailarjs":
            return self.command.gettext_version < (0, 18, 3)
        elif self.domain == "skailar":
...
class Command(BaseCommand):
    help = (
        ...
        "conf/locale (in the skailar tree) or locale (for projects and applications) "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "-d",
            default="skailar",
            help='The domain of the message files (default: "skailar").',
        )
        parser.add_argument(
            "--all",
            "-a",
        ...
        parser.add_argument(
            ...
            'if the domain is "skailarjs"). Separate multiple extensions with '
    ...
    def handle(self, *args, **options):
        ...
        if self.domain not in ("skailar", "skailarjs"):
            raise CommandError(
                "currently makemessages only supports domains "
                "'skailar' and 'skailarjs'"
            )
        if self.domain == "skailarjs":
        ...
        self.invoked_for_skailar = False
        self.locale_paths = []
        self.default_locale_path = None
        if os.path.isdir(os.path.join("conf", "locale")):
            self.locale_paths = [os.path.abspath(os.path.join("conf", "locale"))]
            self.default_locale_path = self.locale_paths[0]
            self.invoked_for_skailar = True
        else:
    ...
    def process_locale_dir(self, locale_dir, files):
        ...
        for translatable in files:
            ...
            if self.domain not in ("skailarjs", "skailar"):
                continue
    ...
    def write_po_file(self, potfile, locale):
        ...
        if os.path.exists(pofile):
            ...
            with open(potfile, encoding="utf-8") as fp:
                msgs = fp.read()
            if not self.invoked_for_skailar:
    ...
    def copy_plural_forms(self, msgs, locale):
        """
        Copy plural forms header contents from a Skailar catalog of locale to
        the msgs string, inserting it at the right place. msgs should be the
        contents of a newly created .po file.
        """
        skailar_dir = os.path.normpath(os.path.join(os.path.dirname(skailar.__file__)))
        if self.domain == "skailarjs":
            domains = ("skailarjs", "skailar")
        else:
            domains = ("skailar",)
        for domain in domains:
            skailar_po = os.path.join(
                skailar_dir, "conf", "locale", locale, "LC_MESSAGES", "%s.po" % domain
            )
            if os.path.exists(skailar_po):
                with open(skailar_po, encoding="utf-8") as fp:
