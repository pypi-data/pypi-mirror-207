    ...
    def test_run(
        ...
        **mock_sources: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        if data_source_type == DataSourceType.PUSH_NO_BATCH:
            msg = f"The `test_run()` method is currently unsupported for Feature Views that are backed by a PushSource without a batch_config"
            raise TectonValidationError(msg)

        if start_time is None or end_time is None:
            msg = "test_run() requires start_time and end_time to be set."
            raise TypeError(msg)
...
def _source_to_pipeline_node(
    ...
    if isinstance(source, framework_data_source.DataSource):
        ...
        # TODO(TEC-13178): Stop populating `SparkSchema` after migration.
        wrapper = type_utils.to_spark_schema_wrapper(request_schema)

        node = pipeline_pb2.RequestDataSourceNode(
            request_context=pipeline_pb2.RequestContext(schema=wrapper.to_proto(), tecton_schema=schema_proto),
            input_name=input_name,
        )
