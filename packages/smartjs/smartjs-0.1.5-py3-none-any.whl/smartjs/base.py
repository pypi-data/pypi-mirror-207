...
class BaseElement(UserString):
    ...
    @property
    def script_elements(self):
        return join(list_of_type(self.args, Script))
    ...
    def render(self):
        if self.tag.value in EMPTY_TAGS:
            return f'{self.render_before()}<{self.tag} {self.config}>{self.render_elements()}{join(self.render_after())}{self.script_elements}{self.render_script()}'
        return f'{self.render_before()}<{self.tag} {self.config}>{self.render_elements()}</{self.tag}>{self.render_after()}{self.script_elements}{self.render_script()}'
