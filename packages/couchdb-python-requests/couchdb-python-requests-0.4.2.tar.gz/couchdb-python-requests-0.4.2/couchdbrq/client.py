...
class ViewResults(object):
    ...
    def docs(self, wrapper_class=None):
        """Return list of documents (row.doc), optionally wrapped by class wrapper_class

        :type: `list`
        """
        if wrapper_class:
            return [wrapper_class(row.doc) for row in self.rows]
        else:
