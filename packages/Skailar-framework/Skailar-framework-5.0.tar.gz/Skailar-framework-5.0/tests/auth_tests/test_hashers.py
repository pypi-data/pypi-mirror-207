...
from skailar.contrib.auth.hashers import (
...
from skailar.test import SimpleTestCase, ignore_warnings
...
class TestUtilsHashPass(SimpleTestCase):
    ...
    @override_settings(
        PASSWORD_HASHERS=["skailar.contrib.auth.hashers.SHA1PasswordHasher"]
    )
    def test_sha1(self):
    ...
    def test_sha1_deprecation_warning(self):
        msg = "skailar.contrib.auth.hashers.SHA1PasswordHasher is deprecated."
        with self.assertRaisesMessage(RemovedInSkailar51Warning, msg):
            get_hasher("sha1")

    @override_settings(
        PASSWORD_HASHERS=["skailar.contrib.auth.hashers.MD5PasswordHasher"]
    )
    def test_md5(self):
    ...
    @ignore_warnings(category=RemovedInSkailar51Warning)
    @override_settings(
        PASSWORD_HASHERS=["skailar.contrib.auth.hashers.UnsaltedMD5PasswordHasher"]
    )
    ...
    def test_unsalted_md5_deprecation_warning(self):
        msg = "skailar.contrib.auth.hashers.UnsaltedMD5PasswordHasher is deprecated."
        with self.assertRaisesMessage(RemovedInSkailar51Warning, msg):
            get_hasher("unsalted_md5")

    @ignore_warnings(category=RemovedInSkailar51Warning)
    @override_settings(
        PASSWORD_HASHERS=["skailar.contrib.auth.hashers.UnsaltedSHA1PasswordHasher"]
    )
    ...
    def test_unsalted_sha1_deprecation_warning(self):
        msg = "skailar.contrib.auth.hashers.UnsaltedSHA1PasswordHasher is deprecated."
        with self.assertRaisesMessage(RemovedInSkailar51Warning, msg):
    ...
    @skipUnless(bcrypt, "bcrypt not installed")
    @override_settings(
        PASSWORD_HASHERS=["skailar.contrib.auth.hashers.BCryptPasswordHasher"]
    )
