"""
Sphinx plugins for Skailar documentation.
"""
import json
import os
import re
...
def setup(app):
    ...
    app.add_object_type(
        directivename="skailar-admin",
        rolename="djadmin",
        indextemplate="pair: %s; skailar-admin command",
        parse_node=parse_skailar_admin_node,
    )
    app.add_directive("skailar-admin-option", Cmdoption)
    app.add_config_value("skailar_next_version", "0.0", True)
    app.add_directive("versionadded", VersionDirective)
    app.add_directive("versionchanged", VersionDirective)
    app.add_builder(SkailarStandaloneHTMLBuilder)
    app.set_translator("skailarhtml", SkailarHTMLTranslator)
    app.set_translator("json", SkailarHTMLTranslator)
    app.add_node(
        ConsoleNode,
...
class VersionDirective(Directive):
    ...
    def run(self):
        ...
        if self.arguments[0] == env.config.skailar_next_version:
...
class SkailarHTMLTranslator(HTMLTranslator):
    """
    Skailar-specific reST to HTML tweaks.
    """
    ...
    #
    version_text = {
        "versionchanged": "Changed in Skailar %s",
        "versionadded": "New in Skailar %s",
    }
...
def parse_skailar_admin_node(env, sig, signode):
    command = sig.split(" ")[0]
    env.ref_context["std:program"] = command
    title = "skailar-admin %s" % sig
    signode += addnodes.desc_name(title, title)
    return command


class SkailarStandaloneHTMLBuilder(StandaloneHTMLBuilder):
    """
    ...
    def finish(self):
        ...
        with open(outfilename, "w") as fp:
            fp.write("var skailar_template_builtins = ")
...
def visit_console_html(self, node):
    ...
    if self.builder.name in ("skailarhtml", "json") and node["win_console_text"]:
...
class ConsoleDirective(CodeBlock):
    ...
    def run(self):
        ...
        # Only do work when the skailarhtml HTML Sphinx builder is being used,
        # invoke the default behavior for the rest.
        if env.app.builder.name not in ("skailarhtml", "json"):
            return [lit_blk_obj]
