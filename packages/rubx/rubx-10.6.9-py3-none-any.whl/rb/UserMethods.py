...
class UserMethods:
    ...
    @classmethod
    def register_device(cls, auth: (str),
    ...
    @classmethod
    def register_account(cls, session: str,
                         device: dict = Device.DEFAULT_DEVICE) -> dict:
        return cls.register_device(session, device)
    ...
    def parsation (
        ...
        if (mode.upper().startswith('HTML')):
            ...
            return Metas(text, user_ids=user_ids, font=font).checker
    ...
    def send_message(
        ...
        **kwargs
            ...
            if parse_mode or '__' in text or '**' in text or '``' in text or '```' in text or '~' in text or mention_user_ids or len(findall(r'<(.*?)>', text)) + 1 >= 2:
                
                parse_mode: str = parse_mode or ''
                
                if parse_mode.lower() == 'markdown' or mention_user_ids or any([*text].count(meta) >= 4 for meta in ('_', '*', '`')) or text.count('~') >= 2:
                    
                    find: bool = True
                    
                    if search(r'(\@.*?\_\_.*?\_\_\w+)|(\@.*?\_\_\w+\_\_\w+)|(\@.*?\_\_\_\_\w+)', text):
                        if mention_user_ids or '__' in sub(compile(r'(\@.*?\_\_.*?\_\_\w+)'), '', text) or '**' in text or '``' in text or '~~' in text or '~' in text:
                            pass # TODO: set a func to use
                        else:
                            find: bool = False

                    if find:
                        base: list = UserMethods.parsation('markdown', text, user_ids=mention_user_ids, font=font)
