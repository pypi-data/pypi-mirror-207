...
__all__ = ['Descriptor', 'HiddenField', 'DetaModelField', 'DateField', 'DateTimeField', 'TitleField', 'EnumField',
           'CheckBoxField', 'AutoField', 'StringField', 'RegexField']
...
class Descriptor(ABC):
    ...
    def validate(self, value):
        ...
        if self.type_hint.check_type(value):
            ...
            raise self.ValidationError(f'{type(self.owner).__name__}.{self.public_name} exige um tipo {self.type_hint.expected_type}, que não coresponde ao encontrado para {value} ({type(value)}).')
