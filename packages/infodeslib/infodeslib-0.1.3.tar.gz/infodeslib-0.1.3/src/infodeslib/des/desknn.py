...
class DESKNN: 
    """
    Late Fusion version of Dynamic ensemble Selection KNN (DES-KNN). 
    k : int (Default = 7)
    ...
    def __init__(self, pool_classifiers=None, feature_subsets=None, k=7, N=3, J=2, DFP=False, knn_metric='minkowski', 
    ...
    def get_pairwise_diversity(self, diversity_func):
        num_models = len(self.pool_classifiers)
        diversity_names_map = {"Q": "Q-statistic", "CC": "Correlation Coefficient", "DM": "Disagreement Measure", 
                               "DF": "Double Fault", "NDF": "Negative Double Fault", "RE": "Ratio Errors"} 
        ...
        resultsDF = pd.DataFrame({"Model": model_names, "Diversity": diversity_scores}) 
        return resultsDF 
    ...
    def select(self, competences):    
        ...
        competenceDF = self.get_pairwise_diversity("DF")  
        competenceDF['Competence'] = competences 
        competenceDF['Index'] = competenceDF.index 
        
        ### get N most accurate 
        selected_models_indices = competenceDF.sort_values(by='Competence', ascending=False)['Index'].tolist()[:self.N] 
        selected_models_indices = competenceDF[competenceDF['Index'].isin(selected_models_indices)].sort_values(by='Diversity')['Index'].tolist()[:self.J] 
