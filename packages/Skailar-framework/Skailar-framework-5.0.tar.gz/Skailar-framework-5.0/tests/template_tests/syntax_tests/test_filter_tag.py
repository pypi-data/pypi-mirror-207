...
class FilterTagTests(SimpleTestCase):
    ...
    @setup({"filter02": "{% filter upper %}skailar{% endfilter %}"})
    def test_filter02(self):
        output = self.engine.render_to_string("filter02")
        self.assertEqual(output, "SKAILAR")

    @setup({"filter03": "{% filter upper|lower %}skailar{% endfilter %}"})
    def test_filter03(self):
        output = self.engine.render_to_string("filter03")
        self.assertEqual(output, "skailar")

    @setup({"filter04": "{% filter cut:remove %}skailarspam{% endfilter %}"})
