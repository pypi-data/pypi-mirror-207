...
from operators.const import ax_pipelines, stop  # , sources
...
from operators.testing.auto_docs import gen_build_doc
...
def remember_and_cut_secondary_nr_exit_wires(op, AP):
    ...
    The removed ones we register at secondary_nr_outs, and will on_next to them
    ...
    for ws in op.get('wires', ()):
        ...
        if pys:
            # when there are py exits (and we removed all wires to NR) we send to NodeRE# via this:
            AP['secondary_nr_outs'].setdefault(op['id'], {})[port] = nrs
...
def rewire_and_register(flow, AP):
    ...
    def insert_virtual_ops():
        for op in flow:
            if op.get('_is_py'):
                for w in list(op['ws']):
                    # from python to node red:
                    if not w in py_op_ids:
                        vsnks.append(make_nr_snk(op, w))
            else:
                # node red to python?
                for w in op['ws']:
    ...
    insert_virtual_ops()
    # FIXME: That should not be needed anymore after secondary_nr_outs feature:
    rm_dupl_ups(vsnks)
...
def modify_wires_to(virtual_nr_snks):
    def to_wires(op):
        ...
        to['wires'] = [[w for w in ws if not w in nrs] for ws in to['wires']]
        # drop except first, when now empty:
        for i in range(len(to['wires']) - 1, 0, -1):
            to['wires'].pop(i) if not to['wires'][i] else 0
        # add nr one to first:
        to['wires'][0].append(op['id'])
        # flatten
        to['ws'] = wires(to)
...
def clear_all(ext_out, dyn_snk_types, into=ax_pipelines):
    ...
    AP['secondary_nr_outs']         = {}  # simple node red outs, w/o pipeline effect
...
def build_pipelines(flow, ext_out=None, dyn_snk_types=None, force_rebuild=None):
    ...
    ext_out is a function handling outgoing (non py) data
    ...
    if not req and not force_rebuild:
        for k in 'ops', 'secondary_nr_outs':
            ax_pipelines[k] = AP[k]
    ...
    if force_rebuild:
        ...
        if diff is None:
            ...
            app.warn('Full rebuild - major change', **diff)
...
def does_change_require_rebuild(AP):
    ...
    if not ax_pipelines:
        ...
        return True, None
    for k in ('cfg', 'envs'):
        if AP[k] != ax_pipelines[k]:
            app.info('major change - cfg / env diff', was=ax_pipelines[k], now=AP[k])
            return True, jdiff(AP[k], ax_pipelines[k])

    oldtabs = literal_eval(ax_pipelines['prebuild_pyops_serialized'])
    newtabs = AP['prebuild_pyops']
    diff = {k: jdiff(ax_pipelines[k], AP[k]) for k in ['ops', 'secondary_nr_outs']}
    ...
    for tab in oldtabs:
        ...
        for opo in po:
            opn = pnm.get(opo['id'], {})
            for k in 'kw', 'name', 'wires', 'type':
                if opo.get(k) != opn.get(k):
                    return (
                        True,
                        {
                            'fulldiff': diff,
...
def bld_tab_ops(tab, AP, tabs):
    ...
    drawplot = partial(
        ...
        py_flow,
        all_tabs=True,
        secondary_nr_outs=AP['secondary_nr_outs'],
        tag=tab + '.post_build',
    )
