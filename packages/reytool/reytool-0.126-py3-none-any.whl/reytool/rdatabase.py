...
class REngine(object):
    ...
    def execute_retry(
        ...
        begin : RootTransaction object.
        _retry : whether is this a retry.
        ...
        if self.__class__ == REngine:
            ...
            ### Retry.
                ...
                rprint(
                    ...
                    frame=roption.print_default_frame_half
                )
                begin.close()
                self.engine = self.create_engine()
        ...
        ## When RConnection.
            ...
            ### Not retry.
                ...
                begin.close()
                self.begin = None
                self.rengine.engine: Engine = self.rengine.create_engine()
                self.engine: Engine = self.rengine.engine
                self.connection = self.engine.connect()

        # Re execute.
        result = self.execute(sql, data, report, True)
        ...
        **kwdata: Any
        ...
        if self.__class__ == REngine:
            ...
            with self.engine.connect() as connection:
                ...
                with connection.begin() as begin:
                    ...
                    try:
                        ...
                        result = self.execute_retry(sql, data, report, begin, _retry)
        ...
        ## When RConnection.
            ...
            try:
                ...
                result = self.execute_retry(sql, data, report, self.begin, _retry)
