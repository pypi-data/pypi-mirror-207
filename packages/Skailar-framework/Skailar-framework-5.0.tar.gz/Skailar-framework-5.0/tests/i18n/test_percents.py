...
from skailar.utils.translation import activate, get_language, trans_real
...
class FrenchTestCase(SimpleTestCase):
    ...
    PO_FILE = os.path.join(SAMPLEPROJECT_LOCALE, "fr", "LC_MESSAGES", "skailar.po")
...
class RenderingTemplatesWithPercentSigns(FrenchTestCase):
    ...
    def test_translates_multiple_percent_signs(self):
        ...
        self.assertEqual(
            block_tpl.render(Context({"name": "Skailar"})),
            "Skailar dit: 1 pour cent signe %, deux signes de pourcentage %%",
        )
