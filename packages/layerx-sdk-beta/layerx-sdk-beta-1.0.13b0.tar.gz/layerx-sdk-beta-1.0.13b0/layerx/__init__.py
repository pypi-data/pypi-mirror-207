...
class LayerxClient:
    ...
    def __init__(self, api_key:str, secret: str, layerx_url: str) -> None:
        ...
        self.__check_sdk_version_compatiblity()

    def __check_sdk_version_compatiblity(self):
        """
        ...
        subProcessForProduction = subprocess.getstatusoutput('pip show layerx-sdk')
        subProcessForDevelopment = subprocess.getstatusoutput('pip show layerx-sdk-beta')

        if(subProcessForProduction[0] == 0):
            version_id = subProcessForProduction[1].split("\n")[1].split(":")[1].strip()
        elif(subProcessForDevelopment[0] == 0):
            version_id = subProcessForDevelopment[1].split("\n")[1].split(":")[1].strip().split("b")[0]
        else:
            raise Exception("SDK version not found")

        _datalake_client.check_sdk_version_compatiblity(version_id)
    ...
    def trash_object_from_datalake(
        self, 
        datalake_query: str, 
        datalake_filter = {}, 
        ...
        try:
            ...
            response = self.__trash_object("", datalake_query, datalake_filter, object_type, [])
            return response
    ...
    def trash_object_from_collection(
        self, 
        collection_id: str, 
        query: str = "", 
        filter = {}, 
        ...
        try:
            ...
            response = self.__trash_object("", query, filter, object_type, [collection_id], False)
            return response
        except Exception as e:
            print("An exception occurred", format(e))
            return {
