...
class TestClient(unittest.TestCase):
    ...
    def test_register_mlflow_model(self):
        ...
        udw_resp = responses.Response(
            method=responses.POST,
            url=f"{self.test_client.api_endpoint}/v1/graphql",
            status=200,
            match=[testutil.query_name_matcher("UserDefaultWorkspace")],
            json={
                "data": {
        ...
        configure_resp = responses.Response(
            method=responses.POST,
            url=f"{self.test_client.api_endpoint}/v1/graphql",
            status=200,
            ...
            json={
                "data": {
                    "insert_model_config": {
                        "returning": [{"id": 1, "runtime": "mlflow"}]
                    }
                }
            },
        )
        responses.add(configure_resp)
        ...
        self.assertTrue(
            b'Content-Disposition: form-data; name="image_path"\r\n\r\nmy-image'
