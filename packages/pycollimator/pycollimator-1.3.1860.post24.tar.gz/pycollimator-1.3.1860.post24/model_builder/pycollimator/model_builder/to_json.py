...
def render_model(model, uuids=None, uiprops=None, is_submodel=False):
    ...
    for node, group in model.groups.items():
        diagrams[group.uuid] = render_diagram(group, uiprops)
        references[node.uuid] = {"diagram_uuid": group.uuid}

    parameters = None
    if is_submodel:
        parameters = [
            {"name": k, "default_value": v, "uuid": id._to_uuid(f"{model.uuid}-{k}")}  # TODO: do we need uuid?
    ...
    return {
        ...
        "reference_submodels": {
            submodel.uuid: render_model(submodel, uuids=uuids, uiprops=uiprops, is_submodel=True)
