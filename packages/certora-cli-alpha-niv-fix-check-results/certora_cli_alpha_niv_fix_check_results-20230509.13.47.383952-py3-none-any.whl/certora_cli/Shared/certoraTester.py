        ...
        for rule, ruleResult in rulesResults.items():
            if rule in expectedRulesResults.keys():
                if isinstance(ruleResult, str):
                    if isinstance(expectedRulesResults[rule], str): # and the rule is flat in the expected as well
                        ...
                        if not compare_flat_to_nested_rule(ruleResult, expectedRulesResults[rule]):
                            test = False
                            appendViolation(table, testName, rulesResults[rule], expectedRulesResults[rule], rule, "")
                
                else:  # nested rule ( ruleName: {result1: [funcionts list], result2: [funcionts list] ... } )
                    if isinstance(expectedRulesResults[rule], str): # but the rule is not nested in the expected
                        if not compare_flat_to_nested_rule(expectedRulesResults[rule], ruleResult):
                            test = False
                            appendViolation(table, testName, rulesResults[rule], expectedRulesResults[rule], rule, "")
