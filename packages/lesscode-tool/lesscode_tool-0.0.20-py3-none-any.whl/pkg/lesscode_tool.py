...
def new(dir, project):
    ...
    click.echo(f'search user:{dir},{project}')
...
@click.option('-a', '--add', type=bool, default=True, show_default=True, help='生成model是否带库名')
...
def sqlacodegen(url, schemas, tables, out, add, ignore):
    """生成SQLALCHEMY模型类"""
    click.echo(f"{url}, {schemas}, {tables}, {out}, {add}, {ignore}")
    if schemas:
