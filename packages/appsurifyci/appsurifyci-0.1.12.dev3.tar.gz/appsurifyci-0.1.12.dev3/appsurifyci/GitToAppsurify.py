...
def get_project_id(base_url, project_name, token):
    ...
    session.mount('http://', HTTPAdapter(max_retries=3))
    session.mount('https://', HTTPAdapter(max_retries=3))
    logging.info('Set execution type3a')
    try:
    ...
    logging.info('Set execution type3b')
    if resp.status_code == 200:
        return resp.json()
    if resp.status_code == 401:
...
logging.info('Set execution type')
...
logging.info('Set execution type2')
...
logging.info('Set execution type3')
...
logging.info('Set execution type4')
...
if 'project_id' in project_id_data:
    ...
    sys.exit(1)
logging.info('Set execution type5')
