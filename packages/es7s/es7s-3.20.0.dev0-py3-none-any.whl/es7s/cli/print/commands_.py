...
class PrintCommandsCommand:
    ...
    def __init__(self, ctx: click.Context, **kwargs):
        ...
        self.DEFAULT_CTYPE_CHAR = get_stdout().render("·", pt.Style(fg=pt.cv.BLUE, dim=True))
        self._run(ctx)
    ...
    def _format_group(self, cmd: CliBaseCommand, stack: list[str]) -> tuple[str, str] | None:
        return self._format_entry(cmd, stack, pt.Style(HelpStyles.TEXT_HEADING, bold=True))
