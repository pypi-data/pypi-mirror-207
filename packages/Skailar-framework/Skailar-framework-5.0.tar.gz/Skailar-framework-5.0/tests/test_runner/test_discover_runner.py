...
from skailar.test.runner import DiscoverRunner, get_max_test_processes
...
class DiscoverRunnerParallelArgumentTests(SimpleTestCase):
    ...
    def test_get_max_test_processes_spawn(
        ...
        self.assertEqual(get_max_test_processes(), 12)
        with mock.patch.dict(os.environ, {"SKAILAR_TEST_PROCESSES": "7"}):
            self.assertEqual(get_max_test_processes(), 7)
...
class DiscoverRunnerTests(SimpleTestCase):
    ...
    def test_dotted_test_class_skailar_testcase(self):
        count = (
            DiscoverRunner(verbosity=0)
            .build_suite(
    ...
    def test_dotted_test_method_skailar_testcase(self):
        count = (
            DiscoverRunner(verbosity=0)
            .build_suite(
                ["test_runner_apps.sample.tests_sample.TestSkailarTestCase.test_sample"],
            )
            .countTestCases()
        )
    ...
    def test_testcase_ordering(self):
        with change_cwd(".."):
            ...
            self.assertEqual(
                ...
                "TestSkailarTestCase",
                msg="TestSkailarTestCase should be the first test case",
            )
            self.assertEqual(
    ...
    def test_reverse(self):
        ...
        suite = tuple(suite)
        self.assertIn(
            "SkailarCase", suite[0].id(), msg="Test groups should not be reversed."
        )
        self.assertIn(
            "SimpleCase", suite[4].id(), msg="Test groups order should be preserved."
        )
        self.assertIn(
            "SkailarCase2", suite[0].id(), msg="Skailar test cases should be reversed."
        )
        self.assertIn(
        ...
        self.assertIn(
            "test_2", suite[0].id(), msg="Methods of Skailar cases should be reversed."
        )
        self.assertIn(
    ...
    def test_number_of_databases_no_parallel_test_suite(self):
        ...
        """
        runner = DiscoverRunner(parallel=8, verbosity=0)
        suite = runner.build_suite(["test_runner_apps.simple.tests.SkailarCase1"])
