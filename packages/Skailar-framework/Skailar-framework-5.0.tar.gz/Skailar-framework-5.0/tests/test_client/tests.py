...
from skailar.http import HttpResponse, HttpResponseNotAllowed
...
from skailar.urls import reverse_lazy
from skailar.utils.decorators import async_only_middleware
...
class ClientTest(TestCase):
    ...
    def test_response_headers(self):
        ...
        self.assertEqual(response.headers["X-SKAILAR-TEST"], "Slartibartfast")
    ...
    @override_settings(
        INSTALLED_APPS=["skailar.contrib.auth"],
    ...
    def test_view_with_inactive_force_login(self):
        ...
        self.client.force_login(
            self.u2, backend="skailar.contrib.auth.backends.AllowAllUsersModelBackend"
        )
    ...
    def test_force_login_without_backend(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["user"].username, "testclient")
        self.assertEqual(self.u1.backend, "skailar.contrib.auth.backends.ModelBackend")
    ...
    @override_settings(SESSION_ENGINE="skailar.contrib.sessions.backends.signed_cookies")
    ...
    def test_external_redirect(self):
        response = self.client.get("/skailar_project_redirect/")
        self.assertRedirects(
            response, "https://www.skailarproject.com/", fetch_redirect_response=False
        )
    ...
    def test_external_redirect_with_fetch_error_msg(self):
        ...
        msg = (
            ...
            "https://www.skailarproject.com/). If the host is served by Skailar, "
            "add 'www.skailarproject.com' to ALLOWED_HOSTS. "
            "Otherwise, use assertRedirects(..., fetch_redirect_response=False)."
        )
        with self.assertRaisesMessage(ValueError, msg):
            self.assertRedirects(response, "https://www.skailarproject.com/")
    ...
    @override_settings(
        INSTALLED_APPS=[],
        SESSION_ENGINE="skailar.contrib.sessions.backends.nonexistent",
    )
...
@override_settings(
    MIDDLEWARE=["skailar.middleware.csrf.CsrfViewMiddleware"],
