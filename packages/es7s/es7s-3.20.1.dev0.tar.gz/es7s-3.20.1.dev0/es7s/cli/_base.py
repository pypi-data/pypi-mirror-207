...
class HelpStyles(Styles):
    TEXT_HEADING = pt.Style(fg=pt.cv.YELLOW, bold=True, frozen=True)       # SYNOPSIS      |   | explicit manual usage only      # noqa
    TEXT_COMMAND_NAME = pt.Style(fg=pt.cv.BLUE, frozen=True)               # es7s exec     |   | auto-detection                  # noqa
    TEXT_OPTION_DEFAULT = pt.Style(fg=pt.cv.HI_YELLOW, frozen=True)        # [default: 1]  |[ ]| requires wrapping in [ ]        # noqa
    TEXT_LITERAL = pt.Style(bold=True, frozen=True)                        # 'ls | cat'    | ' | non-es7s commands (punctuation) # noqa
    TEXT_LITERAL_WORDS = pt.Style(underlined=True, frozen=True)            # 'ls | cat'    | ' | non-es7s commands (words)       # noqa
    TEXT_EXAMPLE = pt.Style(fg=pt.cv.CYAN, bg='full-black', frozen=True)   # ` 4 11`       | ` | output example                  # noqa
    TEXT_ENV_VAR = pt.Style(fg=pt.cv.GREEN, bold=True, frozen=True)        # {ES7S_LE_VAR} |{ }| environment variable name       # noqa
    TEXT_ABOMINATION = pt.Style(bg=pt.cv.BLUE).autopick_fg()               # ::NOTE::      |:: | needs to be wrapped in ::       # noqa
    TEXT_INLINE_CONFIG_VAR = pt.Style(fg=pt.cv.MAGENTA, frozen=True)       # <section.opt> |< >| config variable name            # noqa
    TEXT_ALTER_MODE = pt.Style(italic=True, frozen=True)                   # ^ALT MODE^    |^ ^| alt monitor mode                # noqa
    TEXT_COMMENT = pt.Style(fg=pt.cv.GRAY, frozen=True)                    # // COMMENT    |// | till end of the line            # noqa
    TEXT_ACCENT = pt.Style(bold=True, frozen=True)                         # *important*   | * | needs to be wrapped in *        # noqa
