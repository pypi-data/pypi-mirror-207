...
class flags:
    ...
    class lc_hubs:
        n = 'Node RED servers (we connect to port +1)'
        d = '127.0.0.1:1880'
        t = list
...
log_kws = {'pid': os.getpid(), 'host': socket.gethostname()}
...
class ProcessRcv:
    ...
    def register(msg):
        ...
        if cli_flow and not cli_flow in (nr_flow, 'hub'):
            msg['type'] = 'flow_def'
        ...
        if tabs:
            ...
            def in_tab(node):
                if node.get('type') == 'tab' and not node['id'] in our_tab_ids:
                    return
                z = node.get('z')
            ...
            if ol != len(F):
                app.info(
                    'Filtered out operators', tabs_filter=tabs, had=ol, kept=len(F)
                )

        axp = build_pipelines(
            F, subscribe=True, dyn_snk_types=p['dyn_snk_types'], dyn_snks=p['dyn_snks'],
        )
...
def alive_hubs(): return list([h for h in hubs.keys() if hubs[h]['status'] == 'alive'])
...
def payload_log(logger, msg, direction):
    ...
    m['payload'] = pls[:maxlen] + ('...' if len(pls) > maxlen else '')
