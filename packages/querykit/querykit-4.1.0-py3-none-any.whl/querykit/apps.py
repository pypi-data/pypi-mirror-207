...
class QueryKitConfig(AppConfig):
    name = 'querykit'
    label = 'querykit'
    ...
    def import_models(self):
        ...
        register_variable_scope(self.label, query_index=0)
...
def get_app_label():
    return QueryKitConfig.label


def reverse_app_url(identifier):
    return reverse(f'{QueryKitConfig.label}:{identifier}')


def get_app_config():
    return apps.get_app_config(QueryKitConfig.label)
