...
class AggregationPlan(object):
    ...
    * intermediate_column_prefixes
    ...
    Attributes:
        ...
        continuous_partial_aggregation_transform: Maps the input column name to a list of output pyspark columns
    ...
    continuous_partial_aggregation_transform: Callable[[str], List[Column]]
...
def _make_first_and_last_n_continuous_partial() -> Callable[[str], List[Column]]:
    def _first_and_last_n_continuous_partial(col: str) -> List[Column]:
        return [F.array(F.col(col))]
...
def _make_stddev_var_continuous_partial() -> Callable[[str], List[Column]]:
    def _stddev_var_continuous_partial(col: str) -> List[Column]:
        return [
            F.pow(col, 2),
            F.lit(1).cast("long"),
            F.col(col),
...
def _make_approx_count_distinct_continuous_partial() -> Callable[[str], List[Column]]:
    def _approx_count_distinct_continuous_partial(col: str) -> List[Column]:
        return NotImplementedError("Continuous mode for approx_count_distinct is not yet implemented.")
...
AGGREGATION_PLANS = {
    ...
    afpb.AGGREGATION_FUNCTION_MEAN: AggregationPlan(
        ...
        continuous_partial_aggregation_transform=lambda col: [F.col(col).cast("double"), F.lit(1).cast("long")],
