...
def pic_grouping(df,pic_title,fre,pool_size,width=1000,height=1000):
    ...
    for i in range(len(draw_df)):
        ...
        for j in range(7):
            ret_list[j].append(tempdf.iloc[j]['zbnp'])

    draw_df = pd.merge(draw_df, df.groupby('ret_date').mean()[['zbnp']], how='left', right_index=True, left_on='ret_date')
...
def single_factor_ic(factor_df,factor_name,fre,pool_size,fund_flag=True,grouping_flag=True):
    ...
    if(fund_flag):
        ...
        for i in range(len(date_list)-1):
            ...
            if(grouping_flag):
                #divide the groups
                groupdf=grouping(groupdf,retdf,factor_name,['jjdm', 'zbnp'],factor_date,pool_size,ret_date)
...
if __name__ == '__main__':
    ...
    info_ratio=pd.read_sql("select * from factor_infor_ratio where zblb='2103'   ",con=localdb)
    ...
    raw_df['new_f']=preprocessing.scale(raw_df[['info_ratio','jjzzc']]).mean(axis=1)


    groupdf, factrodf = single_factor_ic(raw_df, factor_name='new_f', fre=fre, pool_size=pool_size,
                                             fund_flag=True)

    pic_grouping(groupdf, 'new_f', fre, pool_size, 1400, 700)
