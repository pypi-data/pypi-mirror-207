...
class Stage(metaclass=Singleton):
    ...
    verified: bool = False
    players: Sequence[tuple[str, Player]] = field(default_factory=list)
    ...
    with midi_output_device("fluidsynth") as midi_out:
        ...
        for notes in note_generator():
            ...
            while True:
                ...
                # 4 / 48 sec
                waitfor = 60 / stage.env.bpm * 4 / stage.env.step_size
                last_dur = time.time() - last_now
                dur = max(0, waitfor - last_dur)
                # print(f"{waitfor:.6f}, {last_dur:.6f}, {dur:.6f}")
                await async_sleep(dur)
                last_now = time.time()
...
async def watch_changes() -> None:
    ...
    with Inotify() as inotify:
        ...
        async for event in inotify:
            logger().debug("path: %s, mask: %s", event.path, event.mask)
            try:
                assert event.path
                if Path(stage.file_to_track).absolute() == event.path.absolute():
