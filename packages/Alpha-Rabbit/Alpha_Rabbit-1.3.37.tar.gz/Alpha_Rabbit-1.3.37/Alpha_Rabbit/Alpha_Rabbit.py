...
class multi_factor_test(object):
    ...
    def multif_classification(self,allfactors,judgefunc,Price,quantiles,days,savedir,index_price = None):
        '''
        输入:
        因子矩阵
        ...
        for fc in list(allfactors):
            ...
            qreturn,clean_factor = sst.factor_ret_test_sheet(index_price,fc,res_df,Price,quantiles,days,savedir[0])
        ...
        for nzfc in list(noise_factors_df):
            ...
            nz_qreturn,clean_factor = sst.factor_ret_test_sheet(index_price,nzfc,nz_res_df,Price,quantiles,days,savedir[1])
            # 因子判别
            if judgefunc(nz_qreturn,nzfc) == True:
                decorr_noise_factors.append(nzfc)
        print('噪声组内正交化后还是被判别成功：')
