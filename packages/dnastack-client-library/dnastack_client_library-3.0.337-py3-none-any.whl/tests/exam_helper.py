...
class BaseTestCase(TestCase):
    ...
    def authenticate_with_device_code_flow(cls, context_urls: List[str]):
        ...
        for context_url in context_urls:
            cmd = ['python', '-m', 'dnastack', 'use', context_url]
            handle_device_code_flow(cmd, cls._states['email'], cls._states['token'])
...
class BasePublisherTestCase(BaseTestCase):
    ...
    def get_context_manager(cls):
        return container.get(ContextManager)
    ...
    def get_factory(cls, registry_url_or_context_name: Optional[str] = None) -> EndpointRepository:
        ...
        return use(context_name,
                   no_auth=True,
                   existing_context_manager=cls.get_context_manager())
