...
def get_tasks():
    ...
    regex_ip_withFile = r"/whois"
    regex_ip_withFile_param = r"/whois (\w+)"
    ...
    if header.startswith('/help'):
    ...
    elif re.search(regex_ip, response['text']) or re.match(regex_ip_withFile, header):
        fName_sort = ''

        if re.search(regex_ip, response['text']):
            list_ip = re.findall(regex_ip, ' '.join(msgs))
        elif re.match(regex_ip_withFile, header):
            if re.match(regex_ip_withFile_param, header):
                fName_sort = re.findall(regex_ip_withFile_param, header)[0] + '_'
            files = response['files'][0]
            response = requests.request("GET", files, headers=headers)
            response.raise_for_status()
            response.encoding="UTF-8"
            list_ip = re.findall(regex_ip, response.text)
        list_ip = list(dict(Counter(list_ip)).keys())
        regex_private = '(^127\.)|(^10\.)|(^172\.1[6-9]\.)|(^172\.2[0-9]\.)|(^172\.3[0-1]\.)|(^192\.168\.)'
        list_ip = [ x for x in list_ip if len(re.findall(regex_private, x)) == 0]
        ...
        for idx in range(len(list_ip)):
            ...
            if idx + 1 > progress * (totalLen/10) and progress < 9 and totalLen > 20:
                now = datetime.datetime.now()
                now = now + datetime.timedelta(days=0, seconds=0, microseconds=0, milliseconds=0, minutes=0, hours=9, weeks=0)
                ModifyMessage(payload, '[*] 진행률 : {}% [ {} / {} ] ; {} seconds - "{}"\n'.format(round(idx/totalLen, 2)*100, idx+1, totalLen, round(time.time() - time_start, 2), now.strftime('%Y-%m-%d %H:%M:%S')) + '▶ ' * progress * extend + '▷ ' * (10 - progress) * extend, messageId)
                progress += 1
            result = GetInfo(ip, idx)
            #time.sleep(0.2)
            if not result['state']:
        ...
        if totalLen > 20:
        	ModifyMessage(payload, '[*] 진행률 : 100% [ {} / {} ] ; {} seconds - "{}"\n'.format(totalLen, totalLen, round(time.time() - time_start, 2), now.strftime('%Y-%m-%d %H:%M:%S')) + '▶ ' * 10* extend, messageId)
        ...
        if len(outputs_trusted) + len(outputs_censored) > 10 :
            with open(f'{fName_sort}list_whois.txt', 'w') as f:
                f.write(outputs)
            SendFile(f'{fName_sort}list_whois.txt', roomId, '')
        else:
        ...
        if len(outputs_ip) > 10:
            with open(f'{fName_sort}list_ip.txt', 'w') as f:
                f.write(msg.replace('\x0a', '\x0d\x0a'))
            SendFile(f'{fName_sort}list_ip.txt', roomId, '')
        else:
        ...
        if len(opt_format) > 0 :
            fullPath = f'{fPath_working}/{fName_sort}result.csv'
