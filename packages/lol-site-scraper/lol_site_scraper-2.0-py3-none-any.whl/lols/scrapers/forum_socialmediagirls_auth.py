...
class ForumSMGAuth:
    def authorize(self):
        ...
        # With existing file
            ...
            # Load up existing cookies into current session
                ...
                logging.debug(f"Using saved auth cookies for {self.DOMAIN}: {cookies}")
                self._http.update_cookies(
                    cookies=cookies,
                    domain=self.DOMAIN
                )

    def _new_login(self):
        self._index_page()  # csrf cookie
        pre_login_token = self._pre_login_page()  # _XfToken
        xf_token, session_id, user_id = self._login_page(pre_login_token)  # Login session cookies
    ...
    def _login_page(self, pre_login_token) -> (XfToken, XfSession, XfUser):
        ...
        cookies = dict(self._http._session.cookies)
