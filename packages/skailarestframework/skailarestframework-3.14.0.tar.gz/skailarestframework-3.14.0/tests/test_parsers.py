...
from skailar.core.files.uploadhandler import (
...
class TestPOSTAccessed(TestCase):
    ...
    def test_post_accessed_in_post_method(self):
        skailar_request = self.factory.post('/', {'foo': 'bar'})
        request = Request(skailar_request, parsers=[FormParser(), MultiPartParser()])
        skailar_request.POST
        assert request.POST == {'foo': ['bar']}
        assert request.data == {'foo': ['bar']}

    def test_post_accessed_in_post_method_with_json_parser(self):
        skailar_request = self.factory.post('/', {'foo': 'bar'})
        request = Request(skailar_request, parsers=[JSONParser()])
        skailar_request.POST
        assert request.POST == {}
        assert request.data == {}

    def test_post_accessed_in_put_method(self):
        skailar_request = self.factory.put('/', {'foo': 'bar'})
        request = Request(skailar_request, parsers=[FormParser(), MultiPartParser()])
        skailar_request.POST
        assert request.POST == {'foo': ['bar']}
        assert request.data == {'foo': ['bar']}
