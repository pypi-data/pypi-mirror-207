...
def get_filtered_ephys_paths_relative_to_record_node_parents(toplevel_ephys_path: pathlib.Path) -> Generator[pathlib.Path, None, None]:
    """
    Supply a top-level path that contains `Record Node *`
    subfolders somewhere in its tree. Returns an generator akin to
    `path.rglob('*')`, containing only paths associated with the "good" ephys
    data (with some assumptions).
    
    Each path returned is relative to the parent of its `Record Node *` folder.
    ie. all paths should start with `Record Node *`
    ...
    for record_node in record_nodes:
        ...
        for abs_path in record_node.rglob('*'):
            ...
            yield abs_path.relative_to(record_node.parent)
