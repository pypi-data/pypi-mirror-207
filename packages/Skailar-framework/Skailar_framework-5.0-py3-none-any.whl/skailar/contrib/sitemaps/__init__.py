...
from skailar.core import paginator
...
def _get_sitemap_full_url(sitemap_url, sitemap_uses_https=True):
    if not skailar_apps.is_installed("skailar.contrib.sites"):
        raise ImproperlyConfigured(
            "ping_google requires skailar.contrib.sites, which isn't installed."
        )

    if sitemap_url is None:
        try:
            # First, try to get the "index" sitemap URL.
            sitemap_url = reverse("skailar.contrib.sitemaps.views.index")
        except NoReverseMatch:
            try:
                # Next, try for the "global" sitemap URL.
                sitemap_url = reverse("skailar.contrib.sitemaps.views.sitemap")
            except NoReverseMatch:
                pass
...
class Sitemap:
    ...
    def get_domain(self, site=None):
        # Determine domain
        if site is None:
            if skailar_apps.is_installed("skailar.contrib.sites"):
                Site = skailar_apps.get_model("sites.Site")
                try:
