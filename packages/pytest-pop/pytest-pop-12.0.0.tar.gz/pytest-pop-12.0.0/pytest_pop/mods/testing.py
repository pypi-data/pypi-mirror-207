...
class _LazyPop:
    ...
    def __setattr__(self, key, value):
        ...
        if value.__class__ in self.__lazy_classes and "__obj" not in key:
            ...
            super().__setattr__(key, self._mix_hubs(value))
            return
    ...
    def _mix_hubs(self, obj):
        new_obj = _LazyPop(obj, lut=self.__lut)

        if isinstance(obj, pop.loader.LoadedMod):
            for name in obj._funcs:
                new_obj.__setattr__(name, obj._funcs[name])
            for name in obj._attrs:
                new_obj.__setattr__(name, obj._attrs[name])
            for name in obj._classes:
                new_obj.__setattr__(name, obj._classes[name])
            for name in obj._vars:
                new_obj.__setattr__(name, obj._vars[name])
        elif isinstance(obj, pop.hub.Sub):
            new_obj._subs = {}
            for name in obj._loaded:
                new_sub = new_obj._mix_hubs(obj._loaded[name])
                new_obj.__setattr__(name, new_sub)
        else:
            raise TypeError(
                f"Mixing of real and test hubs is not supported for {obj.__class__.__name__}. "
            )
        return new_obj
