...
class LineFitting:
    ...
    def integrated_properties(self, line, emis_wave, emis_flux, cont_wave, cont_flux, emis_err=None, n_steps=1000):
        ...
        # Compute the integrated singal to noise
        amp_ref = line.peak_flux - line.cont
        if line._emission_check:
            if amp_ref < 0:
                amp_ref = line.peak_flux

        line.snr_line = signal_to_noise_rola(amp_ref, line.std_cont, emis_wave.size)
    ...
    def profile_fitting(self, line, x, y, weights, z_obj=0, fit_method='leastsq', temp=10000.0, inst_FWHM=np.nan):
        ...
        line.snr_line = signal_to_noise_rola(line.amp, line.std_cont, 6 * line.sigma)
