...
import pyrosex
from pyrosex import __version__, __license__
from pyrosex import enums
from pyrosex import raw
from pyrosex import utils
from pyrosex.crypto import aes
from pyrosex.errors import CDNFileHashMismatch
from pyrosex.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest
)
from pyrosex.handlers.handler import Handler
from pyrosex.methods import Methods
from pyrosex.session import Auth, Session
from pyrosex.storage import FileStorage, MemoryStorage
from pyrosex.types import User, TermsOfService
from pyrosex.utils import ainput
...
class Client(Methods):
    """Pyrosex Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~pyrosex.Client.export_session_string` before stopping the client to get a session string you can
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where pyrosex will store the session files.
    ...
    APP_VERSION = f"Pyrosex {__version__}"
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to Pyrosex (version {__version__})")
        print(f"Pyrosex is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~pyrosex.enums.ParseMode`):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.pyrosex.org/start/auth")
    ...
    async def handle_download(self, packet):
        ...
        try:
            async for chunk in self.get_file(file_id, file_size, 0, 0, progress, progress_args):
                file.write(chunk)
        except pyrosex.StopTransmission:
            if not in_memory:
                file.close()
                os.remove(file.name)
