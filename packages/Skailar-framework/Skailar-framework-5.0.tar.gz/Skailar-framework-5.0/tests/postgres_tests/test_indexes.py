...
from skailar.contrib.postgres.indexes import (
    BloomIndex,
    BrinIndex,
    BTreeIndex,
    GinIndex,
...
from skailar.db import NotSupportedError, connection
from skailar.db.models import CharField, F, Index, Q
from skailar.db.models.functions import Cast, Collate, Length, Lower
from skailar.test import skipUnlessDBFeature
from skailar.test.utils import register_lookup
...
class IndexTestMixin:
    ...
    def test_deconstruction_no_customization(self):
        ...
        self.assertEqual(
            path, "skailar.contrib.postgres.indexes.%s" % self.index_class.__name__
        )
    ...
    def test_deconstruction_with_expressions_no_customization(self):
        ...
        self.assertEqual(
            path,
            f"skailar.contrib.postgres.indexes.{self.index_class.__name__}",
        )
...
class BloomIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "skailar.contrib.postgres.indexes.BloomIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class BrinIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "skailar.contrib.postgres.indexes.BrinIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class BTreeIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "skailar.contrib.postgres.indexes.BTreeIndex")
        self.assertEqual(args, ())
        self.assertEqual(
...
class GinIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "skailar.contrib.postgres.indexes.GinIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class GistIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "skailar.contrib.postgres.indexes.GistIndex")
        self.assertEqual(args, ())
        self.assertEqual(
            kwargs,
            {
...
class HashIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "skailar.contrib.postgres.indexes.HashIndex")
        self.assertEqual(args, ())
        self.assertEqual(
...
class SpGistIndexTests(IndexTestMixin, PostgreSQLSimpleTestCase):
    ...
    def test_deconstruction(self):
        ...
        path, args, kwargs = index.deconstruct()
        self.assertEqual(path, "skailar.contrib.postgres.indexes.SpGistIndex")
        self.assertEqual(args, ())
        self.assertEqual(
...
class SchemaTests(PostgreSQLTestCase):
    ...
    def test_spgist_include_not_supported(self):
        ...
        with self.assertRaisesMessage(NotSupportedError, msg):
            with mock.patch(
                "skailar.db.backends.postgresql.features.DatabaseFeatures."
