...
MODULE_COLOUR = 'blue'
ERROR_COLOUR = 'red'
INTEGER_SELECTION= 'integer_selection'
...
class Input():
    ...
    process: dict[str, Tuple[object, list]] or dict[str, Tuple[object, list, tuple]]
        ...
        An example process:
            ...
            processes = {
                'integer_selection': (function_one, [], (min_integer, max_integer))
                'S': (function_two, [files_to_copy]),
                'Q': (quit, []),
            }

        (If the process key = <INTEGER_SELECTION> then a valid input is created for integers in the
        range (min_integer to the max_integer) and the call is to function_one with the input parameter.)

    allow_null: bool
        If True will all an valid null (<RTN>) entry.
    ...
    def process_response(self) -> object:
        ...
        while True:
            ...
            if self.allow_null and not response:
                return response
            if response not in self.processes:
                print(self.allow_null, response)
                return response
    ...
    def _get_input(self) -> str:
        ...
        while True:
            ...
            if not response and self.allow_null:
                self.valid = True
                return response
    ...
    def _get_error_message(self, error_text: str) -> str:
        ...
        if self.include_valid_in_error:
            ...
            if self.allow_null:
                valid_responses_list.insert(0, '<RTN>')
