...
from skailar.db.models.signals import m2m_changed
    ...
    def test_regular_fields(self):
        ...
        expected = dedent("""
            TestSerializer():
                ...
                comma_separated_integer_field = CharField(max_length=100, validators=[<skailar.core.validators.RegexValidator object>])
...
class TestPosgresFieldsMapping(TestCase):
    ...
    def test_array_field(self):
        ...
        validators = ""
        if skailar.VERSION < (4, 1):
            validators = ", validators=[<skailar.core.validators.MaxLengthValidator object>]"
        expected = dedent("""
            TestSerializer():
    ...
    def test_json_field(self):
        class JSONFieldModel(models.Model):
            ...
            json_field_with_encoder = postgres_fields.JSONField(encoder=SkailarJSONEncoder)
        ...
        expected = dedent("""
            TestSerializer():
                ...
                json_field_with_encoder = JSONField(encoder=<class 'skailar.core.serializers.json.SkailarJSONEncoder'>, style={'base_template': 'textarea.html'})
        """)
        self.assertEqual(repr(TestSerializer()), expected)
...
class TestSkailarJSONFieldMapping(TestCase):
    def test_json_field(self):
        class JSONFieldModel(models.Model):
            json_field = models.JSONField()
            json_field_with_encoder = models.JSONField(encoder=SkailarJSONEncoder, decoder=CustomJSONDecoder)
        ...
        expected = dedent("""
            TestSerializer():
                ...
                json_field_with_encoder = JSONField(decoder=<class 'tests.test_model_serializer.CustomJSONDecoder'>, encoder=<class 'skailar.core.serializers.json.SkailarJSONEncoder'>, style={'base_template': 'textarea.html'})
        """)
        self.assertEqual(repr(TestSerializer()), expected)
