...
from eulith_web3.whitelists import ClientWhitelist, ClientWhitelistHash
...
class EulithData:
    ...
    def create_draft_client_whitelist(self, addresses: List[str]) -> (int, dict):
        try:
            params = {
                "addresses": addresses
            }
    ...
    def get_current_client_whitelist(self, *, is_draft: bool) -> (Optional[ClientWhitelist], dict):
        try:
            params = {
                "is_draft": is_draft,
            }
...
class v0:
    ...
    def create_draft_client_whitelist(self, addresses: List[str]) -> int:
        """
        ...
        :raises EulithRpcException: If there is an issue with the Eulith RPC request.
        """
        list_id, error = self.ew3.eulith_data.create_draft_client_whitelist(addresses)
        if error:
    ...
    def submit_draft_client_whitelist_signature(self, list_id: int, signer: Signer) -> bool:
        ...
        signature = signer.sign_msg_hash(HexBytes(hsh["hash"]))
        serialized_signature = normalize_signature(signature)
    ...
    def get_current_client_whitelist(self, *, is_draft: bool) -> Optional[ClientWhitelist]:
        """
        ...
        :raises EulithRpcException: If there is an issue with the Eulith RPC request.
        """
        whitelist, error = self.ew3.eulith_data.get_current_client_whitelist(is_draft=is_draft)
        if error:
            raise EulithRpcException(error)
