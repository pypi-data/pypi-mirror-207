...
from skailar.utils.numberformat import format as nformat
from skailar.utils.safestring import SafeString, mark_safe
from skailar.utils.translation import (
    activate,
    check_for_language,
    deactivate,
    get_language,
...
from skailar.utils.translation.reloader import (
...
def patch_formats(lang, **settings):
    from skailar.utils.formats import _format_cache
...
class TranslationTests(SimpleTestCase):
    ...
    def test_multiple_plurals_per_language(self):
        """
        Normally, French has 2 plurals. As other/locale/fr/LC_MESSAGES/skailar.po
...
class FormattingTests(SimpleTestCase):
    ...
    def test_sanitize_separators(self):
        """
        Tests skailar.utils.formats.sanitize_separators.
        """
    ...
    def test_iter_format_modules(self):
        ...
        default_mod = import_module("skailar.conf.locale.de.formats")
    ...
    def test_iter_format_modules_stability(self):
        ...
        """
        en_format_mod = import_module("skailar.conf.locale.en.formats")
        en_gb_format_mod = import_module("skailar.conf.locale.en_GB.formats")
        self.assertEqual(
...
class MiscTests(SimpleTestCase):
    ...
    def test_parse_literal_http_header(self):
        tests = [
            ...
            ("es,de", "es"),
            ("es-a,de", "es"),
            # There isn't a Skailar translation to a US variation of the Spanish
            # language, a safe assumption. When the user sets it as the
            # preferred language, the main 'es' translation should be selected
            # instead.
            ("es-us", "es"),
            # There isn't a main language (zh) translation of Skailar but there
    ...
    def test_support_for_deprecated_chinese_language_codes(self):
        ...
        language codes will be removed in Skailar 1.9, these will be incorrectly
    ...
    def test_parse_language_cookie(self):
        ...
        # There isn't a main language (zh) translation of Skailar but there is a
    ...
    def test_app_translation(self):
        ...
        with self.modify_settings(INSTALLED_APPS={"append": "i18n.resolution"}):
            ...
            with self.modify_settings(
                INSTALLED_APPS={"remove": "skailar.contrib.admin.apps.SimpleAdminConfig"}
            ):
                # Force refreshing translations.
                activate("de")
...
class SkailarFallbackResolutionOrderI18NTests(ResolutionOrderI18NTests):
    def test_skailar_fallback(self):
...
@override_settings(
    ...
    MIDDLEWARE=[
        "skailar.middleware.locale.LocaleMiddleware",
...
class CountrySpecificLanguageTests(SimpleTestCase):
    ...
    def test_check_for_language(self):
        ...
        # Specifying encoding is not supported (Skailar enforces UTF-8)
...
class NonSkailarLanguageTests(SimpleTestCase):
    """
    A language non present in default Skailar languages can still be
    installed/used by a Skailar project.
    """
    ...
    def test_non_skailar_language(self):
    ...
    def test_check_for_language(self):
        with tempfile.TemporaryDirectory() as app_dir:
            ...
            open(
                os.path.join(
                    app_dir, "locale", "dummy_Lang", "LC_MESSAGES", "skailar.mo"
                ),
                "w",
            ).close()
            app_config = AppConfig("dummy_app", AppModuleStub(__path__=[app_dir]))
            with mock.patch(
                "skailar.apps.apps.get_app_configs", return_value=[app_config]
            ):
    ...
    def test_plural_non_skailar_language(self):
...
class WatchForTranslationChangesTests(SimpleTestCase):
    ...
    def test_i18n_app_dirs_ignore_skailar_apps(self):
        mocked_sender = mock.MagicMock()
        with self.settings(INSTALLED_APPS=["skailar.contrib.admin"]):
            watch_for_translation_changes(mocked_sender)
