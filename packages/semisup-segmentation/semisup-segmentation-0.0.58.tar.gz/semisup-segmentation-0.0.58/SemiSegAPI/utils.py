...
availableModels=['CGNet', 'FPENet', 'DeepLab','PAN','OCNet','MAnet','LEDNet','DenseASPP','U-Net','UNet']
...
def getLearner(model,backbone,numClasses,path,dls):
    ...
    if model=='U-Net':
        modelo = timm.create_model(backbone)
        learn=unet_learner(dls,modelo(),metric=metric,cbs=[early, save], path=path)
    elif model=='DeepLab':
