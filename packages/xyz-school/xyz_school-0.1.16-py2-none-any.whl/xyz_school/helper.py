...
def update_class_grade():
    ...
    models.Class.objects.filter(
        entrance_session__end_date__lt=today
    ).exclude(
    ...
    models.Class.objects.filter(
        entrance_session__begin_date__gt=today
    ).exclude(
    ...
    models.Class.objects.filter(
        ...
        entrance_session__end_date__gt=today
    ).exclude(
...
def update_student_entrance_session():
    from django.db.models import Count, Min
    qset = models.Student.objects.all().annotate(
        a=Count('class', distinct=True),
        es=Min('class__entrance_session')
    ).filter(a__gt=1)
    sids = []
    for s in qset:
        id = s.id
        models.Student.objects.filter(id=id).update(entrance_session_id=s.es)
        sids.append(id)
    for s in models.Session.objects.all():
        cids = list(s.entrance_classes.all().values_list('id', flat=True))
        count = models.Student.objects.filter(
            class__in=cids
        ).exclude(
            entrance_session_id=s.id
        ).exclude(
            id__in=sids
        ).update(
            entrance_session_id=s.id
...
def update_student_stage():
    for s, sn in choices.CHOICES_STAGE:
        r = models.Student.objects.filter(
            class__stage=s
        ).exclude(
            stage=s
        ).update(stage=s)
...
def current_stage(begin_date, end_date):
    begin_date = dateutils.format_the_date(begin_date)
    end_date = dateutils.format_the_date(end_date)
    today = datetime.date.today()
    if begin_date > today:
        return choices.STAGE_PENDING
    elif end_date >= today:
        return choices.STAGE_STUDYING
    return choices.STAGE_GRADUATED
...
def bind_teacher(teacher, target_user):
    old_user = teacher.user
    from xyz_auth.signals import to_bind_user
    from django.db import transaction
    from xyz_common.signals import to_add_event
    with transaction.atomic():
        to_bind_user.send(teacher._meta.model, old_user=old_user, new_user=target_user)
        teacher.user = target_user
        teacher.save()
    to_add_event.send_robust(sender=teacher._meta.model, instance=teacher, name='bind')
