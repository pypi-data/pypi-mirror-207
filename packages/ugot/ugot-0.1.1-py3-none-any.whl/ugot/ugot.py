...
class UGOT:
    ...
    def transform_set_chassis_height(self, height: float):
        ...
        if not (isinstance(height, float) or isinstance(height, int)):
            logging.error('unsupported param type of height:----{}'.format(height))
            return
        height = height * 10  # 换算成mm
        height = int(num_normal(height, 72, 10))
    ...
    def transform_turn_speed(self, turn: int, speed: int):
        ...
        Args:
            ...
            speed (int): [0-280] 速度，单位 度/秒

        Returns:
            无

        """
        speed = num_normal(speed, 280, 0)
    ...
    def transform_move_turn(self, direction, speed, turn, turn_speed):
        ...
        Args:
            direction (int): 方向（0：前进；1：后退）
            speed (int): [0-80] 前进/后退速度，单位 厘米/秒
            turn (int): 方向（2：左转；3：右转）
            turn_speed (int): [0-280] 旋转速度，单位 度/秒
        ...
        tar_speed = 0
        turn_speed = num_normal(turn_speed, 280, 0)
        if turn == E_Model.Direction.turn_left:
    ...
    def balance_set_acceleration(self, acceleration):
        ...
        if not (isinstance(acceleration, float) or isinstance(acceleration, int)):
            logging.error('unsupported param type of acceleration:----{}'.format(acceleration))
            return
        if acceleration > 0:
    ...
    def mechanical_single_joint_control(self, joint, angle, duration):
        ...
        if valid_joint:
            ...
            logging.error('invalid joint id:{}'.format(joint))
    ...
    def mechanical_move_axis(self, x, y, z, duration):
        ...
        Args:
            x (float): x坐标 [-5, 25] 单位cm
            y (float): y坐标 [-25, 25] 单位cm
            z (float): z坐标 [-16, 16] 单位cm
            duration (int): 运行时长 [20, 5000] 单位：毫秒
        ...
        if not (isinstance(x, float) or isinstance(x, int)):
            logging.error('unsupported param type of x:----{}'.format(x))
            return
        x = int(x * 10)  # 换算成mm
        x = num_normal(x, 250, -50)

        if not (isinstance(y, float) or isinstance(y, int)):
            logging.error('unsupported param type of y:----{}'.format(y))
            return
        y = int(y * 10)  # 换算成mm
        y = num_normal(y, 250, -250)

        if not (isinstance(z, float) or isinstance(z, int)):
            logging.error('unsupported param type of z:----{}'.format(z))
            return
        z = int(z * 10)  # 换算成mm
    ...
    def set_track_recognition_line(self, line_type):
        ...
        Returns:
            无
        """
        logging.debug("set_track_recognition_line ")
