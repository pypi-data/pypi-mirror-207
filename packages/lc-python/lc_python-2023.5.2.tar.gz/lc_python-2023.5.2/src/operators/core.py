...
from operators.ops.tools import NamedSubject, Rx, rx
...
class ax_core_ops:
    ...
    def from_file(data, filename, op, ts=None, counter=None, _c={}, **kw):
        """this is mainly for load injection. counter and ts maybe inserted at arbitrary
        top level positions"""
        try:
            c = _c[op['id']]   # we can have more -> cache per rater
        except KeyError:
            from devapp.tools import project, read_file
            import os
            import json

            fn = os.path.abspath(os.path.join(project.root(), filename))
            if not os.path.exists(fn):
                app.die('File not found', filename=fn)
            s = json.loads(read_file(fn))
            _c[op['id']] = c = [fn, s, 0]
    ...
    def rate(data, msg, op, dt=1, on_stdout=False, _c={}):
        n = time.time()
        try:
            r = _c[op['id']]   # we can have more -> cache per rater
        except KeyError:
            _c[op['id']] = r = [op.get('label', op['id']), n, 0]
        r[2] += 1
        if n - r[1] > dt:
            R = r[2] / (n - r[1])
            msg[f'rate_{r[0]}'] = R
    ...
    def count(data, msg, op, _c={}, now=time.time):
        try:
            r = _c[op['id']]   # we can have more -> cache per counter
        except KeyError:
            n = op.get('label') or op.get('name')
            _c[op['id']] = r = make_counters(n, op, dt_unit='millis')
        try:
    ...
    def brk(data, msg):
        ...
        if sys.stdin.isatty() and sys.stdout.isatty():
            ...
            app.warn('Inspect than continue with c')
            breakpoint()  # FIXME BREAKPOINT
        else:
            app.info('ignoring inspect_interactive - no tty')
        return data

    def dump(data, msg, level=20, truncate=None, full_msg=False):
        r = msg if full_msg else data
        with AppLogLevel(level):
            if truncate:
                d = str(r)
                app.info('Data dump', data=d[:truncate] + '...', len=len(r))
            else:
                app.info('Data dump', payload=r)
        return data
    ...
    def hello(data, msg, name='world', tag=None, count_start=0):
        """test operator"""
        ...
        app.info('ax.hello - logging msg', payload=msg)
        return data
...
class snk:
    ...
    file_out = file.snk  # sideeffect, but this one we want always, w/o add_connection
