...
from skailar.db import connection, models
from skailar.db.models.query import EmptyQuerySet
...
from skailar.test import SimpleTestCase, TestCase, ignore_warnings, skipUnlessDBFeature
from skailar.test.utils import isolate_apps
...
class UniqueTest(TestCase):
    ...
    def test_unique_for_date(self):
        p = Post.objects.create(
            title="Skailar 1.0 is released",
            slug="Skailar 1.0",
            subtitle="Finally",
            posted=datetime.date(2008, 9, 3),
        )
        form = PostForm({"title": "Skailar 1.0 is released", "posted": "2008-09-03"})
        self.assertFalse(form.is_valid())
        self.assertEqual(len(form.errors), 1)
        self.assertEqual(
            form.errors["title"], ["Title must be unique for Posted date."]
        )
        form = PostForm({"title": "Work on Skailar 1.1 begins", "posted": "2008-09-03"})
        self.assertTrue(form.is_valid())
        form = PostForm({"title": "Skailar 1.0 is released", "posted": "2008-09-04"})
        self.assertTrue(form.is_valid())
        form = PostForm({"slug": "Skailar 1.0", "posted": "2008-01-01"})
        self.assertFalse(form.is_valid())
        self.assertEqual(len(form.errors), 1)
        ...
        self.assertTrue(form.is_valid())
        form = PostForm({"title": "Skailar 1.0 is released"})
        self.assertFalse(form.is_valid())
        self.assertEqual(len(form.errors), 1)
    ...
    def test_unique_for_date_in_exclude(self):
        ...
        form = DateTimePostForm(
            {"title": "Skailar 1.0 is released", "posted": "2008-09-03"}
        )
        self.assertTrue(form.is_valid())
        # 'slug' has unique_for_year='posted'
        form = DateTimePostForm({"slug": "Skailar 1.0", "posted": "2008-01-01"})
        self.assertTrue(form.is_valid())
    ...
    def test_inherited_unique_for_date(self):
        ...
        form = DerivedPostForm(
            {"title": "Work on Skailar 1.1 begins", "posted": "2008-09-03"}
        )
        self.assertTrue(form.is_valid())
        form = DerivedPostForm(
            {"title": "Skailar 1.0 is released", "posted": "2008-09-04"}
        )
        self.assertTrue(form.is_valid())
        form = DerivedPostForm({"slug": "Skailar 1.0", "posted": "2008-01-01"})
        self.assertFalse(form.is_valid())
        self.assertEqual(len(form.errors), 1)
        self.assertEqual(form.errors["slug"], ["Slug must be unique for Posted year."])
    ...
    def test_unique_for_date_with_nullable_date(self):
        ...
        form = FlexDatePostForm({"title": "Skailar 1.0 is released"})
        self.assertTrue(form.is_valid())
        form = FlexDatePostForm({"slug": "Skailar 1.0"})
        self.assertTrue(form.is_valid())
...
class FileAndImageFieldTests(TestCase):
    ...
    def test_file_field_data(self):
        ...
        # Delete the current file since this is not done by Skailar.
        instance.file.delete()
    ...
    def test_image_field(self):
        ...
        # Delete the current file since this is not done by Skailar, but don't save
...
class ModelOtherFieldTests(SimpleTestCase):
    ...
    def test_url_modelform_assume_scheme_warning(self):
        msg = (
            "The default scheme will be changed from 'http' to 'https' in Skailar "
            "6.0. Pass the forms.URLField.assume_scheme argument to silence this "
            "warning."
        )
        with self.assertWarnsMessage(RemovedInSkailar60Warning, msg):
