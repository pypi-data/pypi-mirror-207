...
from typing import Any, Dict, List, Tuple, Optional, Set, BinaryIO, Iterator, Callable, Iterable
...
class CertoraBuildGenerator:
    ...
    def build(self, context: CertoraContext) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            self.safe_copy_folder(sources_dir, pre_backup_dir, ignore_patterns, dirs_exist_ok=False)
            ...
            if compiler_lang == CompilerLangSol():
                ...
                self.safe_copy_folder(get_certora_sources_dir(), post_backup_dir, ignore_patterns, dirs_exist_ok=True)
    ...
    @staticmethod
    def safe_copy_folder(source: Path, dest: Path, ignore_patterns: Callable[[str, list[str]], Iterable[str]],
                         dirs_exist_ok: bool):
        """
        Safely copy source to dest. On certain OS/kernels/FS, copying a folder f into a subdirectory of f will
        ...
        shutil.copytree(source, copy_temp, ignore=ignore_patterns, dirs_exist_ok=dirs_exist_ok)
