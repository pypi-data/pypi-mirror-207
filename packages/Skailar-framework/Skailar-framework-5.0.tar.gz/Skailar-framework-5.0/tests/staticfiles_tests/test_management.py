...
from skailar.contrib.staticfiles import storage
from skailar.contrib.staticfiles.management.commands import collectstatic, runserver
...
class TestRunserver(StaticFilesTestCase):
    @override_settings(MIDDLEWARE=["skailar.middleware.common.CommonMiddleware"])
    def test_middleware_loaded_only_once(self):
        command = runserver.Command()
        with mock.patch("skailar.middleware.common.CommonMiddleware") as mocked:
            ...
            with self.settings(
                STORAGES={
                    STATICFILES_STORAGE_ALIAS: {
                        "BACKEND": (
                            "skailar.contrib.staticfiles.storage.StaticFilesStorage"
                        )
                    }
                }
            ):
...
class TestCollectionHelpSubcommand(AdminScriptTestCase):
    ...
    def test_missing_settings_dont_prevent_help(self):
        ...
        """
        self.write_settings("settings.py", apps=["skailar.contrib.staticfiles"])
...
@override_settings(
    STORAGES={
        STATICFILES_STORAGE_ALIAS: {
            "BACKEND": "skailar.contrib.staticfiles.storage.ManifestStaticFilesStorage"
        },
    }
)
