...
class ApplyResearchIntelligence(DataAPIClient):
    ...
    def exchange_volume_usd(self):
        ...
        instruments = instruments.loc[
            instruments["exchange_code"].str.contains("|".join(self.params["exchange"]))
        ]
        ...
        volume_data = self.fetch_data_batches("Count-OHLCV-VWAP", ohlcvs_params)
        ...
        asset_price_data = self.fetch_data_batches("Cross Price", asset_price_params)
        asset_price_data = asset_price_data.rename(columns={"price": "usd_price"})
