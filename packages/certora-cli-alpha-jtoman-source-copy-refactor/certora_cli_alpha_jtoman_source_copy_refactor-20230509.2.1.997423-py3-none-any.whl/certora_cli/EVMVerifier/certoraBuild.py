...
class CertoraBuildGenerator:
    ...
    def build(self, context: CertoraContext) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            self.safe_copy_folder(ignore_patterns, pre_backup_dir, sources_dir)
            ...
            if compiler_lang == CompilerLangSol():
                ...
                self.safe_copy_folder(ignore_patterns, post_backup_dir, get_certora_sources_dir())
    ...
    @staticmethod
    def safe_copy_folder(ignore_patterns, source: Path, dest: Path):
        """
        ...
        shutil.copytree(dest, copy_temp, ignore=ignore_patterns, dirs_exist_ok=True)
        shutil.copytree(copy_temp, source)
