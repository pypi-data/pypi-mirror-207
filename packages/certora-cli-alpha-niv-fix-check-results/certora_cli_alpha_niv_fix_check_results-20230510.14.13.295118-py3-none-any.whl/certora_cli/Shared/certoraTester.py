...
def compareResultsWithExpected(
    ...
    if rules_results != expected_rules_results:
        ...
        compare_results(rules_results, expected_rules_results, violations, test_name)
        # check for rules that were expected but didn't got results
        find_not_existing_rules(expected_rules_results, rules_results, violations, test_name)
        
    # if assertMessages field is defined (in tester)
    if expected_assertion_messages:
        test = compare_assert_messages(test_name, expected_assertion_messages, assert_messages)
...
def find_not_existing_rules(expected_rules_results: Dict[str, Any], rules_results: Dict[str, Any], violations: Set[Tuple], test_name: str):
    rules_not_found = set(expected_rules_results) - set(rules_results)
    for r in rules_not_found:
        res = expected_rules_results[r]
        violations.add((test_name, "Rule not found in results.", 
                        res if isinstance(res, str) else extract_nested_rules_status(res), r))


def compare_results(results: Dict[str,Any], expected: Dict[str,Any], violations: Set[Tuple], test_name: str):
                ...
                if isinstance(rule_result, str):
                    if isinstance(expected_rule_result, str): # and the rule is flat in the expected as well
                        if rule_result != expected_rule_result:
                            ...
                            violations.add((test_name, rule_result, expected_rule_result, rule, ""))
                    else: # the rule is nested in the expected
                        nested_rule_res = extract_nested_rules_status(expected_rule_result) 
                        if rule_result != nested_rule_res:
                            # errors = addError(errors, testName, rule, ruleResult, expectedRuleResult)
                            violations.add((test_name, rule_result, nested_rule_res, rule, ""))
                    ...
                    if isinstance(expected_rule_result, str): # but the rule is not nested in the expected
                        ...
                        if nested_rule_res != expected_rule_result:
                            # errors = addError(errors, testName, rule, ruleResult, expectedRuleResult)
                            violations.add((test_name, nested_rule_res, expected_rule_result, rule, ""))

                    else:  # both rules are nested
                        for result, func_list in rule_result.items():
                            func_list.sort()
                            expected_rule_result[result].sort()

                            # compare functions lists (current results with expected)
                            if func_list != expected_rule_result[result]:
                                for func_name in func_list:
                                    # if function appears in current results but does not appear in the expected ones
                                    if func_name not in expected_rule_result[result]:
                                        # errors = addError(errors, testName, rule, result, "", funcName)
                                        # found results for an unexpected rule
                                        expected_result = findExpected(func_name, expected_rule_result)
                                        violations.add((test_name, result, expected_result, rule, func_name))
            else:
                result = (rule_result
                          if isinstance(rule_result, str)
                          else "Object{" + ", ".join(rule_result.keys()) + "}")
                violations.add((test_name, result, "\033[33mundefined\033[0m", rule, ""))
                # errors += testName + ", " + rule + " is not listed in 'rules'. Expected rules: " + \
                # ','.join(expectedRulesResults.keys()) + "\n"


def compare_assert_messages(test_name: str, expected_assertion_messages: Dict[str,Any], assert_messages: Dict[str,Any]) -> bool:
        test = True
        for rule in expected_assertion_messages.keys():
            if rule not in assert_messages:  # current rule is missing from 'assertMessages' section in current results
                ...
                test = False
                errors += test_name + ", rule \"" + rule + "\": wrong assertion message. Got: \"" + \
                    assert_messages[rule] + "\". Expected: \"" + expected_assertion_messages[rule] + "\".\n"
        return test
