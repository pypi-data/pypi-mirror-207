...
import aikoai
from aikoai.upload_progress import BufferReader
from aikoai.validators import (
...
class Engine:
    @classmethod
    def get(cls, args):
        engine = aikoai.Engine.retrieve(id=args.id)
        display(engine)

    @classmethod
    def update(cls, args):
        engine = aikoai.Engine.modify(args.id, replicas=args.replicas)
        display(engine)
...
class ChatCompletion:
    ...
    def create(cls, args):
        ...
        resp = aikoai.ChatCompletion.create(
            # Required
            model=args.model,
            engine=args.engine,
            messages=messages,
...
class Deployment:
    @classmethod
    def get(cls, args):
        resp = aikoai.Deployment.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = aikoai.Deployment.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = aikoai.Deployment.list()
        print(models)

    @classmethod
    def create(cls, args):
        models = aikoai.Deployment.create(model=args.model, scale_settings={"scale_type": args.scale_type})
        print(models)
...
class Model:
    @classmethod
    def get(cls, args):
        resp = aikoai.Model.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = aikoai.Model.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = aikoai.Model.list()
        print(models)
...
class File:
    ...
    @classmethod
    def get(cls, args):
        resp = aikoai.File.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        file = aikoai.File.delete(args.id)
        print(file)

    @classmethod
    def list(cls, args):
        file = aikoai.File.list()
        print(file)
...
class Image:
    ...
    def create_variation(cls, args):
        with open(args.image, "rb") as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc="Upload progress")
        resp = aikoai.Image.create_variation(
            image=buffer_reader,
            size=args.size,
            n=args.num_images,
            response_format=args.response_format,
...
class Audio:
    ...
    def transcribe(cls, args):
        ...
        resp = aikoai.Audio.transcribe_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
    ...
    def translate(cls, args):
        with open(args.file, "rb") as r:
            file_reader = BufferReader(r.read(), desc="Upload progress")
        resp = aikoai.Audio.translate_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
...
class FineTune:
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = aikoai.File.find_matching_files(
    ...
    def _get_or_upload(cls, file, check_if_file_exists=True):
        try:
            ...
            aikoai.File.retrieve(file)
            return file
        except aikoai.error.InvalidRequestError:
            pass
    ...
    def create(cls, args):
        ...
        resp = aikoai.FineTune.create(**create_args)
    ...
    @classmethod
    def get(cls, args):
        resp = aikoai.FineTune.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = aikoai.FineTune.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise aikoai.error.InvalidRequestError(
    ...
    def _stream_events(cls, job_id):
        ...
        events = aikoai.FineTune.stream_events(job_id)
    ...
    @classmethod
    def delete(cls, args):
        resp = aikoai.FineTune.delete(sid=args.id)
        print(resp)
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import aikoai.wandb_logger

        resp = aikoai.wandb_logger.WandbLogger.sync(
            id=args.id,
