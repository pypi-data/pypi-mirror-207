...
class LoginService:
    ...
    # 生成token
    @staticmethod
    def make_token(user_id, account, platform_id, platform_code):
        # 生成过期时间
    ...
    # 绑定token
    @staticmethod
    def bind_token(user_id, token, platform_id=None, is_create=True):
        if is_create:
            auth = {
                'user_id': user_id,
    ...
    def check_login(account, password, platform_code=None, **kwargs):
        ...
        if not auth_set:
            return None, "账户尚未开通登录服务：" + account + "(" + str(user_set.get("id")) + ")"
