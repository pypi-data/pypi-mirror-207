...
ALLOWED_EXTENSIONS = ["rar", "zip", "gz"]
SUPPORTED_FORMATS_TEXT = _("RAR archive, zip, gzip")

validate_archive_file_extension = validators.FileExtensionValidator(
    allowed_extensions=ALLOWED_EXTENSIONS,
)


class ArchiveField(fields.FileField):

    default_validators = [validate_archive_file_extension]
    default_error_messages = {
        'invalid_archive': _(
            "Upload a valid archive. The file you uploaded was either not an "
            "archive or a corrupted archive."
        ),
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.max_upload_size = getattr(settings, 'THREEDEE_MAX_UPLOAD_SIZE', pow(2, 20))
    ...
    def check_archive_file_format(self, f):
    ...
    def check_archive_file_size(self, f):
    ...
    def to_python(self, data):
        ...
        try:
            ...
            raise ValidationError(
                self.error_messages['invalid_archive'],
        ...
        if f is not None:
            self.check_archive_file_size(f)
            self.check_archive_file_format(f)
