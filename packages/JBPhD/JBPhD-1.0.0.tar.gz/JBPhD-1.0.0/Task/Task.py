...
if is_connected():
    ...
    if os.path.exists("participant number.txt"):
        ...
        class PinSystem:
            def __init__(self, master):
                ...
                self.pins = {}
                wb = openpyxl.load_workbook("pins.xlsx")
                ws = wb.active
                for row in ws.iter_rows(values_only=True):
                    self.pins[str(row[0])] = str(row[1])
        ...
        if __name__ == "__main__":
            ...
            root = tk.Tk()
            icon_path = os.path.abspath("uop.ico")
    ...
    import Location
    import Dependencies
    cpurun = subprocess.Popen([python_path, 'cputask.py'])
    import random
    import platform
    import tkinter as tk
    import time
    ...
    def main():
        ...
        if glob.glob(folder_name + '/*Consent.txt'):
            ...
            def emailsupport():
                ...
                body_label.pack(side=tk.TOP, padx=5, pady=5)
                body_text = tk.Text(email_window, wrap=tk.WORD)
                body_text.pack(side=tk.TOP, padx=5, pady=5)
                submit_button = tk.Button(email_window, text="Send", command=lambda: send_email(body_text.get("1.0", tk.END), email_window))
                submit_button.pack(side=tk.TOP, padx=5, pady=5)

            def send_email(body, email_window):
                import os
