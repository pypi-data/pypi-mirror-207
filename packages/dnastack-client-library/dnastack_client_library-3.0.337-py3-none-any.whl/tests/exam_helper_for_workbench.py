...
from dnastack import use, ServiceEndpoint
from dnastack.alpha.client.ewes.client import EWesClient
from dnastack.alpha.client.ewes.models import MinimalExtendedRun, ExtendedRunRequest, MinimalBatch, BatchRunRequest
...
class EngineAdapterConfiguration(BaseModel):
    type: str
    url: str
    oauth_config: Optional[EngineOAuthConfig]
...
class BaseWorkbenchTestCase(WithTestUserTestCase):
    ...
    def get_factory(cls) -> EndpointRepository:
        return use(cls.get_context_urls()[0], no_auth=True, existing_context_manager=container.get(ContextManager))
    ...
    def submit_hello_world_workflow_batch(self) -> MinimalBatch:
        ewes_client: EWesClient = self.get_ewes_client()
