...
class EC2Manager:
    ...
    def _get_secret_envs(self):
        ...
        for name in self._get_instance_names():
            envs[name] = {}
            for secret_file_name in self._config['instances'][name].get('secret_files', []):
                secret_file_name = secret_file_name.strip('/').replace('\\', '/')
    ...
    def upload_secret_files(self, show_error=False):
        ...
        for name in self._get_instance_names():
            for secret_file_name in self._config['instances'][name].get('secret_files', []):
                secret_file_name = secret_file_name.strip('/').replace('\\', '/')
                secret_file_path = os.path.normpath(os.path.join(os.getcwd(), 'compose', secret_file_name))
                if os.path.exists(secret_file_path):
                    ...
                    with open(secret_file_path, 'rb') as secret_file:
                        self._s3_client.upload_fileobj(
                            ...
                            f'{self._aws_backend_folder}/{name}/{secret_file_name}'
                        )
                elif show_error:
                    self._logger.error(f'Secret file "{secret_file_name}" does not exist on this machine!')
