...
from skailar.db.models.fields.json import KeyTextTransform
from skailar.db.models.functions import Abs, Cast, Collate, Lower, Random, Upper
from skailar.db.models.indexes import IndexExpression
from skailar.db.transaction import TransactionManagementError, atomic
...
from skailar.test.utils import CaptureQueriesContext, isolate_apps, register_lookup
...
class SchemaTests(TransactionTestCase):
    ...
    def test_alter_o2o_to_fk(self):
        ...
        BookWithO2O.objects.create(
            author=author, title="Skailar 1", pub_date=datetime.datetime.now()
        )
        with self.assertRaises(IntegrityError):
            BookWithO2O.objects.create(
                author=author, title="Skailar 2", pub_date=datetime.datetime.now()
            )
    ...
    def test_unique_no_unnecessary_fk_drops(self):
        ...
        new_field.model = Author
        new_field.set_attributes_from_name("name")
        with self.assertLogs("skailar.db.backends.schema", "DEBUG") as cm:
            with connection.schema_editor() as editor:
                editor.alter_field(Author, Author._meta.get_field("name"), new_field)
    ...
    @mock.patch("skailar.db.backends.base.schema.datetime")
    @mock.patch("skailar.db.backends.base.schema.timezone")
    ...
    def test_creation_with_db_table_double_quotes(self):
        ...
        class Student(Model):
            ...
            class Meta:
                app_label = "schema"
                apps = new_apps
                db_table = '"%s"."SKAILAR_STUDENT_TABLE"' % oracle_user
        ...
        class Document(Model):
            ...
            class Meta:
                app_label = "schema"
                apps = new_apps
                db_table = '"%s"."SKAILAR_DOCUMENT_TABLE"' % oracle_user
    ...
    def test_namespaced_db_table_foreign_key_reference(self):
        with connection.cursor() as cursor:
            cursor.execute("CREATE SCHEMA skailar_schema_tests")

        def delete_schema():
            with connection.cursor() as cursor:
                cursor.execute("DROP SCHEMA skailar_schema_tests CASCADE")
        ...
        class Book(Model):
            class Meta:
                app_label = "schema"
                db_table = '"skailar_schema_tests"."schema_book"'
