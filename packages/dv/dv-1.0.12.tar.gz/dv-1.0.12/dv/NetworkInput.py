...
class NetworkInput:
    ...
    def __init__(self, address='127.0.0.1', port=7777, as_numpy=False):
...
@deprecated("Explicit creation of NetworkEventInput will be removed in a future release, "
            "use NetworkInput instead, which provides automatic type detection")
class NetworkEventInput(NetworkInput):
...
@deprecated(
    "Explicit creation of NetworkNumpyEventPacketInput will be removed in a future release, "
    "use NetworkInput instead, which provides automatic type detection, passing as_numpy=True to get events as Numpy arrays"
)
class NetworkNumpyEventPacketInput(NetworkInput):

    def __init__(self, address='127.0.0.1', port=7777):
        super().__init__(address, port, as_numpy=True)
        if self.type != 'EVTS':
            raise TypeError("Incoming stream is not of type 'EVTS', but " + self.type)


@deprecated("Explicit creation of NetworkFrameInput will be removed in a future release, "
            "use NetworkInput instead, which provides automatic type detection")
class NetworkFrameInput(NetworkInput):
...
@deprecated("Explicit creation of NetworkIMUInput will be removed in a future release, "
            "use NetworkInput instead, which provides automatic type detection")
class NetworkIMUInput(NetworkInput):
...
@deprecated("Explicit creation of NetworkTriggerInput will be removed in a future release, "
            "use NetworkInput instead, which provides automatic type detection")
class NetworkTriggerInput(NetworkInput):
