...
class TooOldFipperVersion(Exception):
    """Fipper version is too old, raised by
    :meth:`~pycover.pycover.start` or
    :meth:`~pycover.pycover.run`
    """
    ...
    def __init__(
        ...
        super().__init__(
            f'Needed fipper {version_needed}+, '
            'actually installed is '
            f'{fipper_version}',
        )
...
class PyCoverAlreadyRunning(Exception):
    ...
    def __init__(self):
        super().__init__(
            'PyCover client is already running',
        )
