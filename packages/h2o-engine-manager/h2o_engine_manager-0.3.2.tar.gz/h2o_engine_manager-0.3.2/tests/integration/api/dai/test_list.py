...
@pytest.mark.parametrize(
    ...
    [
        ("list-dai-validation", -20, "", "", ""),
        ("list-dai-validation", 0, "non-existing-token", "", ""),
        ("list-dai-validation", 0, "", "incorrect order by", ""),
        ("list-dai-validation", 0, "", "", "incorrect filter"),
    ],
)
...
def test_list(dai_client):
    ...
    page = dai_client.list_engines("list-dai")
    assert len(page.engines) == 0
    assert page.next_page_token == ""
    assert page.total_size == 0

    # Arrange
    create_engines(dai_client=dai_client, workspace_id="list-dai")

    # Test getting first page.
    page = dai_client.list_engines(workspace_id="list-dai", page_size=1)
    ...
    page = dai_client.list_engines(
        workspace_id="list-dai", page_size=1, page_token=page.next_page_token
    )
    assert len(page.engines) == 1
    assert page.next_page_token != ""
    assert page.total_size == 3
    ...
    page = dai_client.list_engines(workspace_id="list-dai", page_size=1001)
    ...
    page = dai_client.list_engines(
        workspace_id="list-dai", filter="cpu = 0 AND cpu != 0"
    )
    assert len(page.engines) == 0
    assert page.next_page_token == ""
    assert page.total_size == 0
