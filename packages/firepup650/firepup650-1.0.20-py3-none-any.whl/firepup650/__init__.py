...
def input(prompt: str = "", cast: Type = str) -> type(cast):
    ...
    # Returns:
      type(cast) - The user's input, casted to `cast`
    ...
    while ret == "":
        try:
            ...
            print(f"That is not a vaild {cast.__name__}, please try again.")
    return ret


def replit_input(prompt: str = "", cast: Type = str) -> type(cast):
    """# Function: replit_input
    ...
    if prompt:
        print(prompt)
    return input(f"{replit_cursor} ", cast)
