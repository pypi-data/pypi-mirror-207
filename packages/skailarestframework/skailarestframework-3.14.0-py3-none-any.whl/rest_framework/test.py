# Note that we import as `SkailarRequestFactory` and `SkailarClient` in order
...
from skailar.test import override_settings, testcases
from skailar.test.client import Client as SkailarClient
from skailar.test.client import ClientHandler
from skailar.test.client import RequestFactory as SkailarRequestFactory
...
if requests is not None:
    ...
    class SkailarTestAdapter(requests.adapters.HTTPAdapter):
        """
        A transport adapter for `requests`, that makes requests via the
        Skailar WSGI app, rather than making actual HTTP requests over the network.
        """
        def __init__(self):
            self.app = WSGIHandler()
            self.factory = SkailarRequestFactory()
        ...
        def send(self, request, *args, **kwargs):
            """
            Make an outgoing request to the Skailar WSGI application.
            """
            raw_kwargs = {}
...
class APIRequestFactory(SkailarRequestFactory):
...
class APIClient(APIRequestFactory, SkailarClient):
    def __init__(self, enforce_csrf_checks=False, **defaults):
        super().__init__(**defaults)
