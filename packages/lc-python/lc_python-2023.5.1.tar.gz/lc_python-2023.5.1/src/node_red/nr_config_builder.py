...
class op:
    ...
    class ax:
        ...
        src = lambda name, **kw: op.ax.op(name, typ='ax-src', **kw)
        snk = lambda name, **kw: op.ax.op(name, typ='ax-snk', **kw)
        snk_mem = lambda wait_count=None, **kw: op.ax.op(
            'ax.snk.mem', typ='ax-snk', wait_count=wait_count, **kw
        )


ax_op = op.ax.op  # backwards compat
rx_op = lambda name, **kw: op.ax.op('rx.' + name, **kw)
...
nod = lambda *a, **kw: dict(id=a[0], type=a[1], name=a[2], z=a[3], **kw)
hub = lambda tab: nod('hub', 'ax-hub', 'ax-hub', tab)
...
def flow_io(tab, n):
    ...
    wscfg.pop('z')
    wsout = nod(id_ws_out, 'websocket out', '', tab, client='', server=id_ws, wires=[],)
...
is_op = lambda s: isinstance(s, (str, dict))
out_fd_2 = lambda s, **kw: app.info(s, **kw)
...
def complete_nodes(l, ctx):
    ...
    for node in l:
        ...
        if isinstance(node, list):
            ...
            if not node['type'] in no_z_types:
