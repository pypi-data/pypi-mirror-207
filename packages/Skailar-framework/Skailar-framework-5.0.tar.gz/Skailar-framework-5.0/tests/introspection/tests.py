...
from skailar.db.models import Index
from skailar.test import TransactionTestCase, skipUnlessDBFeature
...
class IntrospectionTests(TransactionTestCase):
    ...
    def test_skailar_table_names(self):
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE skailar_ixn_test_table (id INTEGER);")
            tl = connection.introspection.skailar_table_names()
            cursor.execute("DROP TABLE skailar_ixn_test_table;")
            self.assertNotIn(
                "skailar_ixn_test_table",
                tl,
                "skailar_table_names() returned a non-Skailar table",
            )

    def test_skailar_table_names_retval_type(self):
        # Table name is a list #15216
        tl = connection.introspection.skailar_table_names(only_existing=True)
        self.assertIs(type(tl), list)
        tl = connection.introspection.skailar_table_names(only_existing=False)
        self.assertIs(type(tl), list)
    ...
    def test_unmanaged_through_model(self):
        tables = connection.introspection.skailar_table_names()
    ...
    def test_postgresql_real_type(self):
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE skailar_ixn_real_test_table (number REAL);")
            desc = connection.introspection.get_table_description(
                cursor, "skailar_ixn_real_test_table"
            )
            cursor.execute("DROP TABLE skailar_ixn_real_test_table;")
        self.assertEqual(
