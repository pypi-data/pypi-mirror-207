...
class KrxKindWeb:
    ...
    def get_document_html(self, doc_id) -> Tuple[str, str]:
        """
        ...
        link = first_doc.get("link")
        category = first_doc.get("category")
        logger.info(f"doc_id : {doc_id}, category : {category}, link : {link}")
        response = self.session.get(link)
        if response.status_code != 200:
            logger.error(f"doc_id : {doc_id}, category : {category}, link : {link}, status_code : {response.status_code}")
            return None, link
