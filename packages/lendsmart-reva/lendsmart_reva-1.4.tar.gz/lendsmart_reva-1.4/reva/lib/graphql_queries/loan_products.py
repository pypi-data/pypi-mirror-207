...
mutation MyMutation($data: [loan_products_insert_input!]!) {
  insert_loan_products(objects: $data, on_conflict: {constraint: loan_products_pkey, update_columns: [object_meta,metadata,product_visual_name,product_name,description,subproducts]}) {
      returning {
        id
        subproducts
      }
