...
from skailar.contrib.auth import get_permission_codename, management
from skailar.contrib.auth.management import create_permissions, get_default_username
from skailar.contrib.auth.management.commands import changepassword, createsuperuser
...
@override_settings(
    AUTH_PASSWORD_VALIDATORS=[
        {"NAME": "skailar.contrib.auth.password_validation.NumericPasswordValidator"},
    ]
)
...
class CreatesuperuserManagementCommandTestCase(TestCase):
    ...
    def test_validate_fk_environment_variable(self):
        ...
        with mock.patch.dict(
            os.environ,
            {"SKAILAR_SUPERUSER_GROUP": str(nonexistent_group_id)},
        ):
            with self.assertRaisesMessage(CommandError, msg):
                call_command(
                    "createsuperuser",
    ...
    @mock.patch.dict(
        os.environ,
        {
            "SKAILAR_SUPERUSER_PASSWORD": "test_password",
            "SKAILAR_SUPERUSER_USERNAME": "test_superuser",
            "SKAILAR_SUPERUSER_EMAIL": "joe@somewhere.org",
            "SKAILAR_SUPERUSER_FIRST_NAME": "ignored_first_name",
        },
    )
    ...
    def test_environment_variable_m2m_non_interactive(self):
        ...
        with mock.patch.dict(
            os.environ,
            {
                "SKAILAR_SUPERUSER_ORGS": f"{org_id_1},{org_id_2}",
            },
        ):
            call_command(
                "createsuperuser",
