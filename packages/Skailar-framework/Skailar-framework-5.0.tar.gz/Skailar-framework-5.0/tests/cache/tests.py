...
from skailar.core import management, signals
from skailar.core.cache import (
    DEFAULT_CACHE_ALIAS,
    CacheHandler,
    CacheKeyWarning,
    InvalidCacheKey,
...
from skailar.core.cache.backends.base import InvalidCacheBackendError
from skailar.core.cache.backends.redis import RedisCacheClient
from skailar.core.cache.utils import make_template_fragment_key
from skailar.db import close_old_connections, connection, connections
from skailar.db.backends.utils import CursorWrapper
from skailar.http import (
    HttpRequest,
    HttpResponse,
...
from skailar.template import engines
from skailar.template.context_processors import csrf
...
from skailar.test.signals import setting_changed
from skailar.test.utils import CaptureQueriesContext
from skailar.utils import timezone, translation
from skailar.utils.cache import (
    get_cache_key,
    learn_cache_key,
    patch_cache_control,
    patch_vary_headers,
)
from skailar.views.decorators.cache import cache_control, cache_page
...
@override_settings(
    CACHES={
        "default": {
            "BACKEND": "skailar.core.cache.backends.dummy.DummyCache",
        }
    }
)
...
@override_settings(
    CACHES=caches_setting_for_tests(
        BACKEND="skailar.core.cache.backends.db.DatabaseCache",
...
class DBCacheTests(BaseCacheTests, TransactionTestCase):
    ...
    def test_delete_cursor_rowcount(self):
        ...
        cache.set_many({"a": 1, "b": 2})
        with mock.patch("skailar.db.backends.utils.CursorWrapper", MockedCursorWrapper):
...
class DBCacheRouter:
    ...
    def db_for_read(self, model, **hints):
        if model._meta.app_label == "skailar_cache":
            return "other"
        return None
...
class LocMemCacheTests(BaseCacheTests, TestCase):
    ...
    @override_settings(
        CACHES={
            "default": {"BACKEND": "skailar.core.cache.backends.locmem.LocMemCache"},
            "other": {
                "BACKEND": "skailar.core.cache.backends.locmem.LocMemCache",
                "LOCATION": "other",
            },
...
PyLibMCCache_params = configured_caches.get(
    "skailar.core.cache.backends.memcached.PyLibMCCache"
)
PyMemcacheCache_params = configured_caches.get(
    "skailar.core.cache.backends.memcached.PyMemcacheCache"
)

# The memcached backends don't support cull-related options like `MAX_ENTRIES`.
memcached_excluded_caches = {"cull", "zero_cull"}

RedisCache_params = configured_caches.get("skailar.core.cache.backends.redis.RedisCache")
...
@override_settings(
    CACHES=caches_setting_for_tests(
        BACKEND="skailar.core.cache.backends.filebased.FileBasedCache",
    )
)
...
class RedisCacheTests(BaseCacheTests, TestCase):
    ...
    def test_incr_write_connection(self):
        cache.set("number", 42)
        with mock.patch(
            "skailar.core.cache.backends.redis.RedisCacheClient.get_client"
...
class DefaultNonExpiringCacheKeyTests(SimpleTestCase):
    ...
    def test_default_expiration_time_for_keys_is_5_minutes(self):
        ...
        skailar.core.cache.backends.base.BaseCache.__init__().
        """
...
class CacheUtils(SimpleTestCase):
    """TestCase for skailar.utils.cache functions."""
...
class CacheMiddlewareTest(SimpleTestCase):
    ...
    def test_constructor(self):
        ...
        # Value of DEFAULT_CACHE_ALIAS from skailar.core.cache
    ...
    def test_sensitive_cookie_not_cached(self):
        """
        Skailar must prevent caching of responses that set a user-specific (and
...
class TestWithTemplateResponse(SimpleTestCase):
    ...
    def test_patch_vary_headers(self):
        ...
        for initial_vary, newheaders, resulting_vary in headers:
            with self.subTest(initial_vary=initial_vary, newheaders=newheaders):
                template = engines["skailar"].from_string("This is a test")
