...
from calorine.nep.potential import read_potential
...
def test_dipole_consistent_CPU_GPU():
    """
...
def test_get_dipole_gradient_numeric():
    ...
    # copy = structure.copy()
    # dipole = (
    #     get_dipole(copy, potential_filename=nep3) + charge * copy.get_center_of_mass()
    # )
...
def test_get_dipole_gradient_numeric_without_correction():
    """Compare gradient to manually computed, for a two atom system"""
    nep3 = 'tests/nep_models/nep4_dipole_Christian.txt'
    structure = read('tests/example_files/dipole/test.xyz')[:2]
    ...
    # Calculate expected dipole gradient
    # copy = structure.copy()
    # dipole = get_dipole(copy, potential_filename=nep3)
    # displacement = 0.01
    # positions = copy.get_positions()
    # positions[0, 0] += displacement  # move in x direction
    # copy.set_positions(positions)
    # dipole_forward = get_dipole(copy, potential_filename=nep3)
    # expected = (dipole_forward - dipole) / displacement
    # print(expected)
...
def test_get_dipole_gradient_nep_too_small_displacement():
    """
...
def test_get_dipole_gradient_invalid_method(PbTe):
...
def test_get_dipole_gradient_invalid_displacement(PbTe):
    """Tries to get dipole gradient with an invalid displacement"""
    with pytest.raises(ValueError) as e:
        get_dipole_gradient(
            PbTe, potential_filename='hmm.txt', displacement=0, backend='python'
        )
    assert 'displacement must be > 0 Å' in str(e)


def latent_space_reference(structure: Atoms, potential_filename: str) -> np.ndarray:
    ...
    d = get_descriptors(structure, potential_filename=potential_filename)
    z0 = w0 @ d.T
    a0 = np.tanh(z0 - b0)
    z1 = a0.T * w1
