...
def read_structures(dirname: str) -> Tuple[List[Atoms], List[Atoms]]:
    """Parses the ``energy_*.out``, ``force_*.out``, and ``virial_*.out``
    files from a nep run and returns their content as lists.  The
    first and second list contain the structures from the training and
