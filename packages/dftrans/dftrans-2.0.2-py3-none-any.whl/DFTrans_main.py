...
def loaddatafromdatabasebysql():
    ...
    currentdfnumber = len(specialdflist)
    # 获取界面默认值
    defaultrecordlist = [IP_ADDRESS, IP_PORT, DATABASE_NAME, USER_NAME, PASSWORD, SQL_STRING]
    if manualbuttonoperatedict.get(LOAD_SQL, None) is None:
        manualbuttonoperatedict[LOAD_SQL] = {}
    ipaddress, ipport, databasename, username, password, sqlstring = map(
        ...
        # 确定分析文件
            ...
            try:
                ...
                manualbuttonoperatedict[LOAD_SQL] = dict(zip(
                    defaultrecordlist, [ipaddress, port, database, user, password, sqlstr]))
            except Exception as err:
...
def savedatatodatabasebysql():
    ...
    currentdfnumber = len(specialdflist)
    # 获取界面默认值
    defaultrecordlist = [IP_ADDRESS, IP_PORT, DATABASE_NAME, USER_NAME, PASSWORD, SAVEDATA_FILENAME, REPALCE_METHOD]
    if manualbuttonoperatedict.get(SAVE_SQL, None) is None:
        manualbuttonoperatedict[SAVE_SQL] = {}
    ipaddress, ipport, databasename, username, password, outputname, replacemethod = map(
        ...
        # 确定
            ...
            try:
                ...
                manualbuttonoperatedict[SAVE_SQL] = dict(zip(
                    defaultrecordlist, [ipaddress, port, database, user, password, name, replacemethod]))
            except Exception as err:
                Sg.popup_error("相关参数输入不正确：{}！".format(err),
                               modal=True, keep_on_top=True, icon=LOCAL_SYSTEM_ICON)
                continue
