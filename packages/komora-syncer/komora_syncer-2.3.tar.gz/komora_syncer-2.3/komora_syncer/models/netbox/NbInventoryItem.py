...
class NbInventoryItem(NetboxBase):
    def __init__(self, komora_obj):
        ...
        self.komora_url = f"{get_config()['komora']['KOMORA_URL']}/app/inventarization/{self.komora_id}"
        self.netbox_url = komora_obj.netboxUrl
        self.api_object = None

    def find(self):
        if self.komora_id:
            netbox_inventory_item = self.netbox.connection.dcim.inventory_items.get(
                cf_komora_id=self.komora_id)
            if netbox_inventory_item:
                self.api_object = netbox_inventory_item
                return self.api_object
            else:
                netbox_inventory_item = self.netbox.connection.dcim.inventory_items.get(cf_asset_numbers = self.inventory_number)
                if netbox_inventory_item:
                   self.api_object = netbox_inventory_item 
                   return self.api_object
        else:
            ...
            match = re.search(id_pattern, self.netbox_url)
            if match:
                id = match.group(1)
                self.api_object = self.netbox.connection.dcim.inventory_items.get(
                    id=id)
                return self.api_object

        logger.warning(
            f"Unable to find Inventory Item")
        return self.api_object

    def update(self, nb_inventory_item):
        try:
            if nb_inventory_item.update(self.get_params()):
                self.api_object = nb_inventory_item
                logger.info(
                    f"Inventory: {self.inventory_number} updated successfuly")
    ...
    def synchronize(self):
        ...
        if inventory_item:
            ...
            logger.info(
                f"Inventory Item: {self.inventory_number} not found in Netbox.")
    ...
    def get_nb_inventory_items_data():
        ...
        SITES_CACHE = {
            site.id: site for site in pynetbox.connection.dcim.sites.all()}
        DEVICES_CACHE = {dev.id: {
            "device": dev, "site": SITES_CACHE[dev.site.id], "location": dev.location} for dev in pynetbox.connection.dcim.devices.all()}
        ...
        def inventory_to_json(inventory):
            ...
            if site and site['custom_fields'].get('komora_id', ""):
                inventory_dict['site_id'] = site['custom_fields'].get(
                    'komora_id', "")
        ...
        for inv in pynetbox.connection.dcim.inventory_items.filter(cf_abra_discovered=True):
