...
from .orbit import Callable, axis_rotation_matrices, quat_to_dcm
from .coordinates import sun, moon
...
from .observer import lla_to_eci, geodetic_lat_to_geocentric
...
class Station:
    def __init__(self, preset: str = "pogs"):
    ...
    def ra_dec_to_az_el(
        ...
        a = np.arctan2(az_el_unit[1], az_el_unit[0])  # Azimuth in [rad]
        a = a + 180  # Correct for atan2 offset
        h = np.arctan2(
            az_el_unit[2], np.linalg.norm(az_el_unit[:2])
        )  # Elevation in [rad]
        return (a, h)

    def eval_constraints(self, **kwargs) -> np.ndarray[bool]:
        constraint_results = np.array(
            [
                cnstr.eval_fcn(**kwargs).flatten()
                for cnstr in self.constraints
            ]
        ).T
        all_constraints_satisfied = np.all(constraint_results, axis=1)
    ...
    def observe_light_curve(
        ...
        constraint_results = self.eval_constraints(
            dates=dates,
        ...
        lc_noisy[np.logical_not(constraint_results).flatten()] = np.nan
        lc_clean[np.logical_not(constraint_results).flatten()] = np.nan
        return (
            lc_noisy,
            {
