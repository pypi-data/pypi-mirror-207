...
with open(depfile, 'w+') as f:
    ...
    f.write(f"Dependency log for participant {participant_number} at {outputtime} on the {date_time_string} \n \n")
    now = datetime.now()
    outputtime = now.strftime("%H:%M:%S")
    f.write(f'Connection: {outputloc} \n\n{lines}')

    f.write(f"Dependency install began at {outputtime} \n \n")
    f.write(f"Python version: {sys.version}\n \n")
    f.write(f"Python executable path: {sys.executable}\n \n")
    ...
    try:
        ...
        if stderr:
            ...
            f.write('\n')
            raise Exception(f'At {outputtime}, an error occurred during the upgrade of pip')
        
        f.write(stdout.decode('utf-8'))
        f.write('\n')

        packages = ['pygame', 'pandas', 'google-auth', 'google-auth-oauthlib', 'google-auth-httplib2', 'google-api-python-client', 'Pillow']
        ...
        if stderr:
            ...
            f.write('\n')
            raise Exception(f'At {outputtime}, an error occurred during the installation of the Google dependencies')
        f.write(stdout.decode('utf-8'))
        print(Exception)
        f.write('\n')
        ...
        now = datetime.now()
        outputtime = now.strftime("%H:%M:%S")
        f.write(f"End time: {outputtime}")

        def error_message():
            messagebox.showerror("Error", "There has been an error in installing one of the files needed.")

        def submit_email():
            global pptemail
            pptemail = email_entry.get()
            f.close()
            root.destroy()
            
        root = tk.Tk()

        root.title("Dependency Installation Error")
        fontsize = ("Helvetica", 12)
        fontsize1 = ("Helvetica", 14)

        buttonsize = ("Arial", 12)
        fontsize5 = ("Arial", 10)

        label_text = f"There has been an error in installing one of the files needed for the task.\n \n If you are happy, please add your email address here and we will be in contact to resolve this error."
        label = tk.Label(root, text=label_text, font=fontsize)
        label.pack(fill='both', expand=False, pady=20)
        label_width = label.winfo_reqwidth()
        label_height = label.winfo_reqheight()

        window_width = label_width + 90  
        window_height = label_height + 120
        ...
        email_entry.pack(fill='both', expand=False, pady=10)

        submit_button = tk.Button(root, text="Submit", command=submit_email, font=buttonsize)
        submit_button.pack(fill='both', expand=False, pady=5)
...
def startup():
    ...
    def emailsupport():
        ...
        submit_button.pack(fill='both', expand=False, side=tk.TOP, padx=5, pady=5)
    ...
    time_label.pack(expand=False, side=tk.TOP, padx=20, pady=20, anchor="nw")
    ...
    welcome_label.pack(fill='both', expand=False, pady=20)
    ...
    text_label.pack(fill='both', expand=False, pady=10)
    ...
    label2 = tk.Label(button2_frame, text=" - Read for all of the information regarding the study", font=fontsize)
    label2.pack(fill='both', expand=False, side=tk.LEFT)
    ...
    button12_frame.pack(fill='both', expand=False, side=tk.TOP, padx=30, pady=180, anchor="nw")

    button12 = tk.Button(button12_frame, text="Email for assistance or queries", command=emailsupport, font=fontsize)
    button12.pack(fill='both', expand=False, side=tk.LEFT)

    label3 = tk.Label(button3_frame, text=" - Give consent to participate here", font=fontsize)
    label3.pack(fill='both', expand=False, side=tk.LEFT, pady = 5)  # Change the factor as needed
    
    button4 = tk.Button(root, text="Start the Task", command=taskstart, font=fontsize)
    button4.pack(expand=False, side=tk.TOP, padx=40, pady=0, anchor="center")
