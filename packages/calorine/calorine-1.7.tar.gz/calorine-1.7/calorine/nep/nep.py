...
def _setup_nepy(
    ...
    if debug:
        nepy = _nepy.NEPY(potential_filename, N_atoms, cell, symbols, positions, masses)
    else:
        with open(os.devnull, 'w') as f:
            with contextlib.redirect_stdout(f):
                with contextlib.redirect_stderr(f):
                    nepy = _nepy.NEPY(
                        potential_filename, N_atoms, cell, symbols, positions, masses
                    )
    return nepy
...
def get_descriptors(
    structure: Atoms, potential_filename: Optional[str] = None, debug: bool = False
) -> np.ndarray:
    ...
    nepy = _setup_nepy(
        potential_filename, N_atoms, cell, symbols, positions, masses, debug
    )
...
def get_latent_space(
    structure: Atoms, potential_filename: Union[str, None] = None, debug: bool = False
) -> np.ndarray:
    ...
    latent = np.array(latent).reshape(-1, N_atoms).T
    return latent
...
def _dipole_gradient_python(
    ...
    if method == 'forward difference':
        ...
        d = (
            get_dipole(structure, potential_filename)
        ...
        for atom in range(N):
            for cartesian in range(3):
                ...
                d_forward[atom, cartesian, :] = (
                    get_dipole(copy, potential_filename)
        ...
        for atom in range(N):
            for cartesian in range(3):
                ...
                d_forward[atom, cartesian, :] = (
                    get_dipole(copy_forward, potential_filename)
                ...
                d_backward[atom, cartesian, :] = (
                    get_dipole(copy_backward, potential_filename)
...
def _dipole_gradient_nep(
    ...
    if method == 'forward difference':
        ...
        dipoles = (
            _predict_dipole_batch(structures, potential_filename, nep_command) * N
...
def _predict_dipole_batch(
    structures: List[Atoms], potential_filename: str, nep_command: str = 'nep'
) -> np.ndarray:
    ...
    from calorine.nep import write_nepfile, write_structures, read_potential

    with TemporaryDirectory() as directory:
        shutil.copy2(potential_filename, join_path(directory, 'nep.txt'))
        potential = read_potential(potential_filename)

        parameters = dict(
            prediction=1,
            mode=1,
            version=potential.version,
            n_max=[potential.n_max_radial, potential.n_max_angular],
            cutoff=[potential.radial_cutoff, potential.angular_cutoff],
            basis_size=[potential.n_basis_radial, potential.n_basis_radial],
            l_max=[potential.l_max_3b, potential.l_max_4b, potential.l_max_5b],
            neuron=potential.n_neuron,
            type=[len(potential.types), *potential.types],
        )
