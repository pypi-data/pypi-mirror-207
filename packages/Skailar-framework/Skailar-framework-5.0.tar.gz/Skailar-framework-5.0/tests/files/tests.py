...
from skailar.core.files import File, locks
from skailar.core.files.base import ContentFile
from skailar.core.files.move import file_move_safe
...
from skailar.test import override_settings
...
try:
    ...
    from skailar.core.files import images
...
class FileTests(unittest.TestCase):
    ...
    def test_namedtemporaryfile_closes(self):
        """
        The symbol skailar.core.files.NamedTemporaryFile is assigned as
...
class FileMoveSafeTests(unittest.TestCase):
    ...
    def test_file_move_permissionerror(self):
        ...
        try:
            ...
            # file_safe_move().
            with mock.patch("skailar.core.files.move.os.rename", side_effect=OSError()):
                # An error besides PermissionError isn't ignored.
                with mock.patch(
                    "skailar.core.files.move.copystat", side_effect=os_error
                ):
                ...
                with mock.patch(
                    "skailar.core.files.move.copystat", side_effect=permission_error
                ):
                    with mock.patch(
                        "skailar.core.files.move.copymode", side_effect=os_error
                    ):
                        with self.assertRaises(OSError):
                            file_move_safe(
                ...
                with mock.patch(
                    ...
                    with mock.patch(
                        "skailar.core.files.move.copymode", side_effect=permission_error
                    ):
                        self.assertIsNone(
                            file_move_safe(
...
class SpooledTempTests(unittest.TestCase):
    def test_in_memory_spooled_temp(self):
        with tempfile.SpooledTemporaryFile() as temp:
            ...
            skailar_file = File(temp, name="something.txt")
            self.assertEqual(skailar_file.size, 17)
