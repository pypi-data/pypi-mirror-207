...
def test_cpunep_readwrite_dict(NEP3CPUNEP):
    ...
    assert dict['potential_filename'] == 'tests/nep_models/nep3_v3.3.1_PbTe_Fan22.txt'
...
def test_get_dipole_gradient():
    ...
    gradient_forward_cpp = calc.get_dipole_gradient(
    ...
    gradient_central_cpp = calc.get_dipole_gradient(
...
def test_get_dipole_gradient_defaults():
    ...
    gradient_default = calc.get_dipole_gradient()

    calc = CPUNEP(
        nep3,
        atoms=structure,
    )
    gradient_central = calc.get_dipole_gradient(
...
def test_get_dipole_gradient_numeric():
    ...
    gradient_cpp = calc.get_dipole_gradient(
...
def test_get_dipole_gradient_invalid_method(NEP3File, PbTe):
    ...
    with pytest.raises(ValueError) as e:
        ...
        calc.get_dipole_gradient(displacement=0.01, method='lmao')
    assert 'Invalid method lmao for calculating gradient' in str(e)


def test_get_dipole_gradient_invalid_displacement(NEP3File, PbTe):
