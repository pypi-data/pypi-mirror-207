...
class RepositoryPath(AbstractRepositoryPath):
    ...
    def connection_str(self) -> str:
        ...
        con = "sqlite://" + str(db_path.absolute())
        return con
...
class S3RepositoryPath(AbstractRepositoryPath):
    def __init__(self, bucket_name: str, db_path: str, region: str = "us-west-2") -> None:
    ...
    @property
    def connection_str(self) -> str:
        con = "sqlite://" + str(self.db_path.absolute())
        return con
    ...
    def read_df(self, dir_name: str, file_name: str) -> pl.DataFrame:
        s3_file_name = f"{dir_name}/{file_name}"
        stream = io.BytesIO()
        s3 = boto3.resource("s3")
