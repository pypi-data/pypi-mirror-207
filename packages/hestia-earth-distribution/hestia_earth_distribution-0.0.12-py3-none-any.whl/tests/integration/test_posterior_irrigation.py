...
from hestia_earth.distribution.posterior_irrigation import get_post
from hestia_earth.distribution.cycle import sum_water
...
def _should_validate(cycle: dict):
    return cycle.get('completeness', {}).get('water', False) and not is_empty(sum_water(cycle))
...
def test_validate_cycles():
    ...
    expected = [v for v in df['valid_water'].to_list() if not is_empty(v)]
