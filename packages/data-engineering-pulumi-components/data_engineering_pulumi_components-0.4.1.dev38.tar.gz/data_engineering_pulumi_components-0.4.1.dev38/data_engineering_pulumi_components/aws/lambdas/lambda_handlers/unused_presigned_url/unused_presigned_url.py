...
class GetUnpackedLog(ABC):
    ...
    def get_logs(self) -> list:
        ...
        start_query_response = client.start_query(
            ...
            startTime=int((datetime.datetime.today() - datetime.timedelta(hours=1)).timestamp()),
        ...
        logger.info(f"the following query has been executed on CloudWatch '{query}'. "
...
class EvaluateUnpackedLogs(object):
    def __init__(self, unpacked_upload_function: dict, unpacked_validate_function: list) -> None:
...
def lambda_handler(event, context) -> None:
    # Get logs
    upload_function_logs = UnpackUploadLog("/aws/lambda/ap-uploader-prod-upload-function-8c7a314")
    if not upload_function_logs.get_logs():
        logger.info("no presigned urls have been requested in the last hour")
        return
    validate_function_logs = UnpackValidateLog("/aws/lambda/data-eng-uploader-prod-validate")
