...
def _checksampling(e_xyz, delta_b, b_bgd, full_b, flag_no_resamp):
    ...
    resample_e_options = dict(f_s=fs_e)
    ...
    if flag_no_resamp:
        ...
        if fs_b > 1.5 * fs_e:
            ...
            delta_b = resample(delta_b, e_xyz, **resample_e_options)
            b_bgd = resample(b_bgd, e_xyz, **resample_e_options)
            ...
            warnings.warn(
                "Interpolating b and e to 2x e sampling",
                UserWarning,
            )
...
def ebsp(e_xyz, db_xyz, b_xyz, b_bgd, xyz, freq_int, **kwargs):
    ...
    n_wave_period_avg = kwargs.get("nav", 8)
    ...
    if flag_want_fac and fac_matrix is None:
        if xyz is None:
            ...
            xyz = ts_vec_xyz(
                db_xyz.time.data,
                np.tile(xyz, (len(db_xyz), 1)),
            )

        xyz = resample(xyz, db_xyz, **{"f_s": fsb})

    b_bgd = resample(b_bgd, db_xyz, **{"f_s": fsb})
    ...
    if want_ee:
        ...
        temp_ = _checksampling(e_xyz, db_xyz, b_bgd, b_xyz, flag_no_resample)
        e_xyz, db_xyz, b_bgd, b_xyz, in_sampling = temp_
    ...
    in_time = db_xyz.time.data.astype(np.int64) * 1e-9
    ...
    if flag_want_fac:
        ...
        if want_ee:
            if not flag_de_dot_b0:
                ...
                if e_xyz.shape[1] < 3:
                    raise TypeError(
                        "E must be a 3D vector to be rotated to " "FAC",
                    )
    ...
    sw_b = fft.fft(db_xyz.data, axis=0, workers=os.cpu_count())
    ...
    censure = np.floor(
        2 * a_ * out_sampling / in_sampling * n_wave_period_avg,
    )
    ...
    for ind_a, f_a in enumerate(a_):
        ...
        if f_a / n_wave_period_avg > out_sampling:
            av_window = 1 / out_sampling
        else:
            av_window = n_wave_period_avg / f_a
        ...
        if want_ee:
            ...
            if flag_want_fac and not flag_de_dot_b0:
                sum_power_eisr2 = np.sum(
                    2 * np.pi * (w_eisr2 * np.conj(w_eisr2)) / new_freq_mat,
                    axis=1,
                )
            else:
                sum_power_eisr2 = np.sum(
                    2 * np.pi * (we * np.conj(we)) / new_freq_mat,
                    axis=1,
                )
        ...
        if want_polarization:
            ...
            censure_idx = np.hstack(
                [
                    ...
                    np.arange(
                        np.max([0, len(out_time) - censure[ind_a] - 1]),
                        len(out_time),
                    ),
                ],
            )
            ...
            for i in range(n_data_out):
                if np.isnan(a_mat[..., i]).any():
                    ...
                    (
                        ...
                        w_mat[..., i],
                        v_mat[..., i],
                    ) = np.linalg.svd(a_mat[..., i], full_matrices=False)
    ...
    for ind_a in range(len(a_)):
        censure_idx = np.hstack(
            [
                ...
                np.arange(
                    np.max([1, len(in_time) - censure[ind_a]]),
                    len(in_time),
                ),
            ],
        )
    ...
    for i in range(len(idx_nan_b) - 1):
        if idx_nan_b[i] < idx_nan_b[i + 1]:
            for j in range(len(a_)):
                censure_index_front = np.arange(
                    np.max([i - censure3[j], 0]),
                    i,
                )
        ...
        if idx_nan_b[i] > idx_nan_b[i + 1]:
            for j in range(len(a_)):
                censure_index_back = np.arange(
                    i,
                    np.min([i + censure3[j], n_data2]),
                )
    ...
    if want_ee:
        ...
        power_2e_isr2_plot = _average_data(
            power_2e_isr2_plot,
            in_time,
            out_time,
        )
    ...
    if want_polarization:
        ...
        k_th_ph_svd_fac = np.zeros(
            (the_svd_fac.shape[0], the_svd_fac.shape[1], 2),
        )
