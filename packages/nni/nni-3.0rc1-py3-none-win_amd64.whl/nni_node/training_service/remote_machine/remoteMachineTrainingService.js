...
let RemoteMachineTrainingService = class RemoteMachineTrainingService {
    ...
    constructor(config) {
        ...
        this.expRootDir = (0, utils_1.getExperimentRootDir)();
        this.timer = component.get(observableTimer_1.ObservableTimer);
        this.log = (0, log_1.getLogger)('RemoteMachineTrainingService');
        ...
        if (this.sshConnectionPromises.length > 0) {
            ...
            for (const [machineConfig, executorManager] of this.machineExecutorManagerMap.entries()) {
                ...
                if (executor !== undefined) {
                    this.machineCopyExpCodeDirPromiseMap.set(machineConfig, executor.copyDirectoryToRemote(this.config.trialCodeDirectory, executor.getRemoteCodePath((0, experimentStartupInfo_1.getExperimentId)())));
                }
            }
        }
        while (!this.stopping) {
    ...
    async submitTrialJob(form) {
        const trialJobId = form.id === undefined ? (0, utils_1.uniqueString)(5) : form.id;
    ...
    async cancelTrialJob(trialJobId, isEarlyStopped = false) {
        ...
        if (trialJob.rmMeta !== undefined) {
            ...
            (0, assert_1.default)(isEarlyStopped === false, 'isEarlyStopped is not supposed to be true here.');
            trialJob.status = (0, utils_1.getJobCancelStatus)(isEarlyStopped);
        }
    }
        ...
        try {
            for (const executorManager of this.machineExecutorManagerMap.values()) {
                ...
                if (executor !== undefined) {
                    ...
                    const gpuJobPidPath = executor.joinPath(executor.getRemoteScriptsPath((0, experimentStartupInfo_1.getExperimentId)()), 'pid');
    ...
    async initRemoteMachineOnConnected(machineConfig) {
        ...
        await executor.createFolder(executor.getRemoteExperimentRootDir((0, experimentStartupInfo_1.getExperimentId)()));
        const remoteGpuScriptCollectorDir = executor.getRemoteScriptsPath((0, experimentStartupInfo_1.getExperimentId)());
        await executor.createFolder(remoteGpuScriptCollectorDir, true);
        await executor.allowPermission(true, nniRootDir);
        const script = executor.generateGpuStatsScript((0, experimentStartupInfo_1.getExperimentId)());
    ...
    async prepareTrialJob(trialJobId) {
        ...
        if (rmScheduleResult.resultType === gpuData_1.ScheduleResultType.REQUIRE_EXCEED_TOTAL) {
            ...
            trialJobDetail.workingDirectory = executor.joinPath(executor.getRemoteExperimentRootDir((0, experimentStartupInfo_1.getExperimentId)()), 'trials', trialJobDetail.id);
    ...
    async launchTrialOnScheduledMachine(trialJobId, form, rmScheduleInfo) {
        ...
        const nniManagerIp = this.config.nniManagerIp ? this.config.nniManagerIp : await (0, utils_1.getIPV4Address)();
        ...
        const version = this.versionCheck ? await (0, utils_1.getVersion)() : '';
        const runScriptTrialContent = executor.generateStartScript(trialJobDetail.workingDirectory, trialJobId, (0, experimentStartupInfo_1.getExperimentId)(), trialJobDetail.form.sequenceId.toString(), false, this.config.trialCommand, nniManagerIp, this.remoteRestServerPort, version, this.logCollection, cudaVisible);
        await (0, util_1.execMkdir)(path_1.default.join(trialLocalTempFolder, '.nni'));
        await (0, shellUtils_1.createScriptFile)(path_1.default.join(trialLocalTempFolder, executor.getScriptName("install_nni")), containerJobData_1.CONTAINER_INSTALL_NNI_SHELL_FORMAT);
        await (0, shellUtils_1.createScriptFile)(path_1.default.join(trialLocalTempFolder, executor.getScriptName("run")), runScriptTrialContent);
        await this.writeParameterFile(trialJobId, form.hyperParameters);
    ...
    async updateTrialJobStatus(trialJob, executor) {
        const deferred = new ts_deferred_1.Deferred();
        const jobpidPath = this.getJobPidPath(executor, trialJob.id);
        const trialReturnCodeFilePath = executor.joinPath(executor.getRemoteExperimentRootDir((0, experimentStartupInfo_1.getExperimentId)()), 'trials', trialJob.id, '.nni', 'code');
        try {
                ...
                if (match !== null) {
                    ...
                    if (parseInt(code, 10) === 0) {
                        ...
                        if (trialJob.isEarlyStopped === undefined) {
                            ...
                            trialJob.status = (0, utils_1.getJobCancelStatus)(trialJob.isEarlyStopped);
                        }
                    }
    ...
    async writeParameterFile(trialJobId, hyperParameters) {
        const executor = await this.getExecutor(trialJobId);
        const trialWorkingFolder = executor.joinPath(executor.getRemoteExperimentRootDir((0, experimentStartupInfo_1.getExperimentId)()), 'trials', trialJobId);
        const trialLocalTempFolder = path_1.default.join(this.expRootDir, 'trials', trialJobId);
        const fileName = (0, utils_1.generateParamFileName)(hyperParameters);
