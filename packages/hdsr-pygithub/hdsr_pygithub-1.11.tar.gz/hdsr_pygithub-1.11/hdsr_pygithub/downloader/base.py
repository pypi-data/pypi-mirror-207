...
class GithubDownloaderBase(ABC):
    ...
    def personal_access_token(self) -> str:
        ...
        try:
            ...
            assert token, (
                f"file {token_path} exists, but it must contain a row: "
                f"{constants.GITHUB_PERSONAL_ACCESS_TOKEN}=blablabla"
            )
            assert (
                isinstance(token, str) and len(token) > 20
            ), f"{constants.GITHUB_PERSONAL_ACCESS_TOKEN} must a string of at least 20 chars"
            self._personal_access_token = token
        except Exception as err:
            msg = (
                f"could not get {constants.GITHUB_PERSONAL_ACCESS_TOKEN} from {constants.SECRETS_ENV_PATH}. "
                f"err={err}. Please read topic 'Token' on https://pypi.org/project/hdsr-pygithub/"
            )
    ...
    def _validate_personal_access_token(self, github_instance: Github) -> Github:
        try:
            user_url = github_instance.get_user().html_url  # noqa
            return github_instance
        except BadCredentialsException:
            logger.error(f"your github personal access token '{self.personal_access_token}' is invalid")
            raise
        except Exception as err:
        ...
        try:
            ...
            if self.__public_repo_works_okay():
                msg += (
                    ...
                    f"and does user={user} have access? err={err.__class__.__name__}: {err}"
                )
            else:
                msg += (
