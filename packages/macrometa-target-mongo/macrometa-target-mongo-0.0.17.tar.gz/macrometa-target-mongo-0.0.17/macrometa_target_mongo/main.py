    ...
    if record_batch.length() >= record_batch.max_batch_size or (force and record_batch.length() > 0):
        ...
        try:
            if operations:
                ...
                # Calculate the total number of records written
                total_written = result.matched_count + result.modified_count + result.upserted_count
                logger.info(f"Upserted {total_written} records into {collection.full_name}")
