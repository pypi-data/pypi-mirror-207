...
class TemplateHTMLRendererTests(TestCase):
    ...
    def _monkey_patch_get_template(self):
        ...
        """
        self.get_template = skailar.template.loader.get_template

        def get_template(template_name, dirs=None):
            if template_name == 'example.html':
                return engines['skailar'].from_string("example: {{ object }}")
        ...
        skailar.template.loader.get_template = get_template
        skailar.template.loader.select_template = select_template

    def tearDown(self):
        """
        Revert monkeypatching
        """
        skailar.template.loader.get_template = self.get_template
    ...
    def setUp(self):
        ...
        def get_template(template_name):
            if template_name == '404.html':
                return engines['skailar'].from_string("404: {{ detail }}")
            if template_name == '403.html':
                return engines['skailar'].from_string("403: {{ detail }}")
            raise TemplateDoesNotExist(template_name)
