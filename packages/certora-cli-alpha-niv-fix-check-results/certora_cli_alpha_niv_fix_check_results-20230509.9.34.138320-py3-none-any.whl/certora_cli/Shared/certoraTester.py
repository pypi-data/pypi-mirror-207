        ...
        for rule, ruleResult in rulesResults.items():
            if rule in expectedRulesResults.keys():
                if isinstance(ruleResult, str):  # flat rule ( ruleName: result )
                    ...
                    if isinstance(expectedRulesResults[rule], str):
                        for key, val in ruleResult.items():
                            if key == expectedRulesResults[rule]:
                                if len(val) == 0:
                                    test = False
                                    appendViolation(table, testName, ruleResult, expectedRulesResults[rule], rule, "")
                            else:
                                if len(val) != 0:
                                    test = False
                                    appendViolation(table, testName, ruleResult, expectedRulesResults[rule], rule, "")
