...
class AlphaDatabase(AlphaDatabaseCore):
    ...
    def commit(self, close=False, session=None, force: bool = False) -> bool:
        if self.autocommit and not force:
            return True
        if session is None:
    ...
    def get_table_model(self, table: str, bind: str | None = None):
        binds = [bind] if bind is not None else None
        models = self.get_tables_models(tables=[table], binds=binds)
        model = next(
            iter(models),
            None,
        )
    ...
    def select_query(
        ...
        if not converted:
            if not self.use_schemas:
                results_json = [db_utils.instance_to_dict(res) for res in results] if type(results) == list else db_utils.instance_to_dict(results)  
        ...
        for bind, tables_models in tables_by_bind.items():
            ...
            if drop or create:
                try:
                    ...
                    log.info(f"Init of {bind=} with engine {str(meta.bind.engine)}")
                except:
