...
def sqlacodegen(url, schemas=None, table_names=None, file=None, ignore=False):
    packages = {
                    ...
                    for foreign_key in t.foreign_keys:
                        if hasattr(foreign_key, "column"):
                            ...
                            if child_field not in db_table_info_map[t.schema][child_table]:
                                foreign_key_column = fr'{parent_table}.{parent_field}' if ignore else \
    ...
    for db, db_indo in db_table_info_map.items():
        for table, table_info in db_indo.items():
            ...
            table_name = f"{db}_{table}" if not ignore else table
            ...
            if relationship:
                for child_table, relationship_info in relationship.items():
                    ...
                    relation_table = f"{child_db}_{child_table}" if not ignore else child_table
                    self_table = f"{parent_db}_{parent_table}" if not ignore else parent_table
