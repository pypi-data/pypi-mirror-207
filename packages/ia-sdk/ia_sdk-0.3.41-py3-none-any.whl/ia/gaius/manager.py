...
class AgentManager:
    ...
    def start_hoster(self):
        ...
        if output.returncode != 0:
            raise Exception(f'Starting hoster failed with exitcode {output.returncode}')
        return
    ...
    def stop_hoster(self):
        ...
        output = subprocess.run(STOP_COMMAND,
                        ...
                        shell=True)
        if output.returncode != 0:
            raise Exception(f'Stopping hoster failed with exitcode {output.returncode}')
        return
    ...
    def delete_genome(self, genome_name):
        ...
        for name, agent in self.current_agents.items():
            if agent.genome_name == genome_name:
                raise Exception(f'Genome {genome_name} in use by Agent {name}. Please kill agent before removing genome')
