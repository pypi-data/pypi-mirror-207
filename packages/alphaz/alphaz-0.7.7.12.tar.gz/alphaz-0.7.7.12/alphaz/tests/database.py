...
class Database(AlphaTest):
    ...
    def childs_compare(self):
        ...
        tc1 = {x: y for x, y in self.test_p_childs_1.items()}
        tc2 = {x: y for x, y in self.test_p_childs_2.items()}

        tc1["parent"] = {x: y for x, y in self.test_p.items()}
        tc2["parent"] = {x: y for x, y in self.test_p.items()}
