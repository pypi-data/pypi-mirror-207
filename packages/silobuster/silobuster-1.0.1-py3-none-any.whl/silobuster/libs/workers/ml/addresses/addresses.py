...
def dedupe_io_addresses(df: pd.DataFrame, **kwargs):
    '''
    Requires:
    keyword arguments:
    ...
    # Create a new column that concatenates the values from the street number, street name, street type, unit, city, state, and zip columns
    formatted_df['key'] = formatted_df.apply(lambda x: ''.join([str(x[col]).replace(' ', '').lower() for col in ['numerical', 'street_name', 'street_type', 'unit', 'city', 'state_province', 'postal_code']]), axis=1)

    # Remove rows with NaN values in the 'street number', 'street name', 'street type', and 'unit' columns
    # formatted_df.dropna(subset=['numerical', 'street_name', 'street_type', 'unit'], inplace=True)
    ...
    deduper_df = pandas_dedupe.dedupe_dataframe(df=formatted_df, field_properties=[
        ...
        config_name=curpath + '/dedupe_io_addresses_only',
    )

    try:
        deduper_df.drop(columns='cluster_id', axis=1, inplace=True)
    except:
        pass
    
    deduper_df.reset_index(inplace=True)
