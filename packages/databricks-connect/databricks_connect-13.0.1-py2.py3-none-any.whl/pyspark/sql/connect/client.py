...
class SparkConnectClient(object):
    ...
    def retry_exception(cls, e: grpc.RpcError) -> bool:
        ...
        if e.code() == grpc.StatusCode.INTERNAL:
            # There are certain cases where the control plane will return an INTERNAL error whereas
            # the exception is actually retriable. This is a workaround until the control plane
            # will return the correct exceptions and is simply additional protection.
    ...
    def __init__(
        ...
        self._ping_stub = grpc_lib.SparkConnectServiceStub(self._channel)
    ...
    def _execute_ping(self) -> None:
        """
        This is a helper method that will send a nonce request to the
        server extracting the schema for a range(0,2,1) query. To avoid
        executing this as a full query, this is sent via the Analyze RPC.

        The purpose of this request is to avoid the TCP timeout of the
        ...
        try:
            for attempt in Retrying(
                ...
                with attempt:
                    # Using the ping stub
                    resp = self._ping_stub.AnalyzePlan(req, metadata=self._builder.metadata())
                    if resp.session_id != self._session_id:
                        raise SparkConnectException(
                            "Received incorrect session identifier for request:"
                            f"{resp.session_id} != {self._session_id}"
    ...
    def _ping_handler(self) -> None:
        ...
        while not self._channel_closed:
            if self._ping_active > 0:
                self._execute_ping()
            time.sleep(SparkConnectClient.PING_TIMEOUT)
