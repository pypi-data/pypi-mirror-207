...
class PostIO(object):
    ...
    def _detect_relations(self, entry: dict, group_rel_types: List[str], chunk2group: dict):
        new_relations = []
        for rel_type, head, tail in entry['relations']:
            if rel_type not in group_rel_types:
                if chunk2group[head] is None and chunk2group[tail] is None:
                    ...
                    curr_new_relations = [(rel_type, head, ck) for ck in chunk2group[tail]]
                elif chunk2group[head] is not None and chunk2group[tail] is None:
                    curr_new_relations = [(rel_type, ck, tail) for ck in chunk2group[head]]
                else:
                    assert len(chunk2group[head] & chunk2group[tail]) == 0
                    curr_new_relations = [(rel_type, hck, tck) for hck in chunk2group[head] for tck in chunk2group[tail]]
                curr_new_relations = [rel for rel in curr_new_relations if rel not in entry['relations'] and rel not in new_relations]
