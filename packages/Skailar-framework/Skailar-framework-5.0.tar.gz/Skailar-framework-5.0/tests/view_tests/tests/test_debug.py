...
from skailar.db import DatabaseError, connection
from skailar.http import Http404, HttpRequest, HttpResponse
from skailar.shortcuts import render
from skailar.template import TemplateDoesNotExist
from skailar.test import RequestFactory, SimpleTestCase, override_settings
from skailar.test.utils import LoggingCaptureMixin
from skailar.urls import path, reverse
from skailar.urls.converters import IntConverter
...
from skailar.views.debug import (
    CallableSettingWrapper,
    ExceptionCycleWarning,
    ExceptionReporter,
)
from skailar.views.debug import Path as DebugPath
from skailar.views.debug import (
...
from skailar.views.decorators.debug import sensitive_post_parameters, sensitive_variables
...
class DebugViewTests(SimpleTestCase):
    def test_files(self):
        with self.assertLogs("skailar.request", "ERROR"):
    ...
    def test_400(self):
        ...
        with self.assertLogs("skailar.security", "WARNING"):
            response = self.client.get("/raises400/")
        self.assertContains(response, '<div class="context" id="', status_code=400)

    def test_400_bad_request(self):
        # When DEBUG=True, technical_500_template() is called.
        with self.assertLogs("skailar.request", "WARNING") as cm:
        ...
        TEMPLATES=[
            {
                ...
                "OPTIONS": {
                    "loaders": [
                        (
                            "skailar.template.loaders.locmem.Loader",
                            {
                                "403.html": (
    ...
    def test_404_not_in_urls(self):
        ...
        self.assertContains(
            response, "Skailar tried these URL patterns", status_code=404
        )
        self.assertContains(
            response,
            "<p>The current path, <code>not-in-urls</code>, didn’t match any "
    ...
    def test_template_loader_postmortem(self):
        ...
        with tempfile.NamedTemporaryFile(prefix=template_name) as tmpfile:
            ...
            ), self.assertLogs("skailar.request", "ERROR"):
                response = self.client.get(
                    reverse(
            ...
            self.assertContains(
                response,
                "<li><code>skailar.template.loaders.filesystem.Loader</code>: "
...
@override_settings(
    ...
    TEMPLATES=[
        {
            "BACKEND": "skailar.template.backends.dummy.TemplateStrings",
        }
    ],
)
class NonSkailarTemplatesDebugViewTests(SimpleTestCase):
    def test_400(self):
        # When DEBUG=True, technical_500_template() is called.
        with self.assertLogs("skailar.security", "WARNING"):
            response = self.client.get("/raises400/")
...
class ExceptionReporterTests(SimpleTestCase):
    ...
    def test_reporting_frames_source_not_match(self):
        try:
            ...
            exc_type, exc_value, tb = sys.exc_info()
        with mock.patch(
            "skailar.views.debug.ExceptionReporter._get_source",
...
class ExceptionReporterFilterTests(
    ...
    @override_settings(SESSION_COOKIE_NAME="skailarsession")
    def test_cleanse_session_cookie_value(self):
        self.client.cookies.load({"skailarsession": "should not be displayed"})
        response = self.client.get("/raises500/")
