...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or aikoai.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(aikoai.api_type)
        )
        self.api_version = api_version or aikoai.api_version
        self.organization = organization or aikoai.organization
    ...
    def request_headers(
        ...
        if aikoai.app_info:
            user_agent += " " + self.format_app_info(aikoai.app_info)
...
async def aiohttp_session() -> AsyncIterator[aiohttp.ClientSession]:
    user_set_session = aikoai.aiosession.get()
    if user_set_session:
