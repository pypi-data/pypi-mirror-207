...
class FeatureDefinitionWrapper:
    ...
    def request_context_keys(self) -> List[str]:
        ...
        if rc_schema is not None:
            return [field.name for field in rc_schema.fields]
        else:
            return []

    @property
    def request_context_schema(self) -> Optional["SparkSchema"]:
        if self.pipeline is None:
            return None

        request_context = pipeline_common.find_request_context(self.pipeline.root)
        if request_context:
            return request_context.schema
        else:
            return None

    # TODO(TEC-13178): Migrate RequestSource schema off SparkSchema so we can return the entire spine schema
    # by using Tecton Schema.
    @property
    def spine_schema(self) -> Tuple[Schema, Optional["SparkSchema"]]:
