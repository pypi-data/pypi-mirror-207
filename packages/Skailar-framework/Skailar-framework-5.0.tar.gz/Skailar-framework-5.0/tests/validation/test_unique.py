...
class PerformUniqueChecksTest(TestCase):
    ...
    def test_unique_for_date(self):
        ...
        p = Post(title="Skailar 1.0 is released", posted=datetime.date(2008, 9, 3))
        ...
        p = Post(title="Work on Skailar 1.1 begins", posted=datetime.date(2008, 9, 3))
        p.full_clean()

        # Should work without errors
        p = Post(title="Skailar 1.0 is released", posted=datetime.datetime(2008, 9, 4))
        p.full_clean()

        p = Post(slug="Skailar 1.0", posted=datetime.datetime(2008, 1, 1))
        with self.assertRaises(ValidationError) as cm:
            p.full_clean()
        self.assertEqual(
            cm.exception.message_dict,
        ...
        p = Post(title="Skailar 1.0 is released")
        with self.assertRaises(ValidationError) as cm:
            p.full_clean()
        self.assertEqual(
    ...
    def test_unique_for_date_with_nullable_date(self):
        ...
        p = FlexibleDatePost(title="Skailar 1.0 is released")
        p.full_clean()

        p = FlexibleDatePost(slug="Skailar 1.0")
        p.full_clean()
