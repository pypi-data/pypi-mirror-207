...
import imaplib
import aiofiles
from settings.settings import FILES_PATH, IMAP_RETRY_SELECT
...
class DownloadFromIMAP(IMAPClient, DownloadFromBase):
    ...
    +-------------------+----------+-----------+-------------------------------------------------------+
    | credentials       |   Yes    | Credentials to establish connection with IMAP mailbox             |
    +-------------------+----------+-----------+-------------------------------------------------------+
    ...
    +-------------------+----------+-----------+-------------------------------------------------------+
    | download_existing |   Yes    | If true, a file that I already detect is downloaded is            |
    |                   |          | downloaded again                                                  |
    +-------------------+----------+-----------+-------------------------------------------------------+
                ...
                if not directory.exists():
                    if self.create_destination is True:
                        ...
                        raise ComponentError(
                            f'DownloadFromIMAP: Error creating destination directory: {directory}'
                        )
    ...
    async def run(self):
        ...
        try:
            ...
            result, msgs = self._client.search(None, *filter_criteria)  # , charset='US-ASCII'
            ...
            if resp == 'OK':  # mail was retrieved
                ...
                for attachment in msg.attachments:
                    ...
                    if file_path.exists() and self.overwrite is False:
                        # TODO: before to save a new renamed file, we need to check if we have it (checksum)
