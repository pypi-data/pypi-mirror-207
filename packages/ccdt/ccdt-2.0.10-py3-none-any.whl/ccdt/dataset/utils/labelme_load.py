...
class LabelmeLoad(object):
    ...
    def hanzi_to_pinyin(self):
        new_dir = list()
        for root, dirs, files in tqdm(os.walk(self.parameter.input_datasets[0].get('input_dir'), topdown=True)):
            if root.count('00.images') == 1:
                build_dir = {}
                old_path = root
                # 把中文路径转拼音路径
                new_path = self.convert_path_to_pinyin(old_path)
                build_dir.update({old_path: new_path})
                if build_dir not in new_dir:
                    new_dir.append(build_dir)
        # 重命名路径
        for rename_dir in new_dir:
            # 将列表中的元素转换为字符串
            old_dir = ','.join(rename_dir.keys()).replace('\\', '/')
            new_dir = ','.join(rename_dir.values()).replace('\\', '/')
            try:
                os.rename(old_dir, new_dir)
            except OSError as e:
                print(f"重命名目录 {old_dir} 失败: {e}")
                # # 创建目录
