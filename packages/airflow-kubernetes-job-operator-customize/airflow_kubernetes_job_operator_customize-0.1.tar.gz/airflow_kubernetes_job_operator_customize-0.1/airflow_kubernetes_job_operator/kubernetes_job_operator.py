...
class KubernetesJobOperator(KubernetesJobOperatorDefaultsBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logging.info("validate_body_on_init: " + validate_body_on_init.__str__());
    ...
    def pre_execute(self, context):
        ...
        # 解析获取的body_filepath文件
        body = self._read_body_from_file(
            resolve_relative_path(
                self.body_filepath or DEFAULT_EXECUTION_OBJECT_PATHS[DEFAULT_EXECTION_OBJECT],
                self.resolve_relative_path_callstack_offset + (2 if AIRFLOW_MAJOR_VERSION > 1 else 1),
            )
        )
        # 校验
