...
from skailar.core.mail.backends import console, dummy, filebased, locmem, smtp
from skailar.core.mail.message import BadHeaderError, sanitize_address
from skailar.test import SimpleTestCase, override_settings
from skailar.test.utils import requires_tz_support
from skailar.utils.translation import gettext_lazy
from skailar.utils.version import PY311
...
class MailTests(HeadersCheckMixin, SimpleTestCase):
    ...
    def get_decoded_attachments(self, skailar_message):
        """
        Encode the specified skailar.core.mail.message.EmailMessage, then decode
        ...
        """
        msg_bytes = skailar_message.message().as_bytes()
    ...
    def test_backend_arg(self):
        ...
        self.assertIsInstance(
            mail.get_connection("skailar.core.mail.backends.smtp.EmailBackend"),
            smtp.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("skailar.core.mail.backends.locmem.EmailBackend"),
            locmem.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("skailar.core.mail.backends.dummy.EmailBackend"),
            dummy.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("skailar.core.mail.backends.console.EmailBackend"),
            console.EmailBackend,
        )
        with tempfile.TemporaryDirectory() as tmp_dir:
            self.assertIsInstance(
                mail.get_connection(
                    "skailar.core.mail.backends.filebased.EmailBackend",
                    file_path=tmp_dir,
                ),
        ...
        with self.assertRaisesMessage(TypeError, msg):
            mail.get_connection(
                "skailar.core.mail.backends.filebased.EmailBackend", file_path=object()
            )
        self.assertIsInstance(mail.get_connection(), locmem.EmailBackend)

    @override_settings(
        EMAIL_BACKEND="skailar.core.mail.backends.locmem.EmailBackend",
    ...
    def test_connection_arg(self):
        ...
        self.assertEqual(mail.outbox, [])
        self.assertEqual(len(connection.test_outbox), 1)
        self.assertEqual(connection.test_outbox[0].subject, "[Skailar] Admin message")

        connection = mail.get_connection("mail.custombackend.EmailBackend")
        mail_managers("Manager message", "Content", connection=connection)
        self.assertEqual(mail.outbox, [])
        self.assertEqual(len(connection.test_outbox), 1)
        self.assertEqual(connection.test_outbox[0].subject, "[Skailar] Manager message")
    ...
    def test_dont_base64_encode_message_rfc822(self):
        ...
        # Feature test: try attaching Skailar's EmailMessage object directly to the mail.
        parent_msg = EmailMessage(
            "Parent Subject",
            "Some parent body",
            "bounce@example.com",
...
class PythonGlobalState(SimpleTestCase):
    """
    Tests for #12422 -- Skailar smarts (#2472/#11212) with charset of utf-8 text
    parts shouldn't pollute global email Python package charset registry when
    skailar.mail.message is imported.
    """
...
class BaseEmailBackendTests(HeadersCheckMixin):
    ...
    def test_html_mail_managers(self):
        ...
        self.assertEqual(message.get("subject"), "[Skailar] Subject")
        self.assertEqual(message.get_all("to"), ["nobody@example.com"])
        self.assertTrue(message.is_multipart())
        self.assertEqual(len(message.get_payload()), 2)
        self.assertEqual(message.get_payload(0).get_payload(), "Content")
    ...
    def test_recipient_without_domain(self):
        ...
        """
        self.assertTrue(send_mail("Subject", "Content", "tester", ["skailar"]))
        message = self.get_the_message()
        self.assertEqual(message.get("subject"), "Subject")
        self.assertEqual(message.get("from"), "tester")
        self.assertEqual(message.get("to"), "skailar")
    ...
    def test_lazy_addresses(self):
        ...
        """
        _ = gettext_lazy
        self.assertTrue(send_mail("Subject", "Content", _("tester"), [_("skailar")]))
        message = self.get_the_message()
        self.assertEqual(message.get("from"), "tester")
        self.assertEqual(message.get("to"), "skailar")
...
class FileBackendTests(BaseEmailBackendTests, SimpleTestCase):
    email_backend = "skailar.core.mail.backends.filebased.EmailBackend"
...
class ConsoleBackendTests(BaseEmailBackendTests, SimpleTestCase):
    email_backend = "skailar.core.mail.backends.console.EmailBackend"
    ...
    def test_console_stream_kwarg(self):
        ...
        s = StringIO()
        connection = mail.get_connection(
            "skailar.core.mail.backends.console.EmailBackend", stream=s
        )
        send_mail(
            "Subject",
            "Content",
...
class SMTPBackendTests(BaseEmailBackendTests, SMTPBackendTestsBase):
    ...
    def test_connection_timeout_default(self):
        ...
        connection = mail.get_connection("skailar.core.mail.backends.smtp.EmailBackend")
