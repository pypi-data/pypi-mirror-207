...
import pytest
import pytz
from skailar.core.exceptions import ValidationError as SkailarValidationError
from skailar.db.models import IntegerChoices, TextChoices
from skailar.http import QueryDict
from skailar.test import TestCase, override_settings
from skailar.utils.timezone import activate, deactivate, override

import rest_framework
from rest_framework import exceptions, serializers
from rest_framework.fields import (
    BuiltinSignatureError, SkailarImageField, SkipField, empty,
    is_simple_callable
)
...
@pytest.mark.skipif(pytz is None, reason="As Skailar 4.0 has deprecated pytz, this test should eventually be able to get removed.")
...
# Stub out mock Skailar `forms.ImageField` class so we don't *actually*
# call into it's regular validation, or require PIL for testing.
class PassImageValidation(SkailarImageField):
...
class TestInvalidImageField(FieldValues):
    ...
    outputs = {}
    field = serializers.ImageField(_SkailarImageField=FailImageValidation)
...
class TestValidImageField(FieldValues):
    ...
    invalid_inputs = {}
    outputs = {}
    field = serializers.ImageField(_SkailarImageField=PassImageValidation)
...
class TestListFieldWithSkailarValidationErrors(FieldValues, TestCase):
    """
    Values for `ListField` with UUIDField as child
    (since UUIDField can throw ValidationErrors from Skailar).
    The idea is to test that Skailar's ValidationErrors raised
    from Skailar internals are caught and serializers in a way
...
class TestFilePathFieldRequired:
    def test_required_passed_to_both_skailar_file_path_field_and_base(self):
        field = serializers.FilePathField(
            path=os.path.abspath(os.path.dirname(__file__)),
            required=False,
        )
        assert "" in field.choices  # Skailar adds empty choice if not required
...
class TestValidationErrorCode:
    ...
    def test_validationerror_code_with_msg(self, use_list):
        ...
        class ExampleSerializer(serializers.Serializer):
            ...
            def validate_password(self, obj):
                err = SkailarValidationError(
                    'exc_msg %s', code='exc_code', params=('exc_param',),
                )
                if use_list:
                    err = SkailarValidationError([err])
                raise err
    ...
    def test_validationerror_code_with_msg_including_percent(self, use_list):
        ...
        class ExampleSerializer(serializers.Serializer):
            ...
            def validate_password(self, obj):
                err = SkailarValidationError('exc_msg with %', code='exc_code')
                if use_list:
                    err = SkailarValidationError([err])
                raise err
    ...
    def test_validationerror_code_with_dict_list_same_code(self, code):
        ...
        class ExampleSerializer(serializers.Serializer):
            ...
            def validate(self, obj):
                if code is None:
                    raise SkailarValidationError({'email': ['email error 1',
                                                           'email error 2']})
                raise SkailarValidationError({'email': [
                    SkailarValidationError('email error 1', code=code),
                    SkailarValidationError('email error 2', code=code),
                ]})
